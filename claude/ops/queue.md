...
## Landed 2026-08-08 (agent team, five branches, verified before push)

Full gate on the merged tree: node **2703/2703** across 94 suites, `tsc`
clean, UI **1160/1160**. Merged in this order, least-entangled first, because
`session-gone` and `delete-search` both touch `lib/activity.js` and
`lib/server.js`.

- **Insights coverage copy** — the banner explained our fetch loop to users
  ("page limit", "newest first", "a floor") and opened with a warning glyph
  every other use of which means something is broken. Rewritten, both states,
  plus the two adjacent strings so the screen stops contradicting itself.
- **Insights breakdowns counted in the database** (migration **055**,
  UNAPPLIED). Six per-team breakdowns were folded from ≤2,000 paged rows; at
  2,641 rows over 26 days ~641 never reached them. Headline tiles were never
  capped — 027 already counted those. Also fixed `concentration` quoting the
  *fetch* span over a *window* group, which named a real person as a project's
  only contributor over 60d of a 30d group, on the default path.
- **Git clean filter** — the injected block no longer churns `git diff` and no
  longer conflicts on merge. Enabled from `ensureInstalled()`, not
  `setupHooks()`: a user who clicks "Not now" on first-run consent never calls
  the latter. `git status` still shows ` M` on a length-changing rewrite; git's
  index size shortcut fires before any filter runs and no filter can beat it.
- **Teammate sessions open** — search indexed the working cache *plus* the
  durable team archive; `sessionPayload` read the cache only. 39 of 99 indexed
  sessions were unopenable and every one belonged to a teammate. Security
  review cleared the new archive gate before landing.
- **Deleting a project purges its search rows** — `deleteProject` never opened
  `search.db`, so verbatim prompts, summaries and pulled teammate rows stayed
  searchable indefinitely through `/api/search`, the MCP `search_memory`, and
  the PreToolUse Grep hook that injects memory into agents unobserved.

Migration **056** (UNAPPLIED) also landed: `team_feed`'s PUBLIC grant has been
silently regenerated by four `drop`+`create` cycles since 010's revoke, and
054's `revoke ... from anon` was a no-op because `anon` holds no direct grant.
Paired with a `grant` so `authenticated` survives any future recreate.

## Next, in order

1. **Pausing a project bypasses revocation in `search_memory`** — the only
   item here that is a live defect rather than work waiting to ship. Detection
   skips paused projects before the access check, and enforcement builds
   `excludeTeamFor` from tracked projects so a paused one can never enter it.
   A revoked ex-teammate's agent still reaches that project's team content.
   Confirmed with probes; the settling test is in the ticket.
2. **Apply 055 and 056**, human-only. Both reviewed, both with rollbacks, both
   dated UNAPPLIED. Read `APPLY-RUNBOOK.md` — see the reconcile note below
   before trusting any row in it.
3. **Audit the surfaces that read `state.projects` raw** — `liveSessionsPayload`
   and others skip `isProjectOff`. Same family as item 1; two agents hit this
   seam independently from opposite directions.
4. **Close the `migration-state` blind spot** — its "no header contradicts the
   ledger" check short-circuits, so header-UNAPPLIED + ledger-applied passes
   silently. A green run is therefore not evidence the headers agree.
5. **Retention across re-add** — Marco's "keep it searchable" option. Deferred
   deliberately: a project-scoped marker cannot carry it, `replaceProject`
   deletes by project, and shipping the choice before that is solved would
   silently reverse a decision the user made.
6. **`ACCOUNT-DELETION.md` §2.4** names two derived copies that survive a
   backend deletion and misses `search.db`.
7. **Session page should show where a session came from** — render `heldBy` /
   `unavailable`, now that `/api/session` returns them.

## Migration ledger reconciled 2026-08-08 — read this before applying anything

`MIGRATION-STATE.md` and `APPLY-RUNBOOK.md` claimed **054 and nine of twelve
rows in 041–053 were NOT applied. They are live.** Verified read-only against
production, each row by its own nominated query. 049's settling query was
itself defective — it looked for a trigger name that does not exist — and has
been fixed along with its status. Genuinely unapplied: 047, 048, 053; 052
stays PARKED.

**031, 037, 038, 039 and 040 have still not been reconciled.** Same document,
same failure mode; nine of twelve were wrong, so do not assume these five are
right. 031 is the `016` shape — a reconstruction of a live object that predates
the repo — and no automated criterion can settle it.

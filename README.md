...
Capture depth varies by tool. Claude Code has a session-end hook, so its
summaries are written by the agent on purpose; tools without one — Codex
among them — contribute a summary harvested from the last chat message,
which carries what the session concluded but not the separate intent,
decisions and gotchas. See

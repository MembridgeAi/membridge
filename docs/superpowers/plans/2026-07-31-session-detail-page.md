...
Goal: A feed row previews on hover and routes to a session page: 1-2 sentence header, Intent, meta row, five collapsible brief widgets, and the full prompt chain newest-first -- per the spec at `docs/superpowers/specs/2026-07-31-session-detail-page-design.md`.
...
- [ ] Human pass with the real daemon: open a session with full fields, one with empty `decisions`/`gotchas`, one live, and one long session; cmd-click a row; keyboard-tab the whole page; toggle dark/light.

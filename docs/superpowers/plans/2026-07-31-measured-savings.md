...
- [x] **REVERT DONE.** An earlier commit had implemented a now-reversed instruction and stopped persisting USD. That code change was reverted: `lib/ledger-fold.js` accumulates and persists `inCost`/`outCost` again, `lib/ledger-fold-state.js` migrates a legacy ledger's dollar fields forward instead of dropping them, and three checks pin it (the v3 migration keeps them, the on-disk ledger carries them, a legacy ledger keeps them through a fold). The comment at `lib/server.js` is corrected to say computed and stored, never served. **Do not re-reverse this.** The rule is about the wire; the persisted figure is what lets a user-supplied-rate feature reprice history without re-folding.

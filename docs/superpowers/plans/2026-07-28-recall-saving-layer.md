...
Payload exactly per spec §8.5 (install_id random uuid persisted in `~/.membridge/config.json`, version, net_tokens, acceptance, reads_answered, reject_reasons, languages) **plus `direct_avoided` and `compression_realization`** (renamed from `holdout_divergence` — final whole-branch review, MINOR 5: both terms of that ratio live on the direct-avoidance side, so it cannot detect the second-order effects §7.2 exists to find, and the old name read as if it did) so the served/held-out arms can be pooled across installs — that pooling is the whole reason the holdout exists, and it is impossible without these two fields — **no code, file names, project names, or account**. Note: the real §7.2 divergence check needs per-session total volume, which this payload does not carry, and remains future work. POST to `config.diagnosticsUrl` (default the Supabase function URL constant; honour `diagnostics.enabled === false` and `MEMBRIDGE_NO_DIAGNOSTICS=1`). Network send is fire-and-forget with a 5 s timeout; **tests never hit the network** — point `diagnosticsUrl` at the suite's local mock server and assert the payload shape and the privacy fields' absence.

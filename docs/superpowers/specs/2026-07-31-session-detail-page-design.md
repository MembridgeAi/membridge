...
- The row is a real link (`<a>` with an href to the route), not a click handler on a div -- middle-click and cmd-click must open a session in a new window in the Electron shell.

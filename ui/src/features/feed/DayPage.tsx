...
import { FEED_SESSION_PARAM, ROUTES, backLink, dayHref, daySessionHref, parseDayFilters, sessionHref, useRawSearch } from '../../app/routes'
...
import { dayLabel, dedupeById } from './FeedPage'
import './feed.css'
  ...
  return (
    <div ref={ref} className={`day-session${targeted ? ' day-session-targeted' : ''}`}>
      <div className="day-session-head">
        {session.session
          ? (
            <Link href={sessionHref(session.session, from)} className="day-session-link">
              {range}
...
/** Hard ceiling on how far the day view will page backwards on its own before
 *  it hands over to "Look further back". Same number FeedPage caps its own
 *  auto-paging at: five pages of 30 rows is already a long way past what a
 *  reader would sit through, and the button is right there. */
...
export function DayPage({ slug }: DayPageProps) {
  ...
  // UNFILTERED here -- which this used to do, with a comment claiming a cache
  // hit -- was only ever a cache hit when no filter was set. With one set it
  // was a different query key: a cold start, one 30-row unfiltered page, no
  // paging, and a card the reader had just clicked that was frequently not in
  // it. The filters ride on the URL (routes.dayHref) so the two agree.
  ...
  const cards = useMemo(() => buildDayCards(entries), [entries])
  const card = cards.find(c => c.slug === slug) ?? null

  // Page backwards until this ONE day is covered, and not one request further.
  //
  // A cache hit off the Feed needs none of this. A day URL opened cold does:
  // a pasted link, a reload, a cmd-clicked card. The feed loads recent activity
  ...
  // whole days": stop as soon as the OLDEST loaded entry is older than the day
  // in the slug, because everything this day could contain is then already
  // loaded. That also keeps paging when the card is already on screen but
  // incomplete -- the day's earlier sessions can sit one page down, and a day
  // view missing half its prompts is a quieter wrong than a missing one.
  //
  ...
  // nothing stops the loop instead of spinning it. MAX_DAY_PAGES is the
  ...
  const day = useMemo(() => daySlugDay(slug), [slug])
  const oldestLoadedDay = useMemo(() => entries.reduce<string | null>((oldest, e) => {
    const d = localDayKey(new Date(e.at))
    return oldest === null || d < oldest ? d : oldest
  }, null), [entries])
  const dayCovered = !day || (!!oldestLoadedDay && oldestLoadedDay < day)
  const autoPagedFrom = useRef(-1)
  const pageCount = feedQuery.data?.pages.length ?? 0
  useEffect(() => {
    if (dayCovered) return
    if (pageCount >= MAX_DAY_PAGES) return
    ...
    <div className="day-page">
      ...
      <div className="day-head">
        <div className="day-head-who">
          ...
          <div>
            <div className="day-head-name">{card.author}</div>
            <div className="mono day-head-sub">

-- Rollback for 044_removal_rotates_invite_code.sql.
--
...
--
-- Both bodies below are the PRE-044 definitions restated verbatim:
...
-- 044 changed exactly two function bodies and touched no table, no policy and
-- no data, so the prior state is fully described by those two files.
--
-- WHAT IT CANNOT UNDO. Every invite code and invite link that 044 already
...
-- 044 was live still needs a fresh invite after this rollback. There is no
...
-- genuinely stranded, and re-apply 044 immediately afterwards.

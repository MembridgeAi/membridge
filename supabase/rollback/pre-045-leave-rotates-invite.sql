-- Rollback for 045_leave_rotates_invite_code.sql.
--
...
--
-- The body below is the PRE-045 definition restated verbatim from
-- 002_team_v2.sql:252. Nothing was read out of the live system to write this:
-- 045 changed exactly one function body and touched no table, no policy and no
-- data, so the prior state is fully described by that file.
--
-- Independent of pre-044: rolling back one does not require rolling back the
-- other. They restore different functions.
--
-- WHAT IT CANNOT UNDO. Every invite code and invite link that 045 already
...
-- that happened while 045 was live still needs a fresh invite after this
-- rollback.
--
...
-- code, which after 044 §2 is the only role that still receives it — walks

...
| ~~19~~ | `057_member_identity.sql` | **ALREADY APPLIED — SKIP.** Applied 2026-08-13 to `mefgbiecvoszjorwzkfz` and catalog-verified the same day (see its row in [`MIGRATION-STATE.md`](./MIGRATION-STATE.md)). Do not paste it. Its ACL gap is closed by step 20 (`059`), which you DO still need to run. Lets a member rename themselves and pick an avatar, with display names enforced unique per team by a partial unique index (not a client check). Adds columns to `team_members`, a dedupe-on-insert trigger, and `set_display_name`. **NOT independent of 053 — apply-order matters.** §3 drops and recreates `team_members_list` (053's function) to add `avatar`/`avatar_color` to what it returns, carrying 053's `deleted_at` column forward in the same definition — apply THIS file and skip 053 entirely; do not apply 053 afterward (see 053's row above for what that breaks). Independent of everything ELSE above — any order, on its own, with respect to non-053 files. *(member-identity lane)* |
| ~~20~~ | `059_revoke_unique_member_name_authenticated.sql` | **ALREADY APPLIED — SKIP.** Applied and verified 2026-08-13 (see its row in [`MIGRATION-STATE.md`](./MIGRATION-STATE.md)). Do not paste it.  057 is already applied to production, and as applied its revoke line only removed `public`/`anon` from `unique_member_name`/`team_members_dedupe_name` — it never named `authenticated`, which Supabase grants EXECUTE to on every new public-schema function by default. This file closes that: two `revoke ... from authenticated` statements, nothing else. **Must go AFTER 057, not before** — both functions have to exist first, or the `revoke` raises "function does not exist" on a fresh database. On `mefgbiecvoszjorwzkfz` this is moot (057 already ran), but say it plainly for any other environment. Otherwise independent of everything else in this table, including 058 — no `create function`, no other ordering constraint. *(member-identity lane)* |

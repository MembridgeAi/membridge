-- 028_delete_own_entries.sql: self-serve deletion of a member's OWN synced
...
--
-- WHY THIS EXISTS. public.memory_entries has SELECT (schema.sql, narrowed by
-- 025 §2), INSERT (schema.sql) and UPDATE (012) policies and NO DELETE policy
-- at all. RLS defaults closed, so a DELETE through a user JWT currently
-- affects zero rows and reports success. There has never been a way for a
-- person to remove what they pushed, from any surface.
--
-- ⚠ Deploy gate, same discipline as 009 and 013: apply this to the LIVE
-- Supabase BEFORE shipping the client that calls it, and in that order. The
-- offline suite runs against test/mock-supabase.js, which implements these
-- three objects, so a missing live migration will NOT be caught by CI. What a
-- user would see instead is the worst possible failure for this particular
-- feature: `delete_my_entries` 404s (PGRST202, "Could not find the function"),
-- the daemon surfaces an error, and the person who just asked for their data
-- to be gone is told something went wrong with no idea whether any of it was
...
-- (`create policy` has no `if not exists`/`or replace`, see 011 §1's note,
-- the same convention 012, 024 and 025 already follow). Run in the Supabase
-- SQL editor (one transaction) or `supabase db push`; with psql, use
-- `psql -1 -f`.
--
-- If an EARLIER cut of this file was already applied anywhere, re-apply it:
-- delete_my_entries returned a bare integer then and returns one row per
-- project now (§3 explains why), and a client reading the new shape from the
-- old function watermarks the wrong set of projects on the machine it runs on.
--
...
-- OTHER MEMBER's entries in it. A member deleting their own data must never
-- be able to touch a teammate's. Empty project rows are harmless.
...
-- the way the SELECT policy does (025 §2). Both of those predicates are about
-- READING a project, and ANDing either one in here would strand exactly the
-- person most likely to want their data gone:
--
...
--     actor_id = auth.uid() (024, tightened by 025 §5). A plain member POSTing
--     their own audit row is refused by RLS, always.
...
-- is precisely the property Andrew decided this feature must have. Inside a
-- `security definer` function the insert runs as the function owner, which is
-- the table owner, and a table owner is not subject to that table's RLS unless
-- the table carries FORCE ROW LEVEL SECURITY (it does not), so the row lands
-- for a member, an admin and an owner alike. Same transaction as the delete:
-- there is no window in which the rows are gone and the record of it is not.
--
...
-- used to return integer, so the old signature is dropped first. Re-runnable
-- either way (`if exists`), same discipline as the drop-then-create policy in
-- §1. Dropping takes its grants with it; they are re-issued at the foot of
-- this file.

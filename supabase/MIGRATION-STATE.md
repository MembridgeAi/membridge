...
| 054 `sec_jamal_01` | **NOT applied (this branch)** | Three verify queries, one per finding. (1) `select prosrc from pg_proc where proname = 'peek_invite';` — applied when the body returns NULL for `team_name` on invalid invites (a `case ... else null`). (2) `select pg_get_functiondef('public.can_see_project(uuid)'::regprocedure);` — applied when the third arm of the coalesce is `false`. (3) `select has_function_privilege('anon', 'public.projects_materialize_access()'::regprocedure, 'EXECUTE');` — applied when it returns `f`. File on `agent-sec-jamal-01` |

...
| 057 `member_identity` | **NOT applied** | `select proname, prosecdef from pg_proc where proname in ('set_display_name','normalize_member_name','unique_member_name','team_members_dedupe_name');` — applied when all four exist and the three non-`normalize_member_name` rows carry `prosecdef = true`. Also: `select indexdef from pg_indexes where indexname = 'team_members_display_name_unique';` — applied when it returns a partial unique index. New columns (`team_members.avatar`, `team_members.name_released_at`), a new trigger, and new functions; nothing existing is dropped or replaced destructively. File on `feat/member-identity-rename` |

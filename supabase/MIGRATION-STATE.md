...
| 031 `ensure_rls_event_trigger` | not applied (never apply) | **Re-verified 2026-08-08. The status reads "not applied (never apply)" and both halves are load-bearing.** For a RECONSTRUCTION the two senses of "applied" come apart: the FILE has never been run (so, not applied), while the BEHAVIOUR it exists to guarantee is already live in production (so there is nothing to gain by running it). Marking it a bare "applied" was accurate about the behaviour and hid the file's status; marking it a bare "not applied" invites someone to fix that by pasting it. **Never paste this file** — see `APPLY-RUNBOOK.md`, where it is struck for this reason. If 031 ever needs to change, diff `pg_get_functiondef` against the file and write a new migration for the delta. This row previously said NOT applied because "live `rls_auto_enable` lacks 031's fail-closed body". That was FALSE, and it was the dangerous direction to be wrong in: it told a reader production's RLS auto-enable fails OPEN when it fails CLOSED. Live `pg_get_functiondef` carries `set search_path to 'pg_catalog'`, the `raise exception 'rls_auto_enable: refusing to create % without row-level security'`, its `detail`/`hint` text and the skip-logging branch — every distinctive string in the file is present live. The file remains a RECONSTRUCTION, so still diff before any `create or replace`; but the behaviour 031 exists to guarantee is already in production |
...
| 035 `delete_own_entries` | **PARTIALLY applied** | **Reconciled 2026-08-08, read-only against production.** Both RPCs are live: `delete_my_entries(uuid, uuid)` and `my_entry_counts(uuid)`, definer, granted to `authenticated`+`service_role` only. The `memory_entries_delete` policy is **ABSENT** — `select count(*) from pg_policy where polrelid='public.memory_entries'::regclass and polname='memory_entries_delete'` returns **0**; the only policies present are `memory_entries_insert`, `memory_entries_select`, `memory_entries_update`. **That absence is CORRECT and deliberate — do NOT "restore" the policy to make production match this file.** 040 revoked the DELETE privilege and 051 drops the policy on purpose (both applied): a DELETE needs a grant AND a permissive policy, so with no policy RLS denies outright, and re-adding it is what would let an accidentally-restored grant through. `aclexplode` confirms DELETE is held by `postgres, service_role` only. Self-serve deletion still works because `delete_my_entries` is `security definer` and runs outside RLS. **Two prior claims in this row were false**: the policy was never "live", and the file is NOT off-branch — `supabase/migrations/035_delete_own_entries.sql` is on `master` |
...
| 053 `team_members_list_deleted_at` | **NOT applied** | `select prosrc from pg_proc where proname = 'team_members_list';` — applied when the returned columns include `deleted_at`. File lives on `agent-deletion`. Independent of 052 — either order, either alone; unlike 052 this one is ready and should ship with the next batch |
...
| 058 `owner_exit` | applied | `select proname from pg_proc where proname in ('transfer_ownership', 'delete_team') and pronamespace = 'public'::regnamespace;` — applied when it returns BOTH rows. Purely additive: two NEW functions, nothing dropped or replaced, so the rollback (`supabase/rollback/pre-058-owner-exit.sql`) is a plain drop of both. Fixes a trap rather than a leak: `leave_team` refuses every owner (045:96), `set_role` cannot grant 'owner' (002:179) and no `delete_team` existed, so every team owner was permanently bound to every team they created. `set_role` is deliberately UNTOUCHED — a transfer is two atomic row changes and set_role updates one, so widening it would leave two owners. `delete_team` is gated on owner AND a member count of exactly 1 (owner's call, 2026-08-12): disband cascades teams→projects→memory_entries and is the only operation in the product that destroys memory instead of revoking access, so it may not reach other people's rows. DEPENDS ON 049's cascade guard (049:163) — `audit_member_left()` returns null when its team is already gone, which is what stops the cascade aborting on `team_audit`'s own FK, exactly as 040:23-34 predicted. Revert 049 and disband breaks. Applied by Andrew 2026-08-12 against `mefgbiecvoszjorwzkfz`; re-run the query above rather than trusting this cell. Before-state MEASURED, not inferred: the same query run read-only shortly before the apply returned NEITHER function while `leave_team` and `set_role` were both present, so the owner trap was confirmed live in production, not just in the migration tree. File on `feat/settings-team-danger-zone` |

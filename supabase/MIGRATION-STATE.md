...
**Changed 2026-08-13:** migrations 057 and 058 were APPLIED to that project (rows
`20260813020145` and `20260813020159`). Neither has been catalog-verified — the
applying session's verification query was blocked, so the evidence is a
successful apply plus a migration-table row, and a migration table is a ledger
like this one. 057's row below is marked `applied (unverified)` for that reason;
058's file lives on `feat/settings-team-danger-zone` and is recorded there.
Applying 057 also made **053 dangerous to apply** — see its row.
...
| 053 `team_members_list_deleted_at` | **NOT applied (do not apply)** | **STOP — CHANGED 2026-08-13. 057 HAS NOW BEEN APPLIED, so this file must never be applied to `mefgbiecvoszjorwzkfz`. 057 §3 already carries this file's `deleted_at` forward; applying 053 now would `create or replace` `team_members_list` back to a definition WITHOUT `avatar`/`avatar_color`, and every teammate's avatar would go blank client-side with no error and nothing failing. The row below still says "should ship with the next batch" — that instruction is now void and is left visible only so the change is legible.** `select prosrc from pg_proc where proname = 'team_members_list';` — applied when the returned columns include `deleted_at`. File lives on `agent-deletion`. Independent of 052 — either order, either alone; unlike 052 this one is ready and should ship with the next batch. **SUPERSEDED BY 057 — do not apply this file if 057 has already run.** `057_member_identity.sql` §3 drops and recreates `team_members_list` and its version already carries `deleted_at` forward, plus adds `avatar`/`avatar_color`. Applying 057 alone is sufficient — 053 needs no separate apply once 057 has run. **WARNING FOR WHOEVER RE-VERIFIES THIS ROW:** the query above (`deleted_at` present) reads TRUE once 057 alone has been applied, because 057's redefinition includes that column. A green result on that query is NOT proof this file ran — check `select proname from pg_proc where proname = 'team_members_list' and prosrc like '%avatar_color%'` too; if avatar_color is present, what is live is 057's definition, not this file's, and this row should stay "NOT applied" (057's row is the one to mark applied instead). Applying 053 AFTER 057 reverts `team_members_list` to a definition with no avatar columns — see 057's row and `APPLY-RUNBOOK.md`'s row 15 for the consequence. |

...
// "Live" is a TIME claim, not a summary claim. This module always had that
// right; the app did not, and answered the same question with "no summary
// yet", which never expires. There is now ONE definition, in util, and this
// is an alias kept for the existing importers of the old name. The window
// also narrowed from a hand-picked 45 min to util's measured 15 min -- see
// util.LIVE_WINDOW_MS for the session-gap data behind that number.
const STALE_GAP_MS = util.LIVE_WINDOW_MS;
...
//   { who, tool, session, ts, ask, summary, decisions, gotchas, live, liveBasis }
// `liveBasis` says what `live` was judged on, because the two origins do not
// support equally strong claims: 'session-events' for your own session (its
// newest event of any kind) and 'synced-row' for a teammate (the newest row
// that reached this machine, which is recent synced activity rather than
// proof they are working now). Same split as lib/feed.js.
...
function fileProvenance(projectPath, proj, config, file, now = Date.now()) {
  ...
  // Bullet fields keep their line breaks; see digest.bulletClip.
  const clippedBullets = (text, n) => (text ? digest.bulletClip(digest.redactText(digest.plainLines(text), regexes), n) : null);
  ...
  const isLive = ts => util.isLive(ts, now);
  ...
  for (const [session, e] of bySession) {
    ...
    rows.push({
      ...
      decisions: best && best.decisions ? clippedBullets(best.decisions, 240) : null,
      gotchas: best && best.gotchas ? clippedBullets(best.gotchas, 240) : null,
  ...
  for (const e of teamLatest.values()) {
    rows.push({
      ...
      decisions: clippedBullets(e.decisions, 240),
      gotchas: clippedBullets(e.gotchas, 240),
      // Arrival, not the authored ts — same rule and same reasoning as
      // lib/feed.js's normalizeTeam. `ts` is stamped on the pushing machine and
      // read hours stale while a teammate was actively working; created_at is
      // the backend's own insert time, and the daemon pushes within ~60s of the
      // work. These two surfaces must never disagree: this one answers the
      // MCP's get_recent_activity, so a teammate asking "what are they doing"
      // through the tool and reading the same row in the app would otherwise
      // get two different answers. Pre-column backends send none, so ts stays
      // the fallback.
      live: isLive(e.created_at || e.ts),

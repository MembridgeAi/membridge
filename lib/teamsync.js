...
// Sign out: end the session on the BACKEND, then forget it here.
//
// Deleting credentials.json is not a sign-out, it is an amnesia. The refresh
// token in that file is a bearer credential with a life of its own, and any
// copy of the file taken before the delete — a Time Machine backup, a synced
// folder, a second process running as this user, anything that read the file
// while it existed — keeps minting fresh access tokens for as long as the
// session lives. 0600 keeps other UNIX users out; it does not make a copied
// token stop working. So the button that says "sign out" has to reach the
// backend, or it is promising something it never did.
//
// ORDER. The token is refreshed FIRST (getAccessToken), because an expired
// access token cannot authenticate its own logout: GoTrue would answer 401 and
// the refresh token would outlive the sign-out. Refreshing rotates the token
// WITHIN the same session, so revoking after the refresh still kills the token
// the copied file holds.
//
// SCOPE. `local` ends this session only. Global would also sign the account
// out of the user's other machines, which nobody asked for by clicking sign
// out here. A backend too old to understand the parameter ignores it and logs
// out globally — stricter than asked, never weaker, so it stays safe to send.
//
// THE LOCAL DELETE IS UNCONDITIONAL. Someone on a plane, or behind a backend
// outage, must still be able to sign out of their own laptop; gating the
// delete on a network call would take that away exactly when they want it
// most. So the file goes either way — and the caller is told, plainly, which
// of the two things actually happened.
//
// WHAT THIS RETURNS IS THE WHOLE POINT. { cleared, revoked, error }: `revoked`
// is true only when the backend CONFIRMED it, never inferred from the absence
// of news. A failed revocation reported as a clean sign-out would be this
// repo's signature defect — a flag recording an outcome the code never
// achieved — and here it would be a security claim rather than a stale badge.
// Callers must say so out loud; `error` is the reason, ready to show.
async function signOut(config) {
  const creds = loadCredentials();
  if (!creds) return { cleared: false, revoked: false, error: null, wasSignedIn: false };
  const be = backend(config);
  let revoked = false;
  let error = null;
  if (!be) {
    error = 'no MemBridge backend is configured on this machine, so the session could not be ended remotely';
  } else {
    try {
      const fresh = await getAccessToken(config);
      if (!fresh || !fresh.accessToken) throw new Error('no usable session token to revoke with');
      await authLogout(be, fresh.accessToken);
      revoked = true;
    } catch (err) {
  ...
  return { cleared, revoked, error, wasSignedIn: true };
}

// POST /auth/v1/logout. Unlike authRequest's endpoints this one answers 204
// with no body and authenticates with the user's own bearer token rather than
// the anon key alone, so it does not fit that helper.
async function authLogout(be, accessToken) {
  const res = await fetch(`${be.url}/auth/v1/logout?scope=local`, {
    method: 'POST',
    headers: {
  ...
  const msg = data.msg || data.error_description || data.message || `logout failed (${res.status})`;
...
async function rest(config, creds, method, pathname, body, headers) {
  ...
  if (!res.ok) {
    ...
    // session from a backend fault get the status on the error itself. `code`
    // carries the RPC's own error code (e.g. MB001 vs MB002), so callers can
    // tell one 400 from another without matching on message text — a message
    // can be reworded for humans without ever breaking that mapping.
...
// site. WIRED INTO syncTeams: buildCryptoContext (below) calls this on the
// live push/pull path, so everything here runs against the real keychain and
// real crypto in production, not just in tests.
// ---------------------------------------------------------------------------
...
//
// Regeneration is NOT unconditionally safe any more: team keys now seal to
// this identity (resolveTeamKey/resolveCurrentTeamKey below), so overwriting
// a live private key destroys, permanently, every team key sealed to it.
// The keychain read below is therefore tri-state, not a boolean — "the item
// is genuinely not there" is the ONLY case allowed to regenerate. "The read
// failed" (locked keychain, no default keychain, a DPAPI decrypt error) must
// return null and touch nothing, via keychain.loadChecked's `indeterminate`
// flag (lib/keychain.js). `keychain.available()` above does not cover this:
// it only proves the keychain mechanism runs, which stays true throughout a
// locked-keychain failure — that is exactly the incident this guards.
async function readKeySlot(keychain, account) {
  // `loadChecked` is the real backend's tri-state read. A fake dep that only
  // implements the older `load(account) -> string|null` cannot express
  // "unreadable" at all — an in-memory Map is either holding the key or not,
  // never ambiguous — so falling back to `indeterminate: false` reports that
  // fake honestly rather than guessing a failure mode it cannot produce.
  if (typeof keychain.loadChecked === 'function') return keychain.loadChecked(account);
  return { value: keychain.load(account), indeterminate: false };
}
...
async function ensureIdentity(creds, deps) {
  ...
  const priv = await readKeySlot(keychain, PRIVKEY_ACCOUNT);
  if (priv.indeterminate) {
    // Could not tell "no key" from "keychain unreadable" — generating here
    // would be the destructive path this function exists to close off.
    util.log(`team encrypt: keychain read for ${PRIVKEY_ACCOUNT} was indeterminate — ` +
      'not generating a replacement identity (existing key, if any, is untouched)');
    return null;
  }
  const pub = priv.value ? await readKeySlot(keychain, PUBKEY_ACCOUNT) : { value: null, indeterminate: false };
  if (pub.indeterminate) {
    util.log(`team encrypt: keychain read for ${PUBKEY_ACCOUNT} was indeterminate — ` +
      'not generating a replacement identity (existing key, if any, is untouched)');
    return null;
  }
  const privateKey = priv.value;
  ...
  // Both reads completed and reported no value — a provably absent key
  // (first run, or an interrupted one that stored only half the pair), never
  // merely an unreadable one — so replacement is safe.
...
// A soft-deleted account is not a member, and must never be a seal target.
//
// Supabase's dashboard "soft delete" sets auth.users.deleted_at and LEAVES the
// row, so it trips none of the seven foreign keys that make a hard delete
// refuse — it always succeeds. The team_members row survives it untouched, so
// every one of the three key-sealing paths below (join-seal, re-seal, rekey)
// went on handing the deleted account a sealed copy of each new epoch. That is
// not a stale roster entry; it is continuing to grant access to somebody you
// believe you removed, silently and forward, forever.
//
// APPLIED AT THE POINT OF USE, NOT INSIDE mkTeamKeyDeps.fetchMembers, which
// would be one line instead of three. A future dep implementation (a test
// double, another backend) that forgets the filter must not be able to
// reintroduce the hole; the three call sites are where the guarantee has to
// hold, so that is where it is written.
//
// `deleted_at` arrives only once supabase/migrations/053 is applied. Before
// that the field is absent, which MUST read as live — treating unknown as
// deleted would empty every recipient list and break encryption for the whole
// team the moment the client shipped ahead of the migration. Hence the
// positive test for a real value rather than a falsy check on its absence.
// Empty string is not a timestamp and is deliberately not a deletion.
function activeMembers(rows) {
  return (Array.isArray(rows) ? rows : []).filter(m => m && !m.deleted_at);
}
...
async function resolveCurrentTeamKey(identity, deps) {
  ...
  // activeMembers: a soft-deleted account drops out of memberIds here, which
  // feeds BOTH the seal gate below and the "membership shrank -> rotate" test.
  // Rotating on a deletion is correct and is the same treatment a removed
  // member gets: new content moves to an epoch they never receive.
  const members = activeMembers(await deps.fetchMembers());
  ...
  // Filtered to live members BEFORE the pin gate, not after. fetchMemberPubkeys
  // asks for every id team_members_list returned, so a soft-deleted account's
  // key is in this response; letting it reach pins.check would pin it on first
  // sight and raise a KEY CHANGE alert if it ever moved -- sending an owner to
  // verify somebody who no longer exists. An existing pin is left in the store
  // rather than deleted: it is inert once the account is out of `allowed`, and
  // dropping stored trust is not a display filter's decision.
  const fetched = ((await deps.fetchMemberPubkeys()) || [])
    .filter(r => r && memberIds.has(r.user_id))
...
// which prompt fields ride along — the caller passes the sharedFieldsFor()
// mode string (push) or a legacy boolean (reshareSession, kept as compat
// until every caller is on the mode string):
//
//   'off'       → ask and goal both null
//   'distilled' → ask null, goal scrubbed
//   'verbatim'  → both scrubbed
//   true        → treated as 'verbatim' (legacy boolean)
//   false       → treated as 'off'      (legacy boolean)
//
// Non-prompt fields (summary, headline, decisions, gotchas, files, changes)
// ship regardless of mode — that has always been the case, and it is what
// makes a distilled-mode row readable at all. Mirrors the shape the backend
// upserts on (project_id, author_id, ts, source).
...
// ---------------------------------------------------------------------------
// THE WIRE BUILDER: protected by default, exemptions written down.
//
// Every field that leaves this machine used to be protected because somebody
// remembered to wrap it in scrub(), one field at a time. Two content-bearing
// fields never got wrapped — `files`, where the `wire` translation is path
// rewriting for cross-machine identity and not redaction at all, and
// `changes[].file`, sitting directly beside a `changes[].note` that IS
// scrubbed. A third case, a non-array `files`, skipped even the translation.
// And `{ ...c }` inside the changes map shipped every OTHER key of a change
// object untouched, so the next field added there would have inherited the
// same silence.
//
// The individual consequence is bounded — the realistic carrier is an ordinary
// `secrets/<token>.pem`, a credential that landed in a FILENAME, which is
// common enough that anyone who has done it once has shipped it through here.
// But the defect is the architecture, not the two fields: a builder where
// protection is a habit will lose the habit, and the next field added might
// not be a path.
//
// ENCRYPTION IS NOT THE CONTAINMENT IT LOOKS LIKE, and this is the part that
// decides how much the above matters. `plaintextOff` is the shipped default
// and it nulls the plaintext `files` column, so the DATABASE reads clean —
// which is exactly what makes this look contained when you go looking for it
// there. But encryptRow seals the raw value INTO the ciphertext, and every
// teammate decrypts that. So an unprotected field is hidden from the operator
// and delivered intact to precisely the people redaction exists to protect
// against. It is the mirror image of the encrypt-after-redact ordering
// invariant sitting beside it: same instrument, same sealed payload, opposite
// answer.
//
// So the default is inverted. protectWireRow() walks the row it is handed and
// redacts EVERY string at every depth — through arrays, through nested
// objects, through spreads whose keys nobody enumerated. A field added below
// is protected because it exists, not because someone remembered it.
//
// Ordering is unchanged and must stay that way: this runs while the row is
// still plaintext, and encryptRow seals the result afterwards. Redact then
// encrypt. Reversed, the redaction would scrub ciphertext (finding nothing)
// and leave the secret intact inside what every teammate decrypts — invisible
// in the database and total on receipt.
// ---------------------------------------------------------------------------

// The escape hatch, and it is deliberately awkward to use. A value wrapped
// here ships WITHOUT redaction, and the wrapper will not accept the wrapping
// without a written reason — because an exemption that costs nothing to write
// is indistinguishable from forgetting, which is the failure mode this whole
// mechanism exists to end. The reason is not decoration: it is the thing a
// reviewer reads when asking why this one field is different.
const UNREDACTED = Symbol('membridge.wire.unredacted');
const MIN_EXEMPTION_REASON = 24;
function unredacted(value, reason) {
  if (typeof reason !== 'string' || reason.trim().length < MIN_EXEMPTION_REASON) {
    throw new Error(
      'unredacted() requires a written reason (a sentence, not a word) saying why this ' +
      'field may leave the machine without redaction — see protectWireRow in lib/teamsync.js');
  }
  return { [UNREDACTED]: true, value, reason };
}

// Redact every string reachable from `v`. Non-strings that cannot carry text
// (numbers, booleans, null) pass through as themselves — redacting them would
// only risk changing their type for no gain.
function protectWireValue(v, regexes) {
  if (v && typeof v === 'object' && v[UNREDACTED]) return v.value;
  if (typeof v === 'string') return digest.redactText(v, regexes);
  if (Array.isArray(v)) return v.map(x => protectWireValue(x, regexes));
  if (v && typeof v === 'object') {
    const out = {};
    for (const k of Object.keys(v)) out[k] = protectWireValue(v[k], regexes);
    return out;
  }
  return v;
}

// Key order and the key SET are preserved exactly: a batch whose rows disagree
// on their keys is rejected wholesale by PostgREST ("All object keys must
// match"), which is why the builders below return explicit nulls rather than
// letting a field go undefined.
function protectWireRow(row, regexes) {
  const out = {};
  for (const k of Object.keys(row)) out[k] = protectWireValue(row[k], regexes);
  return out;
}
...
function entryToRow(e, projectId, creds, share, regexes, projectPath) {
  ...
  const wire = p => {
    ...
    // Untranslatable paths keep their original text (see above) — which for a
    // path outside any checkout means an absolute one, home directory and
    // username included. Scrub those on the way out, exactly like the text
    // fields: an unmatched path is already only a label to the receiver, so
    // collapsing the home prefix costs nothing and stops the username from
    // riding along on every such row. The scrub is NOT conditional on
    // projectPath: a caller that passes none (the older signature) was
    // shipping raw absolute paths, which is the same leak by a shorter route.
    const key = projectPath ? repoRootLib.wireKeyFor(path.resolve(projectPath, p)) : null;
    return key || redactLib.scrubHomePaths(p);
  };
  ...
  //
  // scrubHomePaths runs HERE and not inside digest.redactText, which is shared
  // with the local injected block where `/Users/<me>/…` is the entire point of
  // the feature (an agent handed `~/x` cannot open the file). Crossing to a
  // teammate — and to the server, since files/changes sit outside the
  // ciphertext on a hatch team — is the moment the same string stops being a
  // useful path and becomes "this is who I am and how my machine is laid out".
  // README promises private paths are scrubbed; this is the only boundary
  // where that promise can be kept without breaking the local block.
  const outbound = text => redactLib.scrubHomePaths(digest.redactText(text, regexes));
  const scrub = (text, n) => (text ? digest.clip(outbound(text), n) : null);
  // Bullet fields cross the wire with their line breaks intact, or a
  // teammate reads as a paragraph what was written as a list.
  // outbound (not bare redactText) so bullet fields get the same home-path
  // scrub as every other text field crossing the team boundary — a decision
  // or gotcha quoting `/Users/<me>/…` leaked the username before this.
  const scrubBullets = (text, n) => (text ? digest.bulletClip(outbound(text), n) : null);
  // THE ONLY EXEMPTIONS, and they share one reason: these five are the row's
  // identity on the backend. (project_id, author_id, ts, source) is
  // upsertEntries' on_conflict key and `session` threads a teammate's entries
  // into one session; all five are machine-generated (uuids, an ISO instant, a
  // tool name, a session id) and none is a channel a person can type into. A
  // redaction pass that altered one byte of any of them would not leak
  // anything — it would break identity: the upsert would insert a duplicate
  // instead of updating in place, or the receiver would split one session in
  // two. That is a real risk rather than a theoretical one, because the
  // entropy backstop matches on shape, and a uuid is exactly the shape it
  // looks for; it spares them today, and "today" is not an invariant.
  //
  // author_name is NOT here on purpose, though it is adjacent to identity: it
  // is a display string the user configures, so it is text a person types, and
  // it goes through the default like every other piece of text.
  const identity = 'row identity: the backend on_conflict key and session thread, machine-generated, never user-typed';
  return protectWireRow({
    project_id: unredacted(projectId, identity),
    author_id: unredacted(creds.userId, identity),
    author_name: creds.displayName,
    ts: unredacted(e.ts, identity),
    source: unredacted(e.source, identity),
    session: unredacted(e.session || null, identity),
    ...
    // Mode routing: 'off' both null, 'distilled' goal only, 'verbatim' both.
    // Legacy boolean tolerated for reshareSession's older signature.
    ask: (share === 'verbatim' || share === true) ? scrub(e.ask, 400) : null,
    goal: (share === 'verbatim' || share === 'distilled' || share === true) ? scrub(e.goal, 200) : null,
    ...
    // `wire` is path TRANSLATION — checkout-relative keys so a teammate at a
    // different monorepo depth agrees on file identity. It was never redaction,
    // and standing where redaction should have been is what made its absence
    // read as a decision. Both now apply: translated here, redacted by
    // protectWireRow on the way out. The non-array branch is translated too;
    // it used to skip even that and ship whatever it held.
    files: Array.isArray(e.files) ? e.files.map(wire)
      : (typeof e.files === 'string' ? wire(e.files) : e.files),
    // `...c` is why this needs the walker rather than one more scrub() call:
    // it spreads every OTHER key a change object carries, and enumerating them
    // here would be the same habit failing one field later.
...
// Change this person's display name, and their avatar (glyph + colour), across
// every team they belong to. The RPC does the work in one statement so the
// teams are atomic.
//
...
async function setDisplayName(config, name, avatar, avatarColor) {
  ...
  // Reject a malformed response instead of interpreting it. `rows` is null on
  // a 204 and on a 200 whose body fails to parse (rest()'s `.catch(() => null)`),
  // and either of those — or a row missing `teams` — would otherwise fall
  // through a truthiness check on `row.teams` into the teamless-success
  // branch below, laundering "the server said nothing intelligible" into "the
  // server wrote nothing", and stamping the REQUESTED name onto
  // credentials.json even though nothing is known to have been written. That
  // is exactly the fail-open-plus-unconditional-success-flag shape this
  // function exists to avoid.
  if (!Array.isArray(rows) || !rows.length || !Number.isInteger(rows[0].teams)) {
    throw new Error('set_display_name returned no usable result — nothing was saved locally');
  }
  const row = rows[0];
...
// Exact windowed BREAKDOWNS for Insights, aggregated in the database
// (055_team_insights_rollup.sql). The sibling of teamFeedCounts above, and it
// exists for the half of the same bug 027 did not reach: 027 moved the two
// headline COUNTS into the database, while the six breakdowns underneath them
// kept being folded on this machine from rows pulled by paging team_feed. That
// paging caps at 2,000 rows and drops the OLDEST ones, so per-person totals ran
// short and two sentences that NAME A PERSON — "only X has worked on this
// project" and "nothing has arrived from X" — could both be false purely
// because the second contributor's rows fell off the tail.
//
// Returns the rollup object, or **null** when the backend predates the
// migration — same degrade-don't-throw stance as teamFeedCounts, and the caller
// then pages the old way and reports the result as truncated. A null is "count
// the old way and say so", never an empty rollup: fabricating zeroed breakdowns
// here would publish "nobody shared anything" as though it had been measured.
async function teamInsightsRollup(config, teamId, opts = {}) {
  const creds = await hubCreds(config);
  try {
    const out = await rpc(config, creds, 'team_insights_rollup', {
      p_team: teamId,
      p_since: opts.since || null,
      p_until: opts.until || null,
    });
    // The function returns a scalar jsonb, which PostgREST hands back as the
    // object itself; tolerate a single-row array in case a future signature
    // change makes it a set-returning function.
    const row = Array.isArray(out) ? out[0] : out;
    if (!row || typeof row !== 'object') return null;
    // Shape gate. A response missing the four arrays is not a rollup, and
    // accepting it would let the caller mistake "the backend answered something
    // else" for "the team has no activity" — this codebase's signature defect,
    // a success flag set on a path that achieved nothing.
    for (const key of ['by_author', 'by_project', 'by_source', 'last_shared']) {
      if (!Array.isArray(row[key])) return null;
    }
    return row;
  } catch (err) {
    // Exactly the three signatures teamFeedCounts matches, and deliberately not
    // one character wider. PostgREST reports an unknown function as PGRST202 /
    // "Could not find the function". Anything else (auth, network, a real SQL
    // fault) must still throw: a broken backend that can masquerade as an old
    // one is a silent fallback nobody would ever see, and this repo has shipped
    // that shape before.
    if (/PGRST202|Could not find the function|does not exist/i.test(err.message || '')) return null;
    throw err;
  }
...
//
// Returns null, never an inferred answer, when it cannot be established:
...
//
// THE EMPTY CASE, which used to be null and is now measured.
//
// This function used to return null for an empty result, on the grounds that
// [] was ambiguous. Half of that ambiguity never existed: rest() throws on
// every non-2xx, so a backend without the view (PostgREST 404 / PGRST205), a
// dead session and a network fault all land in the catch below and were never
// candidates for []. A 200 [] is already an authoritative "the query ran and
// matched nothing". The only open question was WHY it matched nothing.
//
// So the cost of the old answer was not caution, it was a hole: `visible` was
// null for a member revoked from their ONLY shared project, the caller skipped
// the gate entirely, and that member's exposure was total and permanent. The
// old comment recorded this and accepted it.
//
// It is settled with a measurement rather than a policy. project_stats is
// filtered by BOTH `archived_at is null` and can_see_project; the base
// `projects` table is filtered by is_team_member and NOTHING ELSE
// (supabase/schema.sql projects_select — 025 deliberately left it alone, see
// its note on leaking project existence, and see the pin in
// test/suites/revocation-empty-visibility.test.js). The two queries therefore
// differ by exactly the predicate under test, and the empty case decomposes
// with no guessing left:
//
//   base throws              -> inconclusive -> null
//   base has a LIVE project  -> we are a member of a team that has a project
//                               we cannot see -> REVOKED -> empty set
//   base empty               -> not a member, or the team has no projects
//                               -> inconclusive -> null
//   base all archived        -> archived is not revoked -> inconclusive -> null
//
// Cost: one extra primary-key-ish GET, only on the empty path — which a
// healthy install with any visible project never takes.
//
// WHAT THIS DELIBERATELY DOES NOT DEFEND AGAINST, because the next reader will
// wonder. A backend that wrongly reports can_see = false for everyone — a
// materialization bug in 029/032 is the realistic shape — is indistinguishable
// from a real revocation here, and no client can be made to detect it: the
// backend is not failing to answer, it is answering "revoked", and second-
// guessing that would mean re-deriving the backend's own access state on the
// client, which is the thing we are asking it about. What makes this
// acceptable rather than merely unavoidable is that the damage is recoverable:
// everything the revoked branch drops is a CACHE of rows the backend still
// holds, and backfill refills the archive from the backend once access is
// confirmed again (see the restore branch in syncTeams).
//
// Pre-existing and NOT introduced here: project_stats also filters archived
// projects, so an archived project is absent from `visible` and reads as
// revoked. That conflation predates this change and is unchanged by it — the
// branches above only ever make the empty case MORE conservative (a team whose
// projects are all archived stays inconclusive).
async function visibleProjectIds(config, creds, teamId) {
  if (!teamId) return null;
  let rows;
  try {
    rows = await rest(config, creds, 'GET',
  ...
  // Empty. Corroborate before letting the caller act on it.
  return (await teamHasLiveProject(config, creds, teamId)) ? ids : null;
}

// Does this team demonstrably have at least one LIVE (non-archived) project
// that this caller is a member of? Read through the base `projects` table,
// whose select policy is membership-scoped and NOT access-scoped, which is the
// entire reason it can corroborate project_stats.
//
// Answers only "yes, provably" or "I cannot say", and every uncertainty —
// throw, non-array, empty, all-archived — is "I cannot say", because the only
// caller uses a `true` here to authorize destroying local data.
async function teamHasLiveProject(config, creds, teamId) {
  let rows;
  try {
    rows = await rest(config, creds, 'GET',
      `projects?team_id=eq.${encodeURIComponent(teamId)}&select=id,archived_at`);
  } catch {
    return false;
  }
  if (!Array.isArray(rows)) return false;
  return rows.some(r => r && r.id && !r.archived_at);
}

// Removing someone also drops any key alert we were holding ABOUT them.
//
// A key alert says "verify this teammate's key out of band before we seal to
// them again". Once they are off the team there is nobody to verify and
// nothing to seal, so leaving the alert standing turns a real security signal
// into a warning about an account that is simply gone — and statusPayload's
// count is machine-local, so nothing else would clear it promptly. The sync
// pass does rewrite state.keyAlerts wholesale from the roster the backend
// returns, so this was self-healing eventually; "eventually" is the wrong
// guarantee for a security banner whose only job is to be believed.
//
// AFTER the RPC, never before: a removal that the backend refused must not
// clear an alert about a member who is still on the team. Mirrors the prune
// trustMember already does when a key is re-pinned, including its shape — drop
// the key entirely once the list is empty, so `keyAlerts` is absent rather
// than an empty array nothing distinguishes from one.
// Removal is ALSO a CRYPTOGRAPHIC act, not just a row in a policy table.
//
// Before this, removal called remove_member and stopped. The removed device
// kept the current epoch's team key, sealed to its own public key, forever —
// so everything preventing them from reading tomorrow's content was
// server-side RLS, the exact trust boundary E2E exists to remove. Rotation was
// already implemented (`membridge team rekey`, which seals a fresh epoch to
// CURRENT members only) and was simply never called at the one moment it is
// needed.
//
// Best-effort and always REPORTED, never silent:
//   * the removal itself has already landed by then, so a rekey failure must
//     not undo it or throw — the caller gets `{ removed: true, rekey }` and
//     can surface "removed, but the key was not rotated: <reason>";
//   * `rekey.withheld` names members whose published key changed and is not
//     yet trusted (`membridge team trust`). They keep everything they have and
//     miss the NEW epoch until trusted — the pre-existing TOFU rule, surfaced
//     rather than silently applied;
//   * under the explicit team.encrypt=false hatch there is no key to rotate,
//     and rekey comes back null rather than a fake success.
//
// Forward-only, deliberately: old epochs are not rewritten, so history the
// removed member already pulled stays readable to them. No rotation can undo a
// copy someone already holds, and claiming otherwise would be a promise the
// crypto cannot keep.
//
// THIS DOES NOT COVER PROJECT-LEVEL REVOCATION. One team key spans every
// project in the team, so a member revoked from one project while staying on
// the team still legitimately needs that key for the projects they kept —
// there is no rotation that expresses "you may read Q but no longer P". That
// needs project-scoped keys and a schema change (team_keys SELECT is gated on
// is_team_member and never can_see_project; reconcileTeamKeys' seal targets
// come from team membership with no project filter). Flagged, not guessed at.
async function removeMember(config, teamId, userId, opts = {}) {
  const creds = await hubCreds(config);
  await rpc(config, creds, 'remove_member', { p_team: teamId, p_user: userId });
  // Before the encrypt=false early return, or the hatch path would leave a key
  // alert standing about somebody who is no longer on the team.
  forgetKeyAlert(userId);
  if ((((config || {}).team || {}).encrypt === false)) return { removed: true, rekey: null };
  let rekey = null;
  try {
    rekey = await rekeyTeam(config, teamId, { ...opts, creds });
  } catch (err) {
    rekey = { ok: false, error: err.message };
  }
  if (!rekey || !rekey.ok) {
    util.log(`team: removed the member, but the team key was NOT rotated (${(rekey && rekey.error) || 'unknown'}) — they keep a key that opens new content until an owner/admin runs \`membridge team rekey\``);
  } else if (rekey.withheld && rekey.withheld.length) {
    util.log(`team: rotated the team key to epoch ${rekey.epoch}; withheld from ${rekey.withheld.join(', ')} until their changed key is verified (\`membridge team trust\`)`);
  }
  return { removed: true, rekey };
}

// Drop one member's key alert from state, if there is one. Narrow load->save,
// and it writes NOTHING when there is nothing to drop: state.json has no
// locking, so every avoidable write is an avoidable window in which another
// process's concurrent save is lost.
function forgetKeyAlert(userId) {
  if (!userId) return;
  let state;
  try { state = util.loadState(); } catch { return; }
  if (!Array.isArray(state.keyAlerts)) return;
  const next = state.keyAlerts.filter(a => a && a.user_id !== userId);
  if (next.length === state.keyAlerts.length) return; // nothing about this member
  if (next.length) state.keyAlerts = next;
  else delete state.keyAlerts;
  try { util.saveState(state); } catch { /* an unreadable/unwritable state must not fail the removal */ }
}
...
// Hands the owner role to an existing member and demotes the caller to admin,
// in one transaction inside the RPC (058 §1). Deliberately NOT expressed as
// setRole(..., 'owner'): that is one row update, and a transfer is two, so the
// single-row form would leave the team with two owners. set_role still refuses
// 'owner' and that refusal is correct.
async function transferOwnership(config, teamId, userId) {
  const creds = await hubCreds(config);
  await rpc(config, creds, 'transfer_ownership', { p_team: teamId, p_user: userId });
}

// Disband. The RPC refuses unless the caller owns the team AND is its only
// remaining member (058 §2), so this cannot reach another person's memory --
// but for the caller it is total and irreversible: the delete cascades
// teams -> projects -> memory_entries. Every other team operation in this file
// revokes ACCESS; this one destroys content.
async function deleteTeam(config, teamId) {
  const creds = await hubCreds(config);
  await rpc(config, creds, 'delete_team', { p_team: teamId });
}
...
function pruneTeamEntries(projectPath) {
  ...
  const hadCursor = proj.teamPullTs != null || proj.teamPullId != null;
  if (!hadRows && !hadCursor) return false; // nothing to prune -> no save, no window
  proj.teamEntries = [];
  proj.teamPullTs = null;
  // The forward cursor is a PAIR (see fetchPullPage), so both halves reset
  // together. Today an orphaned id half happens to be harmless — the keyset's
  // first leg (`created_at.gt.1970-01-01`) matches everything regardless — but
  // "harmless because of how the other leg happens to be written" is not an
  // invariant, and a half-reset cursor is exactly the kind of state that turns
  // into a silent skip the next time the query shape changes.
  proj.teamPullId = null;
  try {
    util.saveState(state);
  } catch {
    return false;
...
//
// NOT the same shape as removeMember / archiveProject above, despite sitting
// beside them: those are manager actions whose RPCs enforce an owner/admin
// gate. These two are ungated by role on purpose. Every member can erase what
// they themselves pushed, including a member who was just demoted, and
// including one who has already left the team. 035 §1 scopes the DELETE
// policy on author_id alone for exactly that reason. Do not add a role check
// to these, or to the routes that call them.
// ---------------------------------------------------------------------------

// Compare two timestamps as INSTANTS, not as strings.
//
// True when `ts` is strictly after `watermark`; an empty watermark means there
// is none, so everything is beyond it.
//
// Every value fed to this today is produced on THIS machine (a JS
// toISOString() Z-form), so a lexicographic compare currently agrees with it.
// That is a fact about who writes the values, not about the data, and it is
// one assignment away from silently inverting: a backend-rendered timestamptz
// ('2026-06-01T10:00:00+00:00') sorts BELOW the Z-form of the SAME instant,
// because '+' is below both 'Z' and every digit in ASCII. A watermark in that
// spelling would suppress nothing and every deleted entry would resurrect on
// the next drift pass. The feed had the identical defect for real, on the ts
// PostgREST actually returns; see lib/feed.js instantKey. Falls back to the
// string compare when either side is not a timestamp, so junk still orders
// deterministically instead of comparing equal.
function afterWatermark(ts, watermark) {
  if (!watermark) return true;
  const a = Date.parse(ts);
  const b = Date.parse(watermark);
  if (Number.isFinite(a) && Number.isFinite(b)) return a > b;
  return String(ts) > String(watermark);
}

// The latest of several timestamps, by instant, ignoring empty ones.
function latestInstant(...values) {
  const present = values.filter(Boolean);
  if (!present.length) return '';
  return present.reduce((best, v) => (afterWatermark(v, best) ? v : best));
}

// Preview: per-project counts and the first/last entry timestamp, for the
// confirmation screen. Rows come back in the RPC's own snake_case
// (project_id, project_name, entries, first_ts, last_ts); callers map.
async function myEntryCounts(config, teamId) {
  const creds = await hubCreds(config);
  const rows = await rpc(config, creds, 'my_entry_counts', { p_team: teamId });
  return Array.isArray(rows) ? rows : [];
}

// Record on this machine that everything up to `at` has been erased from the
// backend for a team (or for one project in it), so the push side never puts
// it back. Three writes per affected project, all load-bearing:
//
// AFFECTED, and only affected. `deletedByProject` is the per-project row count
// delete_my_entries returned (035 §3), and a project not in it had nothing
// removed, so it gets nothing written. Watermarking every linked project of
// the team is silent and unrecoverable in three ways:
//
//   * a project linked but never synced (no teamPushTs) would get
//     deletedThrough = now and could NEVER push its local history: every entry
//     it holds is older than the watermark, forever.
//   * pushProject's encryption hold deliberately leaves a batch BEHIND the
//     cursor to retry next pass; a deletion elsewhere would bury it
//     permanently.
//   * a session shared after a deletion backfills entries below the
//     watermark, which are then suppressed for good.
//
// None of the three surfaces as an error. The project simply stops syncing.
//
//   teamDeletedThroughTs  the watermark pushProject filters on, in both its
//                         loops. The one that actually prevents resurrection.
//   teamPushTs            the push cursor, moved to the same instant so the
//                         fresh-entry loop has nothing to send either.
//   teamPushSig           cleared: every signature in it describes a backend
//                         row that no longer exists, and a stale map is worse
//                         than none (it would suppress a genuinely needed
//                         re-push after the user starts working again).
//
// Exported for the tests; production callers get it via deleteMyEntries.
//
// `deletedByProject` is a Map of backend project id -> rows removed, or null
// when the backend could not say (an older delete_my_entries returning a bare
// total, which the deploy gate in 035's header makes a reachable state). Null
// falls back to the narrowest rule that is still safe without the breakdown:
// watermark only projects this machine has actually PUSHED. A project with no
// teamPushTs has never sent a row, so the deletion cannot have removed
// anything of its, and giving it a watermark would only strand its history.
function markTeamDataDeleted(teamId, projectId, at, deletedByProject) {
  const state = util.loadState();
  const touched = [];
  for (const [key, proj] of Object.entries(state.projects || {})) {
    const link = loadTeamLink(key);
    if (!link || !link.projectId || link.teamId !== teamId) continue;
    if (projectId && String(link.projectId) !== String(projectId)) continue;
    if (deletedByProject) {
      if (!(Number(deletedByProject.get(String(link.projectId))) > 0)) continue;
    } else if (!proj.teamPushTs) {
      continue;
    }
    // The backend delete is unbounded in time (it removed every row this
    // author had, whatever its ts), so the watermark has to cover everything
    // already pushed, not just everything older than the wall clock. On a
    // machine with a skewed clock (or a session file stamped ahead) the push
    // cursor can already sit in the future, and a watermark of plain `now`
    // would leave those entries above it for the drift pass to resurrect.
    // Never moves backwards, either: a second, narrower deletion must not
    // reopen the door a wider earlier one closed.
    const mark = latestInstant(at, proj.teamPushTs, proj.teamDeletedThroughTs);
    proj.teamDeletedThroughTs = mark;
    proj.teamPushTs = mark;
...
// The deletion itself. The watermark is applied HERE rather than in the HTTP
// route so that every caller gets it: the daemon route, any CLI verb, and any
// future one. A caller that deletes without watermarking re-uploads the lot on
// the next sync tick, which is the single worst failure this feature has, and
// it must not be possible to reintroduce it by forgetting a line.
//
// The RPC runs first and the watermark only on success: watermarking a delete
// that did not happen would silently stop legitimate drift re-pushes for rows
// still sitting on the backend.
async function deleteMyEntries(config, teamId, projectId) {
  const creds = await hubCreds(config);
  const result = await rpc(config, creds, 'delete_my_entries', {
    p_team: teamId,
    p_project: projectId || null,
  });
  // One row per project the deletion actually emptied (035 §3). A bare number
  // is the pre-035 shape and means the backend cannot say which projects were
  // hit: markTeamDataDeleted falls back to its narrower rule rather than
  // guessing, and `deleted` still comes off the same answer either way.
  const rows = Array.isArray(result) ? result : null;
  const deletedByProject = rows
    ? new Map(rows.map(r => [String(r.project_id), Number(r.deleted) || 0]))
    : null;
  const deleted = rows
    ? rows.reduce((n, r) => n + (Number(r.deleted) || 0), 0)
    : Number(result) || 0;
  const at = new Date().toISOString();
  const projects = markTeamDataDeleted(teamId, projectId, at, deletedByProject);
  return { deleted, at, projects };
}

// ---------------------------------------------------------------------------
// Push / pull
// ---------------------------------------------------------------------------
// Normalize `config.team.sharePrompts` into the three-value mode string every
// downstream reader consumes. The full table is the contract:
//
//   input value                    | returned string
//   --------------------------------|----------------
//   'off'                           | 'off'
//   'distilled'                     | 'distilled'
//   'verbatim'                      | 'verbatim'
//   true                            | 'verbatim'   (legacy — pre-migration users keep their choice)
//   false                           | 'off'        (legacy)
//   absent / null / undefined       | 'off'        (fail-closed default)
//   any other value ('yes', 1, …)   | 'off'        (never ship prompts on a typo)
//
// The 'off' default when the value is absent is the alpha-readiness locked
// decision "Nobody's existing setting changes" pulled through: an existing
// user's config only has a sharePrompts key if they installed under an
// on-by-default release or explicitly ran `team share-prompts`; absent means
// they installed under an off-by-default release, and off is what they had.
// The three-value UI/CLI surface (dashboard setting, `membridge team
// share-prompts`) is the audit trail for legacy boolean values — a user with
// `sharePrompts: true` on disk now SEES it as "Verbatim (share raw prompts)"
// and can change it.
function readShareMode(config) {
  const v = ((config && config.team) || {}).sharePrompts;
  if (v === 'off' || v === 'distilled' || v === 'verbatim') return v;
  if (v === true) return 'verbatim';
  return 'off';
}

// The three-value counterpart to the old isShared: returns which fields ride
// along for this session, resolving both per-session opt-in and the global
// config setting. `'off'` blocks both ask and goal; `'distilled'` sends goal
// only; `'verbatim'` sends both. entryToRow reads this at the wire boundary.
//
// Per-session opt-in UPGRADES to verbatim (that is the point of the
// per-session UI — "yes, ship this session's raw prompts"), overriding the
// config default of 'distilled' or even 'off'.
//
// Legacy honor window unchanged in shape: a project that has NEVER been
// touched by the per-session UI (no sharedSessions array at all) falls back
// to the global config setting — through readShareMode, so a legacy boolean
// value is translated the same everywhere.
function sharedFieldsFor(config, proj, sessionId) {
  if (!sessionId) return 'off';
  const list = proj && proj.sharedSessions;
  if (Array.isArray(list)) return list.includes(sessionId) ? 'verbatim' : 'off';
  return readShareMode(config);
}

// Back-compat boolean wrapper: the local feed's `shared` flag (server.js:924,
// 1240) and the /api/team/share-session response (server.js:3091) both want
// "is this session opted in for verbatim?". "opted in for anything at all"
// is a fine translation — distilled mode still leaks the goal, and a
// share badge that lit only for 'verbatim' would misrepresent what has
// actually left the machine.
function isShared(config, proj, sessionId) {
  return sharedFieldsFor(config, proj, sessionId) !== 'off';
}
...
async function pushProject(config, creds, projectPath, proj, link, crypto) {
  ...
  // Deletion watermark (see markTeamDataDeleted): the newest instant the user
  // had erased from the backend, for THIS project. A server-side delete
  // removes rows from memory_entries and touches nothing on this disk: local
  // memory.json still holds every one of them, so without this every entry at
  // or below the watermark is a candidate for re-upload, and the drift pass
  // below is what would actually do it. `''` when nothing was ever deleted,
  // which afterWatermark reads as "no watermark" without a second branch.
  const deletedThrough = proj.teamDeletedThroughTs || '';
  ...
  // without a final pass, even if a future caller hands in raw text. The
  // projectPath activates the env-value deny-list (Task 5B): any literal
  // that appears in this project's own .env* files is redacted here.
  const regexes = digest.compileRedactions(config, { projectPath });
  // Verbatim prompts are the most sensitive field in an entry: they leave the
  // machine only when the user opts in per session (sharedFieldsFor returns
  // 'verbatim') or when config sits at 'verbatim' for a project untouched by
  // the per-session UI. Distilled mode ships the agent-written goal without
  // the raw ask; off mode ships neither. Summary and files upload regardless.
  ...
  const rowFor = e => entryToRow(e, link.projectId, creds, sharedFieldsFor(config, proj, e.session), regexes, projectPath);
  ...
  let pushed = 0;
  // The watermark is ANDed in rather than relied on through the cursor alone.
  // markTeamDataDeleted does set teamPushTs to the same instant, so today
  // these two clauses agree, but teamPushTs is a moving cursor that any later
  // push rewrites, and teamDeletedThroughTs is a fact that never moves
  // backwards. Deriving the deletion guarantee from the cursor would make it
  // one careless assignment away from silently vanishing.
  const fresh = all.filter(e => e.ts > cursor && afterWatermark(e.ts, deletedThrough));
  ...
  for (const e of all) {
    ...
    // THE RESURRECTION GUARD. This is the loop that would undo a deletion.
    // "Entries with no recorded signature re-send once" (above) is precisely
    // the state a deletion leaves behind (markTeamDataDeleted clears
    // teamPushSig), so every deleted entry would look like a stale backend
    // copy in need of healing, and merge-duplicates would insert it right back
    // with a bumped created_at, resurfacing it in every teammate's feed.
    // Deleted is not stale. Nothing at or below the watermark is ever
    // re-pushed by this pass or any later one.
    if (!afterWatermark(e.ts, deletedThrough)) continue;
...
// ---------------------------------------------------------------------------
// The E2E observation ledger — local, append-only evidence that a team is
// actually encrypting, kept in state.teamE2E[teamId].
//
// WHY IT EXISTS. The read path used to treat "this row carries ciphertext" as
// the only question worth asking, so a row served WITHOUT ciphertext skipped
// the crypto branch entirely and its server-controlled plaintext columns
// rendered as authentic teammate content — no `undecryptable` flag, the E2E
// badge still green. The in-code comment claimed protection against a server
// CORRUPTING ciphertext; omitting it was the unhandled case, and it is
// reachable with no attacker at all (one teammate with `team.encrypt:false`
// pushes plaintext rows into a team everyone else believes is E2E).
//
// WHAT MAKES A DOWNGRADE DISTINGUISHABLE FROM A LEGITIMATE PLAINTEXT ROW.
// Config cannot answer this: encryption is default-ON, so "my config says
// encrypt" is true even for a team that has never sealed a byte, and treating
// that as proof would black out every pre-E2E install. The server cannot
// answer it either — it is the thing being distrusted. The only trustworthy
// evidence is what THIS device has already seen and written down:
//
//   active       — set the first time a row carrying ciphertext arrives from
//                  this team. Never cleared. A hostile server can set it (that
//                  only makes this client stricter) but can never unset it, so
//                  a team that has once been observed encrypting stays that way
//                  across restarts.
//   minSealedId  — the LOWEST memory_entries.id whose ciphertext this device
//                  actually DECRYPTED. Lowering it requires valid ciphertext,
//                  i.e. the team key, so the server cannot widen the exemption
//                  below on its own.
//
// The two legitimate transition cases both fall out of this without a special
// case: (1) a team that genuinely never enabled encryption never sets `active`,
// so nothing about its pulls changes; (2) rows written BEFORE the team's
// cutover carry ids below `minSealedId` and stay readable, because id ordering
// and the forward pull's created_at ordering agree — a fresh device walking
// history forward meets the pre-cutover rows while `active` is still false, and
// the backward backfill walker meets them below `minSealedId`.
//
// RESIDUAL, DELIBERATELY NOT CLOSED HERE: pre-cutover history is not
// authenticable by any local evidence — nothing signed it — so a hostile server
// can still place forged content in the id range below `minSealedId`. Closing
// that means deciding to blank pre-E2E history, which is a product decision,
// not a bug fix.
function e2eLedgerFor(state, teamId) {
  const rec = (((state || {}).teamE2E || {})[teamId]) || {};
  return {
    teamId,
    active: !!rec.active,
    minSealedId: Number.isFinite(rec.minSealedId) ? rec.minSealedId : null,
    sealedSeenAt: rec.sealedSeenAt || null,
    lastDowngradeAt: rec.lastDowngradeAt || null,
    // Per-pass counters (never persisted as totals — the badge asks "did this
    // pass observe a downgrade", not "how many ever").
    downgrades: 0,
    sealedRows: 0,
  };
}

function saveE2ELedger(state, e2e) {
  if (!state || !e2e || !e2e.teamId) return;
  if (!state.teamE2E || typeof state.teamE2E !== 'object') state.teamE2E = {};
  state.teamE2E[e2e.teamId] = {
    active: !!e2e.active,
    minSealedId: e2e.minSealedId,
    sealedSeenAt: e2e.sealedSeenAt,
    lastDowngradeAt: e2e.downgrades ? new Date().toISOString() : e2e.lastDowngradeAt,
  };
}

// Every content field an opaque row carries. One definition, so the pull path
// and the feed path cannot drift into disagreeing about what "opaque" means.
function opaqueContent() {
  return { ask: null, goal: null, decisions: null, gotchas: null, summary: null, headline: null, files: [], changes: null };
}

// Is a row that arrived WITHOUT usable ciphertext a downgrade for this team?
// See the ledger header for why these are the only two inputs.
function isPlaintextDowngrade(r, e2e) {
  if (!e2e || !e2e.active) return false;               // no evidence this team encrypts
  const id = Number(r && r.id);
  if (Number.isFinite(id) && Number.isFinite(e2e.minSealedId) && id < e2e.minSealedId) {
    return false;                                       // pre-cutover history
  }
  return true;
}
...
// crypto reason.
//
// OMITTING the ciphertext is the same attack as corrupting it, and used to be
// the unhandled half: a row with `ciphertext: null` skipped this branch
// entirely and its plaintext columns rendered as authentic content. It now
// takes the same opaque path whenever the ledger says this team encrypts —
// see e2eLedgerFor for how a downgrade is told apart from a team that has
// simply never turned encryption on.
async function mapPulledRow(r, link, teamCrypto) {
  let content = r;
  let undecryptable = false;
  const e2e = teamCrypto && teamCrypto.e2e;
  const warnCtx = teamCrypto
    ? (teamCrypto.ctx || (teamCrypto._warn = teamCrypto._warn || { warned: new Set() }))
    : null;
  if (teamCrypto && r.ciphertext && r.nonce) {
    // Ratchet: this team is now KNOWN to encrypt, whether or not this device
    // can open the row. Recorded before the decrypt attempt on purpose — a
    // row we cannot read is still proof that ciphertext is in use.
    if (e2e && !e2e.active) { e2e.active = true; e2e.sealedSeenAt = new Date().toISOString(); }
    if (e2e) e2e.sealedRows++;
    let payload = null;
    ...
    if (payload) {
      ...
      // Only a row we actually opened may lower the pre-cutover boundary:
      // lowering it widens the plaintext exemption, so it must cost the
      // attacker the team key rather than a made-up id.
      const id = Number(r.id);
      if (e2e && Number.isFinite(id) && (!Number.isFinite(e2e.minSealedId) || id < e2e.minSealedId)) {
        e2e.minSealedId = id;
      }
  ...
  } else if (teamCrypto && teamCrypto.required && isPlaintextDowngrade(r, e2e)) {
    // No ciphertext on a row from a team this device has SEEN encrypt. The
    // plaintext columns are whatever the server chose to put there, so they
    // are not content — same treatment as ciphertext that will not open.
    undecryptable = true;
    content = opaqueContent();
    if (e2e) e2e.downgrades++;
    warnOnce(warnCtx, `pull-plain:${link.teamId || ''}`,
      'team encrypt: a pulled row arrived with NO ciphertext from a team that is encrypting — rendering it opaque (fail-closed). Either a teammate is syncing with team.encrypt=false, or the server stripped it.');
  }
  return {
...
// ONE forward page, ordered the way the rows ARRIVED at the backend.
//
// The order is `created_at` (server-stamped) and never `ts` (stamped by the
// pushing machine): a row authored on Monday and pushed on Friday carries
// Monday's ts, so a ts cursor that has already reached Friday would step over
// it and never come back — invisible, not merely late. That much has been true
// since team sync shipped. What was missing is the BOUNDARY.
//
// created_at is not unique. `created_at` defaults to now(), which is constant
// within a transaction, so one PUSH_BATCH upsert lands 50 rows sharing an
// identical created_at; the drift re-push is worse still — it stamps ONE
// client-side `now` across every stale row it sends (see the `stale` loop
// above), so a tie group is bounded by the entry window, not by the batch.
// A lone `created_at=gt.<cursor>` page therefore has no way to stop in the
// middle of a tie group: PULL_LIMIT cuts the page, the cursor advances to the
// tie's created_at, and the siblings that fell past the cut are skipped
// permanently. backfillArchivePage documents this same hazard below and
// answered it by paging on `id`; the forward pull never got the same fix.
//
// The answer here is a KEYSET on the pair (created_at, id) rather than a move
// to `id` alone. `id` alone would have meant abandoning the timestamp cursor
// every install already has on disk, and (created_at, id) is exactly the sort
// key the page is ordered by, so it can address a position INSIDE a tie group:
//
//   or=(created_at.gt.C, and(created_at.eq.C, id.gt.I))
//
// Nothing is skipped (a tie sibling is only passed once its own id is passed)
// and nothing is re-fetched (`gt.` on both legs is strict), including when a
// tie group is larger than a whole page — the second leg keeps walking through
// it, id by id, page after page. Ties are broken by `id` because it is the
// only column with no ties of its own: a bigint identity PK.
//
// The comparison lives in the WHERE clause, never in JS. `created_at.eq.C`
// sends back a value the backend rendered (`.55+00:00` for a `.550Z` push) and
// Postgres compares it as an instant; the same test in JS on the raw strings
// would never match a single row. That is the whole reason there is no local
// "is this row at the boundary?" check anywhere on this path.
//
// LEGACY / FIRST PAGE: with no id half of the cursor (every install upgrading
// to this build, and every fresh link) the FILTER is the plain `gt.` page
// exactly as it shipped, and the id half is learned from that page. So the
// first pull after an upgrade fetches precisely the rows it would have fetched
// before — it cannot skip anything today's build would have caught — and every
// page after it is exact. The only difference on that page is the added `id`
// sort key, which does not change WHICH rows come back, only that a tie group
// comes back in a defined order instead of the backend's choice.
//
// A 400 means the backend refused the filter shape (an older PostgREST, say).
// That degrades to the legacy page rather than failing the project's whole
// pull — but LOUDLY, because a silent fallback would leave the tie bug in
// place while reporting a healthy sync. Any other error (401, a network
// fault) propagates untouched: a second doomed request helps nobody.
async function fetchPullPage(config, creds, proj, link, cursor) {
  const base = `project_id=eq.${link.projectId}` +
    `&author_id=neq.${creds.userId}`;
  const page = `&order=created_at.asc,id.asc&limit=${PULL_LIMIT}`;
  const legacy = `${base}&created_at=gt.${encodeURIComponent(cursor)}${page}`;
  const cursorId = proj.teamPullId;
  if (cursorId == null) return fetchMemoryRows(config, creds, legacy);
  const c = encodeURIComponent(cursor);
  const keyset = `${base}` +
    `&or=(created_at.gt.${c},and(created_at.eq.${c},id.gt.${encodeURIComponent(cursorId)}))` +
    `${page}`;
  try {
    return await fetchMemoryRows(config, creds, keyset);
  } catch (err) {
    if (err && err.status === 400) {
      util.log(`team pull: backend rejected the keyset page (${err.message}) — falling back to the timestamp cursor, which can skip rows sharing a created_at`);
      return fetchMemoryRows(config, creds, legacy);
    }
    throw err;
  }
}
...
async function pullProject(config, creds, proj, link, teamCrypto) {
  ...
  const rows = await fetchPullPage(config, creds, proj, link, cursor);
  ...
  // Both halves of the cursor, from the LAST row of a page ordered
  // (created_at asc, id asc) — the greatest position this page consumed.
  // teamPullTs stays the timestamp it always was (lib/server.js reports it as
  // "last pulled", and `membridge team repull` prints it), and teamPullId is
  // the tiebreaker that lets the next page resume INSIDE a tie group.
  //
  // A row with no id leaves teamPullId null rather than storing `undefined`:
  // the next page then takes the legacy branch instead of sending
  // `id.gt.undefined`, which would 400 every pull from here on.
  const last = rows[rows.length - 1];
  proj.teamPullTs = last.created_at;
  proj.teamPullId = last.id != null ? last.id : null;
  return rows.length;
}
...
// plaintext columns. ctx may be null (identity unavailable): every
// ciphertext row is then opaque. Callers under the explicit encrypt:false
// hatch skip this function entirely.
//
// A row with NO ciphertext gets the same gate as the pull path (see
// e2eLedgerFor): opaque when this device has already observed this team
// encrypting, untouched when it has not (pre-cutover history, or a team that
// never turned encryption on). This is READ-ONLY on the ledger — the feed
// renders on every poll and state.json has no locking, so only the sync pass
// writes it.
async function decryptTeamRows(config, creds, teamId, rows, ctx) {
  ...
  let e2e = null;
  try { e2e = e2eLedgerFor(util.loadState(), teamId); } catch { /* no ledger, no gate */ }
  const out = [];
  for (const r of rows || []) {
    if (!r || !r.ciphertext || !r.nonce) {
      if (r && isPlaintextDowngrade(r, e2e)) {
        warnOnce(warnCtx, `feed-plain:${teamId || ''}`,
          'team encrypt: a feed row arrived with NO ciphertext from a team that is encrypting — rendering it opaque (fail-closed)');
        out.push({ ...r, ...opaqueContent(), undecryptable: true });
      } else {
        out.push(r);
      }
      continue;
...
async function trustMember(config, needle, opts = {}) {
  ...
  const seen = new Map();
  for (const t of teams) {
    // activeMembers: `team trust` re-pins a key so it can be sealed to. A
    // deleted account must not be trustable -- blessing its key would put it
    // straight back into the three seal paths this filter exists to keep it
    // out of. Falls through to the existing "no teammate matching" error.
    for (const m of activeMembers(await rpc(config, creds, 'team_members_list', { p_team: t.team_id }))) {
...
async function rekeyTeam(config, teamId, opts = {}) {
  ...
  // Same filter as the other two seal sites: a rekey exists to move the team
  // to a key some party must not hold, so sealing the fresh epoch to a deleted
  // account would defeat the entire operation.
  const live = activeMembers(members);
  const memberIds = new Set(live.map(m => m.user_id));
  const nameById = new Map(live.map(m => [m.user_id, m.display_name || '']));
  ...
  // `live`, not `members`: a deleted account is not a teammate whose key was
  // withheld pending verification -- it is not a teammate. Reporting it would
  // tell an owner to go and trust somebody who no longer exists.
  const withheld = live
...
async function syncTeams(opts = {}) {
  ...
  // Team id -> E2E observation ledger, seeded from disk and shared by every
  // project linked to that team in this pass (the evidence is a property of
  // the TEAM's key, not of one project). Written back below.
  const e2eByTeam = new Map();
  for (const [key, proj] of Object.entries(state.projects || {})) {
    ...
    try {
      ...
      if (visible && !visible.has(String(link.projectId))) {
        ...
        if (!proj.teamAccessLost) {
          ...
          // The confirmed-revoked branch below empties teamEntries and erases
          // the derived caches, but a project only reaches an injection pass
          // when it is DIRTY (lib/scan.js filters on it), and none of that
          // clearing used to set the flag. So the block already written into
          // CLAUDE.md / AGENTS.md kept rendering the revoked teammate's rows
          // forever, for exactly the project this member stopped opening (any
          // local event would have marked it dirty on its own). One mark here
          // is enough: it triggers exactly one re-render on the next scan
          // pass, which is all a now-empty block needs. lib/scan.js writes
          // .membridge/memory.md / memory.json inside that SAME dirty-gated
          // loop, so this also puts them back in the work list on the next
          // pass — though they never carried teammate content to begin with
          // (memorydb.buildEntries reads only proj.events), so nothing in
          // them actually changes. Deliberately on the TRANSITION only (this
          // guard), not every pass this branch runs — this loop revisits
          // every linked project every sync tick regardless of local
          // activity, so marking it unconditionally here would re-render an
          // abandoned-and-revoked project forever. See
          // test/suites/revocation-marks-dirty.test.js.
          proj.dirty = true;
          util.log(`team access lost for ${key} — dropping cached teammate rows, the derived notes index and the durable archive`);
        }
        proj.teamEntries = [];
        proj.teamPullTs = null;
        proj.teamPullId = null; // both halves of the forward cursor, as one unit
      ...
      if (link.teamId && !e2eByTeam.has(link.teamId)) e2eByTeam.set(link.teamId, e2eLedgerFor(state, link.teamId));
      const teamCryptoArg = encryptOn
        ? { ctx: cryptoCtx, keyDeps, required: true, e2e: link.teamId ? e2eByTeam.get(link.teamId) : null }
        : null;
  ...
  // The E2E ledger is what makes the badge report what was VERIFIED rather
  // than what was configured. Persisted for every team touched this pass;
  // `teamPlaintextRows` is the pass-level alert the status endpoint reads, and
  // is recomputed every pass so a clean pass clears it.
  {
    let downgraded = 0;
    for (const e2e of e2eByTeam.values()) {
      saveE2ELedger(state, e2e);
      downgraded += e2e.downgrades;
    }
    if (downgraded) {
      state.teamPlaintextRows = `${downgraded} team row(s) arrived with no ciphertext from a team that is encrypting — shown as unreadable (fail-closed). A teammate may be syncing with team.encrypt=false.`;
    } else if (e2eByTeam.size) {
      delete state.teamPlaintextRows;
    }
  }
...
module.exports = {
  ...
  signup, login, oauthAuthorizeUrl, loginWithTokens, clearCredentials, signOut, loadCredentials, getAccessToken,
  ...
  ensureIdentity, resolveTeamKey, resolveCurrentTeamKey, encryptRow, entryToRow, isShared, sharedFieldsFor, readShareMode, plaintextOffFor,
  buildCryptoContext, classifyAuthFailure, protectWireRow, unredacted, decryptTeamRows, fingerprintReport, trustMember, rekeyTeam, reconcileTeamKeys,
  activeMembers,
  createTeam, joinTeam, listTeams, setDisplayName, linkProject, unlinkProject, pruneTeamEntries, loadTeamLink, repoUrl,
  parseInviteToken, inviteUrl, createInvite, revokeInvite, join,
  listMembers, teamFeed, teamFeedCounts, teamInsightsRollup, projectStats, visibleProjectIds, isSoloMachine,
  removeMember, setRole, renameTeam, rotateInvite, leaveTeam, transferOwnership, deleteTeam,
  archiveProject, unarchiveProject,
  myEntryCounts, deleteMyEntries, markTeamDataDeleted,
  detectAutoLinks, resolveSuggestion,
  syncTeams, reshareSession, pullProject, pushProject, backfillArchivePage, credentialsPath, teamFilePath,
};

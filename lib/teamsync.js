...
function entryToRow(e, projectId, creds, share, regexes, projectPath) {
  ...
  return {
    ...
    decisions: e.decisions ? scrub(e.decisions, WIRE_NOTE_MAX) : null,
    gotchas: e.gotchas ? scrub(e.gotchas, WIRE_NOTE_MAX) : null,
...
function dedupeOnConflictKey(rows) {
  const byKey = new Map();
  for (const r of rows) byKey.set(`${r.project_id} ${r.author_id} ${r.ts} ${r.source}`, r);
...
module.exports = {
  ...
  listMembers, teamFeed, projectStats, visibleProjectIds, isSoloMachine,

...
function protectWireValue(v, regexes) {
  ...
  // Two filters, and they cover different things. digest.redactText handles
  // CREDENTIALS; scrubHomePaths handles IDENTITY — `/Users/<me>/…` names the
  // human and their machine layout, and redactText has no home-path pattern on
  // any code path, so a path shipped straight through here today.
  //
  // It runs at THIS boundary and not inside digest.redactText because that
  // function is shared with the local injected block, where the real absolute
  // path is the entire product (an agent handed `~/x` cannot open the file).
  // Crossing to a teammate — and to the server, since files/changes sit outside
  // the ciphertext on a hatch team — is the moment the same string stops being
  // a useful path and becomes a disclosure. README promises private paths are
  // scrubbed; this is the only boundary where that promise can be kept without
  // breaking the local block.
  //
  // Folded into the DEFAULT rather than applied per-call-site so home-path
  // scrubbing inherits the same "protected because it exists" property as
  // credential redaction: the next field added is covered by both, not one.
  if (typeof v === 'string') return redactLib.scrubHomePaths(digest.redactText(v, regexes));
...
function entryToRow(e, projectId, creds, share, regexes, projectPath) {
  ...
  // Untranslatable paths keep their original text — which for a path outside
  // any checkout means an absolute one, home directory and username included.
  // Those are scrubbed by protectWireValue on the way out (see its header), so
  // this only has to answer the cross-machine-identity question.
  const wire = p => {
...
// Removing a member is a CRYPTOGRAPHIC act, not just a row in a policy table.
//
...
async function removeMember(config, teamId, userId, opts = {}) {
  ...
  // Removal also drops any key alert we were holding ABOUT them. A key alert
  // says "verify this teammate's key out of band before we seal to them
  // again"; once they are off the team there is nobody to verify and nothing
  // to seal, so leaving it standing turns a real security signal into a
  // warning about an account that is simply gone.
  //
  // AFTER the RPC, never before: a removal the backend refused must not clear
  // an alert about a member who is still on the team. And BEFORE the
  // encrypt=false early return below, because a team running the plaintext
  // hatch still has key alerts to drop — gating this on rotation would leak
  // the stale banner on exactly the teams that never rotate.
...
// Drop one member's key alert from state, if there is one. Called by
// removeMember above (see the rationale there: statusPayload's count is
// machine-local, and though the sync pass rewrites state.keyAlerts wholesale
// from the backend roster, "eventually" is the wrong guarantee for a security
// banner whose only job is to be believed). Mirrors the prune trustMember
// already does when a key is re-pinned, including its shape — drop the key
// entirely once the list is empty, so `keyAlerts` is absent rather than an
// empty array nothing distinguishes from one. Narrow load->save,

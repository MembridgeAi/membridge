  ...
  for (let i = 0; i < entries.length; i += PUSH_BATCH) {
    ...
    for (;;) {
      try {
        ...
        const m = /'(summary|goal|decisions|gotchas|changes)' column/i.exec(err.message);
        if (!m) throw err;
        const drop = m[1];
...
const OPTIONAL_PULL_COLUMNS = ['goal', 'decisions', 'gotchas', 'changes'];
...
async function pullProject(config, creds, proj, link) {
  ...
  let selectCols = ['author_name', 'ts', 'source', 'session', 'ask',
    'goal', 'decisions', 'gotchas', 'summary', 'files', 'changes', 'created_at'];
  let rows;
  for (;;) {

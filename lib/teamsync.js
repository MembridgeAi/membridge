...
// ---------------------------------------------------------------------------
// Self-serve deletion of the caller's OWN synced entries (migration 028).
//
...
// including one who has already left the team. 028 §1 scopes the DELETE
...
// delete_my_entries returned (028 §3), and a project not in it had nothing
// removed, so it gets nothing written. Watermarking every linked project of
// the team -- which this did while the RPC returned a bare total -- is silent
// and unrecoverable in three ways:
//
//   * a project linked but never synced (no teamPushTs) got deletedThrough =
//     now and could NEVER push its local history: every entry it holds is
//     older than the watermark, forever.
//   * pushProject's encryption hold deliberately leaves a batch BEHIND the
//     cursor to retry next pass; a deletion elsewhere buried it permanently.
...
// total, which the deploy gate in 028's header makes a reachable state). Null
...
function markTeamDataDeleted(teamId, projectId, at, deletedByProject) {
  ...
  for (const [key, proj] of Object.entries(state.projects || {})) {
    ...
    const mark = [at, proj.teamPushTs || '', proj.teamDeletedThroughTs || ''].sort().pop();
...
// route so that every caller gets it: the daemon route, the CLI (which calls
// teamsync directly, like every other `membridge team` subcommand), and any
...
async function deleteMyEntries(config, teamId, projectId) {
  ...
  // One row per project the deletion actually emptied (028 §3). A bare number
  // is the pre-028 shape and means the backend cannot say which projects were
...
async function pushProject(config, creds, projectPath, proj, link, crypto) {
  ...
  // Deletion watermark (see markTeamDataDeleted): the newest ts the user had
  // erased from the backend, for THIS project. A server-side delete removes
  // rows from memory_entries and touches nothing on this disk: local
  // memory.json still holds every one of them, so without this every entry
  // at or below the watermark is a candidate for re-upload, and the drift
  // pass below is what would actually do it. `''` when nothing was ever
  // deleted, which every `e.ts > ''` / `e.ts <= ''` comparison below reads as
  // "no watermark" without a second branch.
  ...
  // these two clauses agree, but teamPushTs is a moving cursor that any
  // later push rewrites, and teamDeletedThroughTs is a fact that never moves
  ...
  const fresh = all.filter(e => e.ts > cursor && e.ts > deletedThrough);
  ...
  for (const e of all) {
    ...
    // copy in need of healing, and merge-duplicates would insert it right
    // back with a bumped created_at, resurfacing it in every teammate's feed.
    // Deleted is not stale. Nothing at or below the watermark is ever
    // re-pushed by this pass or any later one.
    if (e.ts <= deletedThrough) continue;
...
async function fetchMemoryRows(config, creds, where) {
  ...
  let selectCols = ['id', 'author_name', 'ts', 'source', 'session', 'ask',
...
async function pullProject(config, creds, proj, link, teamCrypto) {
  ...
  for (const r of rows) {
    ...
    const k = `${mapped.author}|${mapped.ts}|${mapped.source}`;
...
// A membership/RLS refusal almost always means the project carries a team.json
// (often committed by a teammate) for a team this account isn't in — say so,
// instead of leaving a bare backend error. Shared by the per-project catch and
// the push-only catch inside it.
    ...
    try {
      ...
      if (visible && !visible.has(String(link.projectId))) {
        ...
        if (!proj.teamAccessLost) {
          ...
          util.log(`team access lost for ${key} — dropping cached teammate rows and the durable archive`);
        }
        proj.teamEntries = [];

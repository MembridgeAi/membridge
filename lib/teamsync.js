...
// stops for good and records the honest `stopped: 'archive-full'` — never a
// `done: true` claim of having reached the true start of history. The
// pre-fetch check below reads only the tiny sidecar (teamArchive.
// backfillStatus), so a project that is already done/stopped costs no
// network round trip on the sync passes after it stops.
async function backfillArchivePage(config, creds, proj, link, teamCrypto) {
  ...
  if (status.backfill.done || status.backfill.stopped) return 0;
...
module.exports = {
  ...
  listMembers, teamFeed, projectStats, isSoloMachine,

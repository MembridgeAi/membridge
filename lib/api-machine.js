...
// The stdio target every spawned daemon inherits: the shared logfile, or
// 'ignore' when it cannot be opened. Extracted so the two spawn paths below
// cannot drift on where a restarted daemon's output goes.
function daemonLogStdio() {
  try {
    return fs.openSync(util.logPath(), 'a');
  } catch {
    return 'ignore';
  }
}

// Spawn the replacement daemon and resolve ONLY once the OS confirms it
// started. Rejects with a MachineError otherwise.
//
// This exists because a process cannot report that it successfully restarted —
// if it had, it would be gone — and the endpoint used that (correct) fact to
// justify pre-confirming, which made it report success for a restart that
// never happened at all. The achievable claim is narrower and this is it: the
// replacement was *started*. Confirming completion is the client's job, by
// watching for a daemon to answer again.
//
// Node's 'spawn' event is the confirmation and 'error' is the failure, and BOTH
// are asynchronous. That matters: a missing or non-executable node binary
// (ENOENT/EACCES) surfaces as an async 'error' event, NOT a synchronous throw,
// so wrapping spawn() in try/catch alone would still report success for the
// most likely real-world failure. Only argument-level mistakes throw inline,
// and those are caught here too.
//
// `error` stays attached with `on` rather than `once` deliberately: after this
// settles, a later failure in the child must still find a listener, because an
// 'error' event with none is an uncaught exception — in the one process whose
// job right now is not to crash while replacing itself.
function spawnReplacement(deps = {}) {
  const spawnImpl = deps.spawnImpl || spawn;
  // Bounded so a child that emits neither event cannot hang the HTTP request
  // forever. A timeout is reported as failure, not success: see the caller in
  // lib/server.js for why the safe direction is to stay alive.
  const timeoutMs = deps.timeoutMs === undefined ? 5000 : deps.timeoutMs;
  return new Promise((resolve, reject) => {
    let child;
    try {
      child = spawnImpl(process.execPath, [BIN, 'daemon'], {
        detached: true,
        stdio: ['ignore', daemonLogStdio(), daemonLogStdio()],
        windowsHide: true,
        // Mark this start as a restart handoff. cmdDaemon's duplicate-daemon
        // guard (bin/membridge.js) refuses to boot when a live MemBridge
        // daemon already owns the pid file -- which, during a restart, is the
        // very daemon we are replacing (it stays alive until it has flushed the
        // HTTP response, then exits ~200ms later). Without this signal the
        // replacement mistakes its predecessor for a duplicate and refuses, so
        // the pid never changes and the restart silently fails. The guard
        // consumes the flag on read, so it authorizes exactly this one boot and
        // does NOT weaken the guard against a genuine second daemon.
        env: { ...process.env, MEMBRIDGE_TAKEOVER: '1' },
      });
    } catch (err) {
      reject(new MachineError(500, `could not start a replacement MemBridge: ${err.message}`));
      return;
    }
    if (!child || typeof child.on !== 'function') {
      // A test stub that is not an EventEmitter: nothing to wait for, and no
    ...
    // NOT unref'd, deliberately, unlike scheduleExit's timer below. An unref'd
    // timer does not hold the event loop open, so if nothing else is pending it
    // never fires at all and this promise hangs forever instead of timing out.
    // It is always cleared by settle(), so it cannot outlive the outcome.
    timer = setTimeout(() => settle(() => reject(new MachineError(
      504, 'a replacement MemBridge was launched but never confirmed it started'))), timeoutMs);
    child.on('error', err => settle(() => reject(new MachineError(
      500, `could not start a replacement MemBridge: ${err.message}`))));
...
// Schedule THIS process to exit, once the caller has finished with it. Split
// from the spawn above so a failed spawn can decline to call this and leave the
// user with the daemon they still have.
function scheduleExit(deps = {}) {
  const exit = deps.exit || (code => process.exit(code));
  const delayMs = deps.delayMs === undefined ? 200 : deps.delayMs;
  const timer = setTimeout(() => exit(0), delayMs);
  if (timer && typeof timer.unref === 'function') timer.unref();
...
module.exports = { MachineError, resolveTrackedPath, openInFileManager, openTarget, restartDaemon, spawnReplacement, scheduleExit, existingRealpath, defaultOpenDeps };

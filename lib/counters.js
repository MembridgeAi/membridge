...
// The six read-only MCP tools (lib/mcp.js registerTools) — presence only,
// never counts or arguments. Lives here (not lib/mcp-usage.js) so mcp-usage
// can require it from counters without counters ever requiring mcp-usage
// back: one direction only, no cycle.
const MCP_TOOLS = ['get_project_memory', 'get_recent_activity', 'list_projects', 'recall', 'search_memory', 'why'];
const COUNTER_NAMES = ['heartbeat', 'recall_state', 'hook_registration', 'environment', 'mcp_tool_used'];
...
function buildCounters({ recallState, shape, registration, mcpToolsUsed }) {
  ...
  // Iterate MCP_TOOLS itself (not the caller-supplied list) so the emitted
  // count is structurally bounded by the allowlist's own length, however
  // many times a tool name shows up in mcpToolsUsed: a Set collapses
  // duplicates, and looping the fixed allowlist -- rather than the variable
  // input -- makes "at most one entry per known tool" true by construction,
  // not by convention.
  const usedTools = new Set(mcpToolsUsed || []);
  for (const tool of MCP_TOOLS) {
    if (usedTools.has(tool)) out.push({ name: 'mcp_tool_used', dims: { tool } });
  }
  return out;
}
...
// Precedence is by PRESENCE of the config key, not by its truthiness. An
// explicitly empty countersUrl is an OPT-OUT, not "unset": self-hosted and
// open-source builds blank it to mean never send (see counters-backend.json),
// and that has to outrank the URL compiled into the release. `||` collapses
// "absent" and "explicitly emptied" into one falsy case, so a self-hosted build
// that set countersUrl: '' still sent. Only an absent key falls back to the
// baked endpoint -- that path is what our own builds ship for.
// Whitespace is trimmed on both sides of the decision: it reads as the same
// opt-out, and untrimmed it was truthy enough to clear emitCounters' guard and
// have us POST to the literal string "   ".
...
async function postCounters(url, body, { fetchImpl = fetch } = {}) {
  ...
  try {
    ...
    // fetch only rejects on transport failure — a 500, a 404, or a WAF block
    // all resolve normally. Returning true on those would report a send that
    // the other end refused, which is the one thing a fire-and-forget
    // diagnostic cannot afford to get wrong: it is the only success signal
    // anyone has. A response with no `ok` field (a hand-rolled test double)
    // is treated as success so existing fakes keep their meaning.
...
async function emitCounters(state, config, { fetchImpl = fetch, now = Date.now(), registration = null, mcpToolsUsed = [] } = {}) {
  try {

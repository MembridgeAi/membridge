...
function countersUrl(config) {
  // An explicitly empty countersUrl is an OPT-OUT, not "unset": self-hosted and
  // open-source builds blank it to mean never send (see counters-backend.json).
  // Only an absent key falls back to the baked endpoint.
  if (config && typeof config.countersUrl === 'string') return config.countersUrl;
  return DEFAULT_COUNTERS_URL;
}

...
// `search` runs the PreToolUse Grep/Glob search hook (lib/hooks-search.js);
...
// Required lazily: these subcommands are the only callers, and the Stop and
// recall paths must not pay for a module they never touch. `search` is the
// PreToolUse Grep/Glob hook (lib/hooks-search.js), which drags in the feed
// normalizers and the ranker, a require graph the far hotter recall path has
// no use for.
else if (argv[0] === 'search') require('./hooks-search').runSearch();

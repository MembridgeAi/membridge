...
function emptyArchive(projectId) {
  return { version: 1, projectId, backfill: { done: false, beforeId: null }, rows: [] };
}
...
function loadArchive(projectId) {
  try {
    ...
    if (!bf || typeof bf !== 'object' || !('beforeId' in bf)) {
      ...
      parsed.backfill = { done: !!(bf && bf.done), beforeId: null };
    }
    return parsed;
  } catch {
...
function saveArchive(arc) {
  try {
    ...
    // pid-suffixed: syncTeams runs from both the CLI and the daemon tick, so
    // two processes can race to write the same project's archive — a fixed
    // tmp name would let one process's rename land on the other's half-written
    // file mid-write.
...
function setBackfill(projectId, backfill) {
  ...
  arc.backfill = { done: !!safe.done, beforeId: safe.beforeId != null ? safe.beforeId : null };
  saveArchive(arc);
}

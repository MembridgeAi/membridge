...
// done".
//
// COMPACTION, and why it is safe: "fewer bytes, same content", NEVER "keep
// less content".
//
// The append fast path used to write every incoming line unconditionally,
// including keys the file already held. Harmless for an ordinary forward pull
// (its rows are new by construction) and permanently expensive for anything
// that re-walks history: a full re-pull roughly DOUBLED the .ndjson, forever,
// because nothing ever rewrote it except an eviction trim — and every later
// loadArchive pays that size again in read+parse, which is precisely the ~4s
// per project the read cache upstream exists to hide.
//
// So appendRows rewrites the file whenever an incoming row's key is already on
// disk, and appends only when every incoming key is new. What the rewrite drops
// is not a row: readRows has always been last-line-wins per rowKey, so a
// superseded line is one no reader has ever returned. The compacted file is
// byte-different and MEANING-identical, which is the property that makes
// compaction incapable of losing data.
//
// Keep those two concerns apart. Compaction must never drop content by age —
// the long tail is the entire reason this file exists (cross-teammate overlap
// has a ~50-day median) — and it must never take over the cap, which the
// asymmetric forward trim below already owns. The moment compaction is allowed
// to decide what content to keep, it can lose history; while it only decides
// how many bytes the same content occupies, it cannot.
//
// meta.rowCount stays honest for free: it has always been computed from the
// deduped set, so it never counted the duplicate lines and the 50,000-row cap
// was never fooled by them. Freeing room below the cap also lets a backfill
// walk that recorded `stopped: 'archive-full'` resume from its exact cursor —
// see backfillArchivePage, which was written expecting this.
//
...
// deleting it — writeAtomic's default path has no fsync, so a crash right
// after an unlink could leave a zero-length rows file next to a sidecar
// claiming the full row count, an empty archive that believes it is complete.
// The append now (a) prepends a newline before appending if the file's last
// byte isn't already one, so a crash mid-append can never glue the next
// append's first row onto a truncated final line and lose both, and (b) runs
// under a short-lived "<id>.lock" (fs.openSync 'wx'), because the CLI and the
// daemon tick both call syncTeams and can otherwise interleave two processes'
// writes mid-file; a lock older than a couple of minutes is treated as
// abandoned so a crashed holder can never wedge the archive.
//
// That lock now spans the READ as well (appendRows holds it once around
// read -> merge -> write). It used to cover the write alone, which left the
// rewrite path replacing the whole file from a pre-lock snapshot and silently
// discarding rows another process had appended in the meantime.
...
// Keyed on the author's DISPLAY NAME, deliberately, even though rows now also
// carry a stable `authorId` (lib/teamsync.js mapPulledRow) and the pull cache's
// own dedupe key prefers it. Do not "finish the job" by switching this to the
// id: this file is append-only NDJSON deduped at PARSE time (readRows), so a
// key change would give every line already on disk a different key from the
// same logical row appended after it — duplicating the entire existing archive
// on read, on the one store with no rewrite path to repair it.
//
// The name instability this leaves is already answered downstream, once, by
// session: lib/activity.js eventKey keys on the session id and
// collapseIdentityTwins collapses rows that are the same event under two
// spellings of one account (the 'andrewludwigbrown' / 'Andrew Brown' case is
// named in that comment). A second answer to a solved problem is how the two
// would drift.
//
// THE `ts` COMPONENT IS THE RAW BACKEND STRING, AND THAT IS CORRECT HERE.
// Do not "finish the job" by parsing it into an instant the way lib/feed.js
// instantKey does. Reviewed deliberately, because the same defect class was
// real and shipped in lib/feed.js and lib/server.js, and the reason those
// needed a parsed key does not hold on this path.
//
// The key IS string-fragile, so the question was worth asking. A local entry's
// ts is a JS toISOString() Z-form string (lib/scan.js) pushed verbatim into a
// timestamptz column, and PostgREST renders its reads through Postgres's own
// to_json, which does not hand back the string that went up. Verified against
// Postgres 17 (Supabase runs 17.6.1):
//
//   input:   '2026-08-05T12:00:00.550Z'::timestamptz
//   to_json: "2026-08-05T12:00:00.55+00:00"
//
// Z becomes +00:00, AND the trailing zero is trimmed. Hand appendRows one of
// each spelling of ONE instant and the archive holds that row twice. Measured,
// not assumed: it stores two distinct rows. Worse than the feed case, because
// this store is append-only NDJSON deduped at PARSE time and has no repair
// path: a doubled archive stays doubled.
//
// IT CANNOT HAPPEN, because both sides of every comparison come out of ONE
// renderer. These are the only reachable states of a `ts` this key ever sees:
//   * a row read back off the .ndjson (readRows), put there by appendRows;
//   * a row arriving at appendRows, only ever lib/teamsync.js mapPulledRow's
//     output, whose `ts` is fetchMemoryRows' PostgREST read of
//     memory_entries.ts, `timestamptz` since the original schema
//     (supabase/schema.sql) and never re-typed by any migration;
//   * a row recovered by migrateLegacy, written by an older appendRows out of
//     those same two pull paths.
// There is no fourth state. appendRows has exactly two production callers,
// pullProject and backfillArchivePage, and no local entry, reshare or import
// reaches either.
//
// NOTE WHAT IS NOT THE REASON. `author_id=neq.<self>` on both pull queries is
// what protects lib/feed.js's self-twin dedupe; here it is nearly beside the
// point. Executed both ways: a self row, had it been pulled, still arrives in
// the backend's spelling and folds correctly, while a local-spelled TEAMMATE
// row doubles the archive. The load-bearing invariant is single-renderer, not
// self-exclusion, so do not reason about this key by analogy with the feed's.
//
// THE WAY TO BREAK IT is therefore to give appendRows a third caller fed by
// anything that did not come back through PostgREST. That is the change this
// comment exists to stop, and test/suites/team-archive-instant.test.js pins it.
...
// Held for the duration of one appendRows() read+merge+write — see
// withAppendLock.
...
//
// `opts.fsync` flushes the tmp file's data blocks to disk BEFORE the rename.
// Without it, rename is atomic with respect to other readers but not with
// respect to power loss: the rename can be journaled before the data lands,
// leaving a short or zero-length file where a complete one is claimed. That is
// the exact failure class migrateLegacy renames-instead-of-deletes to avoid.
// Passed by the ROWS REWRITE only, deliberately — a rewrite is rare (an
// eviction trim, or a compacting append) and can afford the flush, while the
// append path runs on every pull and the sidecar write runs even more often.
// Do not "finish the job" by turning it on everywhere: this module's whole
// performance argument is that the common path never pays whole-file costs.
function writeAtomic(p, content, opts = {}) {
...
// THE TWO WRITE BODIES. Both assume the caller already holds the append lock
// (appendRows is the only caller and takes it once around read+merge+write), so
// neither takes it itself. They throw on failure; withAppendLock converts that
// into `false` for the caller, which is what stops a failed write being counted
// as a success.
function appendLinesLocked(projectId, rows) {
...
// Whole-file replacement, used for an eviction trim AND for compaction. fsync
// before rename (see writeAtomic): this is the one path that can lose already
// archived history to a badly-timed power cut, and it is rare enough to pay for
// the flush.
function rewriteRowsLocked(projectId, rows) {
  writeAtomic(rowsPath(projectId), serializeRows(rows), { fsync: true });
}
...
function appendRows(projectId, mappedRows, opts = {}) {
  ...
  // ONE lock acquisition around read -> merge -> write.
  //
  // The read used to sit OUTSIDE the lock, with only the write inside it, and
  // that was a live data-loss bug rather than untidiness: the rewrite path
  // replaces the whole file with a set computed from a snapshot taken before
  // the lock existed, so any rows another process appended in between were
  // silently discarded. Both processes are ordinary (`membridge team sync` and
  // the daemon tick both call syncTeams), the loser's append reported success,
  // and the rows were gone from the one store that holds history past the
  // 100-row cache. Reading inside the lock is what actually closes it; putting
  // the lock around the rename alone only narrows the window.
  let finalCount = null;
  const wrote = withAppendLock(projectId, () => {
    const existing = readRows(projectId);
    const byKey = new Map(existing.map(r => [rowKey(r), r]));
    // COMPACTION, decided here because this is the one place that knows both
    // what is on disk and what is arriving. See the module header for why the
    // file grows without it.
    let duplicates = 0;
    for (const r of valid) {
      if (byKey.has(rowKey(r))) duplicates += 1;
      byKey.set(rowKey(r), r);
    }
    ...
    // Rewrite when a line would otherwise be written for a key the file
    // already holds — that is exactly the duplicate, and `merged` is already
    // in hand, so compaction costs one write on the passes that need it and
    // nothing at all on the passes that do not. An eviction trim rewrites for
    // its own reason, as before.
    if (mustTrim || duplicates > 0) rewriteRowsLocked(projectId, finalRows);
    else appendLinesLocked(projectId, valid); // fast path: just the new lines
  ...
  if (!wrote || finalCount == null) return null;

  const meta = readMeta(projectId) || emptyMeta(projectId);
  saveMeta({ projectId, backfill: meta.backfill, rowCount: finalCount });
  return finalCount;
}

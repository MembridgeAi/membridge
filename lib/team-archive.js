...
function withAppendLock(projectId, fn) {
  ...
  try {
    ...
    if (!err || err.code !== 'EEXIST') return; // locking itself failed — skip this append
    try {
      ...
      return; // still held, or reclaiming raced with someone else — skip
    }
  }
  if (fd === undefined) return; // a live process holds it — skip this append
  try {
    fn();
  } catch {

...
//   now           optional; DEFAULT IS Date.now(), a real wall-clock read.
//                 That default is a deliberate, documented exception to this
//                 module's "no side effects" claim above: decide() is pure
//                 ONLY when the caller supplies `now` explicitly. Kept as a
//                 default (rather than made required) because every existing
//                 caller either doesn't care about the 14-day holdout
//                 boundary at all, or -- like this suite's holdout tests --
//                 sets projectCreatedAt comfortably clear of the boundary
//                 (1 day in / 30 days in against a 14-day window), so a few
//                 ms of real-clock drift between building `input` and
//                 calling decide() can never flip the outcome. Any FUTURE
//                 test that deliberately targets the exact boundary (e.g.
//                 projectCreatedAt set to now-14days±1ms) MUST pass `now`
//                 explicitly instead of relying on this default, or it will

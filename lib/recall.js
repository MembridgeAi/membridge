...
function tierFor(input) {
  ...
  const readHash = sessionState && sessionState.reads ? sessionState.reads[relPath] : null;
  if (readHash && fileStat && readHash === fileStat.hash
      && readByThisSession(ledger, relPath, sessionId)) return 'A';

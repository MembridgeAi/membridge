...
// MIN_COMPRESSION gates `callTokens / skeletonTokens` -- "is the skeleton
// enough cheaper than the file to be worth serving instead of it".
//
// RE-MEASURED AGAINST THE HONEST TOKENIZER, AND IT HELD (Tier 2 follow-up,
// ticket #55). This constant was chosen while BOTH sides of that ratio came out
// of chars/4, an estimator that understates real source by ~6% in aggregate AND
// whose error tracks file type -- so it was wrong by DIFFERENT amounts on the
// two sides, and claude/ops/decisions.md recorded it as needing recalibration
// the moment a real tokenizer landed. That was the right thing to expect. It is
// not what the measurement says.
//
// The predicted effect was that honest pricing makes skeletons dearer, pushes
// files under the floor, and cuts serves. Measured over 283 real repo files
// under both estimators on the SAME skeleton text, the opposite happens: the
// call side rises by a fixed ~10% (bytes/4 -> bytes/BYTES_PER_TOKEN) while a
// CODE skeleton's BPE-vs-chars/4 error is smaller than that, so code
// compression ratios RISE (x1.02) and the serve population goes 197 -> 204.
// Three files lose the floor, ten gain it. The drop everyone expected comes
// entirely from Markdown, whose heading-only skeletons do tokenize expensively
// (x0.82) -- but the lowest Markdown ratio in the repo is 8.07x, so not one
// Markdown file is anywhere near this threshold and the fall changes no
// decision. Averaging all file types into one number is what made an artefact
// look like a threshold problem.
//
// So the value stays 2.25, and it now means more than it used to: with both
// sides counted in the same vocabulary, 2.25 is an actual 2.25x in tokens
// rather than a number calibrated through a biased lens. Re-tuning it to
// reproduce the old population would be re-introducing that bias as a constant.
// test/suites/compression-floor.test.js re-derives all of the above on every
// run and FAILS if the serve population ever does shrink -- that assertion is
// the trigger to redo this work, not decoration.

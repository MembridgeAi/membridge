...
function statementText(e, max) {
  const headline = String(e.headline == null ? '' : e.headline).trim();
  if (headline) return clipWords(headline, max);
  ...
  return clipWords(first.length <= max ? first : summary, max);
}
...
function pickSessionStatement(entries, max) {
  ...
  for (const e of entries) {
    ...
    const text = statementText(e, max);
    if (!text) continue;
  ...
  return best ? { entry: best, text: bestText } : null;
}
...
function buildDayDigests(entries, opts = {}) {
  ...
  for (const bucket of buckets.values()) {
    ...
    for (const sessionRows of bySession.values()) {
      const picked = pickSessionStatement(sessionRows, max);
      if (picked) statements.push(picked);
    }
    ...
    const omittedSessions = distinct.length - kept.length;
    ...
    const notes = [];
    if (omittedSessions > 0) notes.push(`${omittedSessions} more session${omittedSessions === 1 ? '' : 's'} not shown`);

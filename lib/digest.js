...
//
// `opts.projectPath` (alpha readiness Task 5B) attaches the per-project
// env-value deny-list. It is opt-in per call site because only the two
// paths that actually know a project's root use it today:
//   - teamsync.pushProject / entryToRow (wire) — always passes it
//   - memorydb.buildEntries (local capture) — always passes it
// Every other caller (server render endpoints, hooks-search, provenance,
// recall-store, activity, retrievals) renders text that has ALREADY been
// through the pipeline once at capture time, so the env matcher already
// fired upstream. Adding it there is fine but not required by 5B, and this
// branch keeps the diff bounded to the two must-have callers.
//
// Same single pipeline (redactText below) — this is NOT a parallel redactor;
// it is an additional compiled layer inside the same compile/apply pair.
function compileRedactions(config, opts) {
  const user = [];
  ...
  if (opts && opts.projectPath) {
    // lazy require — env-deny reads from disk, and pulling it in
    // unconditionally would run readdir on every compileRedactions call.
    const envDeny = require('./env-deny');
    env = envDeny.redactorFor(opts.projectPath);
  }
  return { useDefaults: !config || config.redactDefaults !== false, user, env };
}

// THE single redaction pipeline. Defaults first (named [redacted:<name>]
// markers, incl. the entropy backstop), then the per-project env-value
// deny-list ([redacted:env], when compileRedactions was given a projectPath),
// then user patterns ([redacted]). Accepts a bare regex array too, so any
// older/direct caller still works with defaults on. Always redact BEFORE
// clipping — truncation must not sever a pattern's anchor.
function redactText(text, compiled) {
  let t = String(text);
  const c = Array.isArray(compiled) ? { useDefaults: true, user: compiled, env: null } : (compiled || { useDefaults: true, user: [], env: null });
  if (c.useDefaults) t = redact.redactDefault(t);
  if (c.env) t = c.env.replace(t);
...
// Where the FIRST managed block starts and ends, or null when the file carries
// no usable pair. `end` is one past the closing marker.
//
// The subtle part is which END closes the block, and it decides whether a
// user keeps or loses their own instructions:
//
//   - A FORGED end marker (smuggled into the block by a prompt-injected agent,
//     or left by a version before renderBlock neutralized them) has no BEGIN
//     after it. Splitting at it would strand the smuggled tail OUTSIDE the
//     block, where it survives every re-render and every uninstall — a
//     permanent foothold in the file every AI tool reads at startup. So it
//     must be re-absorbed, which means taking the LAST end marker.
//
//   - A DUPLICATED block — the shape a git merge of a checked-in CLAUDE.md
//     produces when both sides brought their own block — DOES have a BEGIN
//     after its end marker, and the user's own hand-written lines sit between
//     the two pairs. Taking the last end marker there swallows those lines.
//
// The BEGIN that follows is exactly what tells the two apart, so the block
// ends at the last END that precedes the next BEGIN, falling back to the last
// END in the file when this BEGIN never closes before the next one (a nested
// pair, which collapses whole as it always did).
function blockSpan(text) {
  const b = text.indexOf(BEGIN);
  if (b === -1) return null;
  const nextBegin = text.indexOf(BEGIN, b + BEGIN.length);
  let e = nextBegin === -1 ? -1 : text.lastIndexOf(END, nextBegin - END.length);
  if (e <= b) e = text.lastIndexOf(END);
  if (e === -1 || e <= b) return null;
  return { begin: b, end: e + END.length };
}

// Cut one block span out, collapsing the blank-line seam it leaves behind to a
// single `eol` — see removeBlock() for why the runs are matched as whole CRLF
// pairs rather than as lone LFs.
function spliceBlockOut(text, span, eol) {
  const blankRun = eol === '\r\n' ? /(?:\r\n)+$/ : /\n+$/;
  const leadingBlankRun = eol === '\r\n' ? /^(?:\r\n)+/ : /^\n+/;
  return text.slice(0, span.begin).replace(blankRun, eol)
    + text.slice(span.end).replace(leadingBlankRun, '');
}

// Remove every complete marker pair in `text`, keeping the user's own lines
// around and between them. One managed block per file is the contract; a merge
// that brought two in leaves a stale pair that has to go, but the instructions
// the user wrote between them must survive it.
function stripAllBlocks(text, eol) {
  let out = text;
  // Each pass removes at least one whole pair, so this always terminates; the
  // bound is belt-and-braces against a future blockSpan that stops shrinking.
  for (let guard = 0; guard < 1000; guard++) {
    const span = blockSpan(out);
    if (!span) break;
    out = spliceBlockOut(out, span, eol);
  }
  return out;
}
...
function inject(filePath, block, preamble = '') {
  ...
  let updated;
  const span = blockSpan(existing);
  if (span) {
    // Replace the first block in place, and clear any duplicate pairs left in
    // the tail so the file converges on exactly one managed block.
    updated = existing.slice(0, span.begin) + ownBlock
      + stripAllBlocks(existing.slice(span.end), eol);
...
function removeBlock(filePath, opts = {}) {
  ...
  if (!blockSpan(existing)) return null; // see blockSpan(): no usable pair, nothing to strip
  ...
  const eol = eolOf(existing);
  // EVERY pair, not just the first: a file that a merge left holding two
  // blocks must come out of an uninstall with neither of them — and with the
  // user's own lines between them still there. Cutting first-BEGIN to last-END
  // in one span instead would take those lines with it, and then delete the
  // whole file for looking empty.
  const rest = stripAllBlocks(existing, eol);
...
// The OTHER shape of harvested filler: agent narration -- the agent thinking
// aloud about what it is ABOUT to do rather than reporting what it did.
// Openers on the real feed: "Now let me look at the digest/memorydb
// pipeline...", "Let me check the redaction path first", "OK, let me now
// examine what happens when the sync loop restarts", "First, let me look at
// how pickSummary handles empty text". Length is not the signal -- the same
// shape lives at 30 characters ("Let me check X") and at 200 -- so
// SCAVENGED_STATEMENT_MIN never catches it. The signal IS the shape: an
// imperative "let me / let's / let us" opener, optionally prefaced by a
// discourse marker ("OK", "Now", "First", "Alright", "Then", "So", "Well").
// This is the same rule the goal path already enforces one field over: an
// intent is never promoted into an outcome.
//
// Applied uniformly to harvested AND distilled first-sentences, headlines
// included. A real outcome statement does not open this way ("Fixed X.",
// "Removed Y.", "Confirmed Z." all pass through untouched); an intent one
// does, whatever wrote it.
const INTENT_OPENER = /^\s*(?:(?:ok|okay|alright|now|first|next|then|so|right|hmm|well|actually|hold on)[,\s]+)*let(?:'s| us| me)\b/i;
...
function statementFor(e, max) {
  ...
  if (headline) {
    // An authored headline is normally a statement outright, but an
    // intent-shaped one ("Let me check X") is not, whatever the length --
    // reject it the same way the summary path does below.
    return { text: clipWords(headline, max), stands: !INTENT_OPENER.test(headline) };
  }
  ...
  // Two independent disqualifications on the summary path:
  //   1. Scavenged filler that is too short to be a report (see
  //      SCAVENGED_STATEMENT_MIN's comment for the corpus measurement).
  //   2. Agent narration -- an intent lead-in, at any length. See
  //      INTENT_OPENER's comment for why length is not the signal here.
  const intent = INTENT_OPENER.test(source);
  const long = !!e.distilled || source.length >= SCAVENGED_STATEMENT_MIN;
  return {

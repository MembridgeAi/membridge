...
function renderBlock(projectPath, proj, config, target, precomputedSessions, mcpRows) {
  ...
  const team = teamInjectSlice(teamRowsFor(proj), config);
  if (team.length) {
  ...
  if (mcpLiveFor(target, mcpRows)) {
    lines.push('This project\'s memory is also queryable over MCP (`search_memory`, `get_project_memory`, `why`) — search it before exploring a file fresh or re-deriving a decision already made.');
    lines.push('');
  }

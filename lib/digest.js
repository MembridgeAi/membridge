...
// Edits get their own budget for the same reason reads got theirs: one loud
// kind must not evict a quiet one out of a shared pool. Edits became the loud
// kind when subagent edits started being captured, roughly tripling their
// volume on a delegating session (42 to 158 measured on one real session).
// Sharing the narrative cap with prompts and summaries meant a single operator
// running heavy delegation halved the retained prompt and summary history for
// EVERYONE on that project, since the pool is per project and eviction is
...
function mergeEvents(state, events, config) {
  ...
  for (const key of touched) {
    ...
    for (const e of proj.events) {
      ...
      else if (!isPlumbing(e)) narrative.push(e);
...
module.exports = {
  ...
  compileRedactions, redactText, clip, bulletClip, plainText, plainLines, shortDate, relativeLabel, recentPrompts, recentFiles,

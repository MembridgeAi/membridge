...
// is made of (measured: ~90% narrative loss on a heavy repo). The two classes
...
function mergeEvents(state, events, config) {
  ...
  for (const key of touched) {
    ...
    // downstream still sees one chronological array.
    const narrative = [], plumbing = [];
    for (const e of proj.events) (isPlumbing(e) ? plumbing : narrative).push(e);
    if (narrative.length > cap || plumbing.length > plumbingCap) {
      proj.events = narrative.slice(-cap).concat(plumbing.slice(-plumbingCap)).sort(byTs);
    }
  }
  return touched;
}
...
function renderBlock(projectPath, proj, config, target, precomputedSessions) {
  ...
  if (target === 'AGENTS.md' && (!config || !config.distill || config.distill.enabled !== false) && config && config.distill && config.distill.consent === 'granted') {
    lines.push('As you complete work here, append a line to `.membridge/summaries.jsonl`: `{"session":"<your session id>","ts":"<ISO time>","goal":"<what you set out to do>","did":"<1-3 sentences on what you did>","decisions":"","gotchas":"","highlights":[]}` — plain text, only what a teammate needs; goal is one line, highlights is up to 2 key files with a short note each. On a long session, append a new line as the work grows, each one restating the WHOLE session so far so it supersedes the earlier lines; never edit earlier lines.');
    lines.push('');
  }

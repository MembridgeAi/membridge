...
// The markers, and the rule for where the block they delimit starts and ends,
// live in lib/block-span.js — one implementation for the writer here and for
// every reader (lib/server.js, lib/hooks-prime.js). Re-exported below so
// digest.BEGIN / digest.END keep working for existing callers.
//
...
const blockSpanLib = require('./block-span');
const { BEGIN, END } = blockSpanLib;
const blockSpan = blockSpanLib.firstBlockSpan;
...
// env-value deny-list. Pass it from every call site that knows a project's
// root:
//   - teamsync.pushProject / entryToRow (wire) — always passes it
//   - memorydb.buildEntries (local capture) — always passes it
//   - renderBlock (the injected CLAUDE.md/AGENTS.md block) — always passes it
//
// This used to claim the other callers were safe because their text "has
// ALREADY been through the pipeline once at capture time, so the env matcher
// already fired upstream". That was false: there is no capture-time
// redaction anywhere (capture stores ev.text verbatim, see proj.events.push
// below), so a caller that omits projectPath renders RAW event text with the
// env layer off. renderBlock was the expensive case — it writes the block
// users are documented to commit — and it had projectPath in hand as its
// first parameter and simply did not forward it. Measured before the fix:
// 8 of 11 ordinary .env values (incl. 32-hex API keys, which sit under the
// entropy floor) reached the injected block in clear while memory.json
// showed them all [redacted:env].
//
// Remaining callers that still omit it (server render endpoints,
// hooks-search, provenance, recall-store, activity) are loopback/local
// surfaces, not files users commit — lower exposure, but the same latent
// bug. Do not re-justify them with the upstream-capture claim.
//
...
function renderBlock(projectPath, proj, config, target, precomputedSessions, mcpRows) {
  const regexes = compileRedactions(config, { projectPath });

...
// of .membridge/recall/events.jsonl, already reduced to plain deltas) into
// the running accumulator. Kept as pure arithmetic here -- the same
// "table-testable with plain object literals" contract the rest of this
// module holds -- so all the fs/estimator work stays on the caller's side of
// the boundary (see ledger-fold-recall.js's own header). `settlement` is
// optional: a one-shot buildProjectLedger call with no recall queue to fold
// (most callers, including every existing test) passes nothing and this is
// a no-op, exactly like readKeyFor's projectPath-less mode above.
function foldRecallSettlement(st, settlement) {
  if (!settlement) return;
  const { avoidedDelta, holdoutDelta } = settlement;
  if (avoidedDelta) {

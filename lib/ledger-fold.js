...
function foldRequests(st, events, keyCap) {
  ...
  // vol.inCost/vol.outCost are deliberately NOT accumulated. See the
  // no-USD-on-disk note on foldProjectLedger's return below: a priced figure
  // is never persisted, so accumulating one here would only build a total
...
function foldRecallSettlement(st, settlement) {
  ...
  const { avoidedDelta, holdoutDelta, openServes, holdoutSeenKeys, notesDelta, notesSeenKeys, billedDelta } = settlement;
  if (avoidedDelta) {
...
function foldProjectLedger(prev, events, plumbingCap, projectPath, settlement) {
  ...
  return {
    ...
    volume: st.volume,
    // NO USD, deliberately. inCost/outCost used to be persisted here from
    // lib/pricing.js's rate table -- a table that file itself flags as going
    // stale -- which meant every user's disk accumulated a dollar total that
    // silently drifted away from the truth and that nothing was ever allowed
    // to serve. MemBridge also prices at list-price API rates while many
    // users are on flat subscription plans, so the figure was not merely
    // stale but structurally wrong for them. Tokens are the honest unit; a
    // priced figure is neither computed into the ledger nor written down.
    // A legacy ledger's inCost/outCost are dropped on the next fold rather
    // than carried forward (see lib/ledger-fold-state.js normalize).
    reads: st.reads,

...
module.exports = function dashboardStyles(ICON_DATAURI, teamCss, setupCss, upgradeCss) {
...
.px-fix { color:var(--accent); font-weight:600; cursor:pointer; white-space:nowrap; text-decoration:underline; text-underline-offset:2px; }
.px-fix:hover { opacity:.75; }
.px-fix[data-busy] { color:var(--text3); cursor:default; text-decoration:none; }
...
/* Logo tile — mirrors the membridge.app site mark: fixed brand gradient + border,
...
/* Wordmark beside the tile — matches the membridge.app nav lockup
...
#openSignin { display:none; } body.signed-out #openSignin { display:inline-flex; }
/* The upgrade CTA used to be hidden from signed-out users, which left a solo
   user with no way into a team from the header at all — only a bare "Team"
   nav item leading to a sign-in wall. It now stays put and reads "Invite a
   teammate", because signing in is a step INSIDE that flow, not a gate before
   it. Hidden only for a signed-out user who is already on a team, where the
   honest next action is signing back in. */
body.signed-out:not(.solo) #openInvite { display:none; }

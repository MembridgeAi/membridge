...
// "Nothing to save" (spec §8.3) is an honest zero, not a hidden panel or a
// 0-token/0% line that would just look broken — same wording as the
// per-project zero state below.
function pxHomeSavingsLine(totals) {
  var avoided = (totals && totals.avoided) || {};
  var tokens = avoided.tokens || 0;
  if (tokens <= 0) return 'no repeat reads yet';
...
// <serves> reads answered", or the same honest zero when nothing has been
// avoided yet. Never the word "saved" — avoided is what §7.1 measures,
// saved implies the bill fell, which this data cannot support (§7.4/§8.2).
function pxProjectSavingsLine(avoided) {
  var a = avoided || {};
...
function pxRowHtml(p) {
  ...
  var savingsCell = '<div class="px-acc">' + esc(pxProjectSavingsLine(savingsEntry && savingsEntry.avoided)) + '</div>';

...
module.exports = function dashboardClient(MARK_WHITE, teamJs, setupJs, upgradeJs) {
  return `'use strict';
/* ================= shared helpers ================= */
...
// Solo = no project on this machine shares with a team (/api/status.solo).
// Until the first status poll lands we assume solo: a brand-new install IS
// solo, and claiming "Synced" before we know is the exact lie this fixes.
var isSolo = true;
// Why team push is paused, straight from /api/status.auth.paused — set by
// refreshStatus, read here. Team push fails closed on an expired session, and
// it used to do so silently: the pill kept counting up "Synced · Nh ago" while
// the user's team received nothing for two days. Only 'session-expired' warns.
// 'backend-unreachable' is transient and not the user's to fix, and a pill that
...
var setPill = function (ok, teamLastSync) {
  ...
  var warn = !isSolo && authPaused === 'session-expired';
  // setPill is the pill's only author (label, colour and title alike) — the
  // colour has to be reset on every healthy call, or a session that recovers
  // leaves the header amber forever.
  if (pillEl) {
    pillEl.style.color = warn ? 'var(--amber)' : 'var(--accent)';
    pillEl.style.borderColor = warn ? 'var(--amber)' : 'var(--accent-brd)';
    pillEl.style.background = warn ? 'var(--amber-soft)' : 'var(--accent-soft)';
    pillEl.title = warn
      ? 'Your team session expired, so nothing has reached your team since. Sign in again to resume — nothing is lost; held entries push on the next sync.'
      : isSolo
        ? 'Local only — your memory stays on this Mac. Click to sync your projects now.'
        : 'Click to sync now';
  }
  // "Synced" describes a team round-trip. With no team there is nothing to
  // sync WITH, so the pill states what is actually true instead.
  if (isSolo) { pillLabelEl.textContent = 'Local only'; return; }
  if (warn) { pillLabelEl.textContent = 'Sync paused — sign in'; return; }
...
// Encryption state answers "is my team sync safe?". A solo user has no team
// sync, so the badge has no subject — showing "Encrypted" to someone whose
// memory never leaves the machine implies a transmission that isn't happening.
// It returns, accurate, the moment a project is linked to a team.
function applySoloChrome(solo) {
  isSolo = !!solo;
  document.body.classList.toggle('solo', isSolo);
  var invite = document.getElementById('openInvite');
  if (invite) {
    invite.textContent = isSolo ? 'Invite a teammate' : 'Invite';
    invite.title = isSolo
      ? 'Share project memory with someone — creates your team and an invite link'
      : 'Invite teammates';
  }
  // The title is decided in setPill (the pill's single author), which this
  // call below repaints — it has to weigh solo against a paused session too.
  setPill(true);
}
function setE2EBadge(enc) {
  var el = document.getElementById('e2eBadge');
  if (!el) return;
  if (!enc || isSolo) { el.style.display = 'none'; return; }
...
function refreshStatus() {
  return fetch('/api/status').then(function (r) { return r.json(); })
    .then(function (s) {
      ...
      // Before applySoloChrome, which repaints the pill: the paused reason has
      // to be in hand by then or the pill renders one poll behind the truth.
      authPaused = (s && s.auth && s.auth.paused) || null;
      // Solo chrome first: setE2EBadge reads isSolo to decide whether the
      // encryption badge has a subject at all.
      if (s && typeof s.solo === 'boolean') applySoloChrome(s.solo);
...
var currentTab = function () {
  ...
  if (location.hash === '#setup') return 'setup';
  if (location.hash === '#upgrade') return 'upgrade';
...
function showView() {
  ...
  document.getElementById('view-setup').className = t === 'setup' ? 'active' : '';
  document.getElementById('view-upgrade').className = t === 'upgrade' ? 'active' : '';
...
function applyRun() {
  ...
  // #setup renders from its own state and never polls — a wizard that repaints
  // under the user's cursor would lose checkbox focus mid-selection.
  else if (t === 'setup') swRender();
  // #upgrade owns its own state machine and never polls, for the same reason.
  else if (t === 'upgrade') ugRender();
}
...
document.getElementById('openInvite').onclick = function () {
  // A solo user gets the guided upgrade (what changes -> account -> share ->
  // link). Anyone already on a team just wants the Team screen.
  if (isSolo) ugEnter(); else location.hash = '#team';
};
...
function renderSyncBanner(unreachable) {
  ...
  // Delegated rather than duplicated: this runs on a timer, so a second copy of
  // the label logic silently overwrote the solo "Local only" with "Synced" a
  // few seconds after every render. setPill is the single author.
  setPill(!unreachable);
...
/* ================= theme ================= */
// MemBridge opens light unless the reader picked otherwise — a dark desktop no
// longer decides the dashboard's theme for them.
function themePref() {
  try {
...
// Deliberately survives the 5s poll: a user who expanded the list must not
// have it collapse under them.
var pxShowAll = false;
...
function loadProjectsIndex() {
  ...
  fetch('/api/projects').then(function (r) { return r.json(); }).then(function (projects) {
    ...
    Promise.all([
      ...
      // Task 8 (dashboard savings panel): the same read-only payload Task 6
      // wired up. Never fatal to the projects list -- a savings fetch
      // failure degrades to "no repeat reads yet" everywhere, same as a
      // project with no ledger yet.
      fetch('/api/savings').then(function (r) { return r.ok ? r.json() : { projects: [], totals: {} }; }).catch(function () { return { projects: [], totals: {} }; })
    ].concat(detailPs)).then(function (arr) {
      var spinnerPaintedOver = clearSpinner('projects', pxLoadToken);
      setPill(true);
      var cu = arr[0] || {}, team = arr[1] || {}, feed = arr[2] || {}, savings = arr[3] || {};
      var statsByPath = {};
      ...
      var savingsByPath = {};
      (savings.projects || []).forEach(function (s) { if (s && s.path) savingsByPath[s.path] = s; });
      ...
      var fp = JSON.stringify({ p: projects, s: statsByPath, e: pxData.recent, ts: pxData.lastViewedTs, t: pxData.onTeam, off: offline, sv: savingsByPath, svt: pxData.savingsTotals, m: (feedMembers || []).map(function (x) { return x.id; }) });
...
// Only summarised sessions are candidates: an unsummarised one has nothing to
// say about what changed, and letting it win on recency would blank the card's
// glance line while an older session with a real summary sat right behind it.
function pxLatestEntry(p, recent, entryInProject, hasSummary) {
  var best = null;
  (recent || []).forEach(function (e) {
    if (!entryInProject(e, p)) return;
    if (hasSummary && !hasSummary(e)) return;
...
//   2. Never truncated. hooks.js tells every capture that a headline
...
// Three hard rules:
//   1. Never a prompt. \`ask\` is what you ASKED, not what happened.
//   2. Never HARVESTED prose. When a session writes no distilled summary, the
//      adapter falls back to the agent's last chat message (claude-code.js
//      lastText). That is the agent talking to you mid-session — "Three
//      remaining: the day card falls back to prompts in two places" — and
//      presenting it as the record of what changed is how the page ends up
//      quoting Claude back at itself. Only a DISTILLED summary is a summary.
//   3. Never truncated. hooks.js tells every capture that a headline "renders
//      verbatim on a card that never truncates" and bounds it to 80 chars.
function pxGlanceFor(entry) {
  ...
  var summary = entry && entry.distilled && entry.summary ? String(entry.summary).trim() : '';
  if (summary) return { text: summary, kind: 'summary' };
  return { text: 'No summary captured for this session', kind: 'none' };
}
...
// Pure: human-readable token count (1.4M / 118k / 42) for the savings panel
// (Task 8, spec §8). One decimal place at most, so an estimate never reads
// with false precision.
function pxFmtTokens(n) {
  n = Math.max(0, Math.round(n || 0));
  if (n >= 1000000) return (Math.round(n / 100000) / 10) + 'M';
  if (n >= 1000) return Math.round(n / 1000) + 'k';
  return String(n);
}
// Pure: "% of context loaded" (spec §8.4) — the conservative denominator is
// avoided tokens as a share of everything that loaded PLUS what was avoided,
// not just a share of what loaded. volume is the ledger's total token
// volume for the project (lib/ledger-fold.js), independent of recall.
function pxSavingsPct(avoidedTokens, volume) {
  var denom = (avoidedTokens || 0) + (volume || 0);
  if (denom <= 0) return 0;
  return Math.round(1000 * (avoidedTokens || 0) / denom) / 10;
}
// Pure: how many reads the ledger has classified as REPEATS — the same two
// non-first tiers lib/ledger-fold.js counts (a read of a path some session
// has already read, within the session or across sessions). Distinct from
// avoided.tokens, which counts what the recall layer actually SERVED: reads
// can repeat all day while avoidance stays zero, because a hot path only
// becomes servable once it survives skeletonization (lib/recall-store.js's
// warm()). Conflating the two is what made both lines below claim "no repeat
// reads yet" on a project whose ledger had found six.
function pxRepeatReads(reads) {
  var r = reads || {};
  return (r.sameSession || 0) + (r.crossSession || 0);
}
// Pure: the shared zero/near-zero wording for both lines below. Three states,
// not two (spec §8.3 assumed zero avoidance implied zero repeat reads, which
// is false — see pxRepeatReads):
//   no repeats at all     -> "no repeat reads yet"   (§8.3's honest zero)
//   repeats, nothing served -> "<n> repeat read(s) · nothing avoided yet"
// Never "saved" (§8.2) and never a dollar figure (§8.1) in either.
function pxSavingsZeroLine(reads) {
  var n = pxRepeatReads(reads);
  if (n <= 0) return 'no repeat reads yet';
  return n + ' repeat read' + (n === 1 ? '' : 's') + ' · nothing avoided yet';
}
// Pure: the Home roll-up stat (spec §8.2) — "<total> tokens of file reading
// avoided · <pct>% of context loaded", arithmetically the sum of the
// per-project numbers because it is read straight off /api/savings' totals.
// With nothing avoided it falls through to pxSavingsZeroLine, which reports
// detected repeat reads rather than denying them.
function pxHomeSavingsLine(totals) {
  var avoided = (totals && totals.avoided) || {};
  var tokens = avoided.tokens || 0;
  var billed = ((totals && totals.billed) || {}).tokens || 0;
  if (tokens <= 0 && billed <= 0) return pxSavingsZeroLine(totals && totals.reads);
  // Ride-along billed accrual (spec §7.5): when the actual per-request figure
  // exists it leads, and the % finally compares like with like — billed and
  // volume both count every request's context. Still "avoided", never
  // "saved" (§8.2); still tokens, never dollars (§8.1). billed can lag
  // avoided by a pass (a serve too fresh to have ridden yet) — fall back to
  // the once-only line rather than showing a zero that reads as "no effect".
  if (billed > 0) {
    var billedPct = pxSavingsPct(billed, (totals && totals.volume) || 0);
    return pxFmtTokens(billed) + ' tokens of request context avoided (' + pxFmtTokens(tokens) +
      ' at first load) · ' + billedPct + '% of all context';
  }
  var pct = pxSavingsPct(tokens, (totals && totals.volume) || 0);
  return pxFmtTokens(tokens) + ' tokens of file reading avoided · ' + pct + '% of context loaded';
}
// Pure: the teammate-notes input-cost line (notes spec §9). Deliberately a
// SEPARATE line from pxHomeSavingsLine above, never a term inside it: an
// injected note is input spent, and what it buys is a mistake nobody made,
// which this ledger cannot observe. Netting the two would need either a
// fabricated avoidance figure or a feature that always reads as a loss, so the
// number is reported plainly and the second sentence says why it sits outside.
// Tokens, never dollars; "avoided", never "saved" (§9, token-reduction
// §8.1/§8.2). Empty string when nothing has been injected — a zero here is
// noise, not the honest-zero case §8.3 is about.
function pxNotesInjectedLine(totals) {
  var n = (totals && totals.notesInjectedTokens) || 0;
  if (n <= 0) return '';
  return 'Tokens injected as teammate notes: ' + pxFmtTokens(n) +
    '. Teammate notes are an input cost, not an avoidance — their value is work ' +
    'you did not redo, which this ledger cannot measure. Counted here, kept out ' +
    'of the figure above.';
}
// Pure: the per-project savings line (spec §8.2/§8.3) — "<n> avoided ·
// <serves> reads answered", or pxSavingsZeroLine's honest report when nothing
// has been avoided yet. Never the word "saved" — avoided is what §7.1
// measures, saved implies the bill fell, which this data cannot support
// (§7.4/§8.2). The reads argument is the same per-project block /api/savings
// already ships beside avoided; omitted, this degrades to the old two-state
// behaviour rather than throwing.
function pxProjectSavingsLine(avoided, reads, billed) {
  var a = avoided || {};
  var tokens = a.tokens || 0;
  var b = (billed || {}).tokens || 0;
  if (tokens <= 0 && b <= 0) return pxSavingsZeroLine(reads);
  // Ride-along billed accrual (spec §7.5): same leading-figure rule as
  // pxHomeSavingsLine — the per-request figure when it exists, the once-only
  // figure as fallback. billed is the third argument (an older payload
  // simply omits it) so old callers degrade to exactly the old line.
  if (b > 0) {
    return pxFmtTokens(b) + ' avoided across requests (' + pxFmtTokens(tokens) +
      ' at first load) · ' + (a.serves || 0) + ' reads answered';
  }
  return pxFmtTokens(tokens) + ' avoided · ' + (a.serves || 0) + ' reads answered';
}
...
// How far a project's newest commit may outrun its newest captured session
// before we call capture broken rather than merely quiet. A day is wide enough
// that committing without an AI session (a manual fix, a rebase) never trips
// it, and narrow enough that a genuinely dead adapter shows up the same day.
var PX_CAPTURE_GAP_MS = 24 * 3600 * 1000;
// "Commits are still landing here, but nothing has been captured since well
// before the latest one." Commits come from the git post-commit hook and
// events from the transcript scan, so this divergence means one path died
// while the other kept working — previously invisible, because a project that
// stops reporting looks exactly like a project nobody is touching.
function pxCaptureStale(p) {
  if (!p || p.paused || !p.lastCommit) return false;
  var commit = new Date(p.lastCommit).getTime();
  if (!isFinite(commit)) return false;
  // The repo itself has gone quiet: nothing to warn about.
  if (Date.now() - commit > PX_DORMANT_DAYS * 86400000) return false;
  var seen = p.lastActivity ? new Date(p.lastActivity).getTime() : 0;
  if (!isFinite(seen)) seen = 0;
  return commit - seen > PX_CAPTURE_GAP_MS;
}
...
// The one-click repair for a missing block. A syncing pass rewrites the block
// into every context file, so the fix is a project-scoped sync — no new
// endpoint, and the same code path that would have written it in the first
// place. Rendered next to the warning, because a health warning the user
// cannot act on is just nagging.
function pxFixBlockHtml(pathAttr) {
  return ' <span class="px-fix" data-px-fixblock="' + pathAttr + '">Fix it</span>';
}
// Does this entry have anything to say about what changed? A session with no
// distilled summary produced no record, and a row saying "no summary captured"
// is still a row of nothing — so such sessions are omitted from timelines
// entirely rather than rendered as placeholders. Prompts and harvested chat
// prose do not count; pxGlanceFor is the single authority on what does.
function pxHasSummary(e) {
  return pxGlanceFor(e).kind !== 'none';
}
...
function pxRowHtml(p) {
  ...
  var captureStale = pxCaptureStale(p);
  ...
  if (pxIsDim(p)) {
    ...
    var status = p.paused
      ...
      : captureStale
        // Not dormant, broken: the repo is being committed to right now.
        ? '<span class="px-chip">not capturing</span><span>commits are landing here, but ' +
          (p.lastActivity
            ? 'nothing has been captured since <span data-ago="' + esc(p.lastActivity) + '">' + esc(ago(p.lastActivity)) + '</span>'
            : 'nothing has ever been captured') + '</span>'
        : pxBlockMissing(p)
          ? '<span class="px-chip">memory not in your context files</span>' +
            '<span>your AI tools here are reading no shared memory' + pxFixBlockHtml(pathAttr) + '</span>'
          : ageDays === null
  ...
  var latest = pxLatestEntry(p, pxData.recent, pxEntryInProject, pxHasSummary);
  ...
  var mparts = [];
  // Leads the meta line: an active-looking row whose capture has actually
  // stopped is the one case where the row's own content is misleading.
  if (captureStale) mparts.push('<span class="px-chip">not capturing</span>');
  if (pxBlockMissing(p)) mparts.push('<span class="px-chip">memory not in your context files</span>' + pxFixBlockHtml(pathAttr));
  ...
  // What you still owe here, in words. The old meta line carried a bare count
  // of these; the text is the thing that actually reloads your head.
  ...
  // Savings line (Task 8, spec §8.2/§8.3), from /api/savings via
  // pxData.savingsByPath — "<n> avoided · <serves> reads answered", or the
  // honest zero when the project has no ledger yet or nothing avoided so far.
  var savingsEntry = (pxData.savingsByPath || {})[p.path];
  var savingsCell = '<div class="px-acc">' + esc(pxProjectSavingsLine(savingsEntry && savingsEntry.avoided, savingsEntry && savingsEntry.reads, savingsEntry && savingsEntry.billed)) + '</div>';
  ...
  return '<div class="px-card">' +
    ...
    latestCell + todoCell + accCell + savingsCell + whoCell + spark + menu + '</div>';
}
...
function renderProjectsIndex() {
  ...
  // Which filters have earned their space, decided BEFORE anything filters:
  // a hidden filter must never keep filtering, or rows vanish with no visible
  // control explaining why. Person options are recent authors UNION team
  // members, so a member with no local recent activity is still selectable
  // (their shared projects match by membership in pxPersonFilter).
  ...
  // Home savings stat (Task 8, spec §8.2): tokens of file reading avoided,
  // arithmetically the sum of the per-project numbers because it comes
  // straight off /api/savings' totals. An honest zero (spec §8.3) reads
  // "no repeat reads yet" — never hidden, never a dollar figure.
  var savingsLine = pxHomeSavingsLine(pxData.savingsTotals);
  // Teammate-note input cost (notes spec §9) — its own line, under the
  // avoidance figure and outside it. Absent entirely until something has
  // actually been injected.
  var notesLine = pxNotesInjectedLine(pxData.savingsTotals);

  // filters row: Show/Sort are fixed sets → segmented controls; Person grows
  // with the team → dropdown. Each draws only if it has more than one option;
  // Sort needs more than one project to be worth sorting. A row with no
  // surviving control collapses entirely rather than leaving an empty rule.
  var ctls =
  ...
  host.innerHTML =
    '<main style="max-width:780px;margin:0 auto;padding:48px 28px 110px;animation:mbFade .4s cubic-bezier(.16,1,.3,1)">' +
      ...
      '</div>' +
      '<div style="color:var(--text3);font-size:12.5px;margin-bottom:6px">' + esc(subline) + '</div>' +
      '<div style="color:var(--accent);font-size:12.5px;font-weight:600;margin-bottom:' + (notesLine ? '4px' : '30px') + '">' + esc(savingsLine) + '</div>' +
      (notesLine ? '<div style="color:var(--text3);font-size:11.5px;line-height:1.5;max-width:62ch;margin-bottom:30px">' + esc(notesLine) + '</div>' : '') +
      filters + grid + footer +
    '</main>';
}
...
function renderProjectsEmpty(host) {
  Promise.all([
    ...
    // This screen already asked /api/scan for the tool list, and that payload now
    // carries every discovered project plus whether it is watched (worktrees
    // already folded into their main repo). So first run can offer the projects
    // you have ALREADY worked in instead of asking for a path you clearly have.
    stDiscovered = (scan.projects || []).filter(function (p) { return !p.tracked; });
    var dashTile = '<span style="width:24px;height:24px;border-radius:8px;border:1.5px dashed var(--border2);flex:none"></span>';
    var addBtn = '<button data-px-add style="height:38px;padding:0 15px;border-radius:10px;border:1px solid var(--accent-brd);background:var(--accent-soft);color:var(--accent);font-size:12.5px;font-weight:600;cursor:pointer;font-family:inherit;transition:all .2s" style-hover="box-shadow:var(--shadow-accent)">Add manually</button>';
    var nDisc = stDiscovered.length;
    var adoptBtn = '<button data-adopt-all style="height:38px;padding:0 15px;border-radius:10px;border:none;background:var(--grad);color:#fff;font-size:12.5px;font-weight:600;cursor:pointer;font-family:inherit;box-shadow:var(--shadow-accent);transition:all .2s ease-out" style-hover="transform:translateY(-1px)" style-active="transform:scale(.98)">' + (nDisc === 1 ? 'Watch it' : 'Watch all ' + nDisc) + '</button>';
    var lastRow = nDisc
      ? '<div style="display:flex;gap:14px;align-items:center;padding:14px 0;border-top:1px solid var(--border)">' + checkTile +
          '<div style="flex:1"><span style="font-weight:600;font-size:13.5px">' + nDisc + ' project' + (nDisc === 1 ? '' : 's') + ' found on this Mac</span>' +
          '<span style="color:var(--text3);font-size:12.5px"> \u2014 your AI tools have already worked in ' + (nDisc === 1 ? 'it' : 'them') + '</span></div>' +
          '<div style="display:flex;gap:8px">' + addBtn + adoptBtn + '</div></div>'
      : '<div style="display:flex;gap:14px;align-items:center;padding:14px 0;border-top:1px solid var(--border)">' + dashTile +
          '<div style="flex:1"><span style="font-weight:600;font-size:13.5px">Add your first project</span>' +
          '<span style="color:var(--text3);font-size:12.5px"> \u2014 point MemBridge at a folder you code in</span></div>' + addBtn + '</div>';
    host.innerHTML =
      '<main style="max-width:780px;margin:0 auto;padding:48px 28px 110px;animation:mbFade .4s cubic-bezier(.16,1,.3,1)">' +
        '<div style="display:flex;align-items:flex-end;gap:16px;flex-wrap:wrap;margin-bottom:8px">' +
          '<h1 style="margin:0;font-family:Calistoga,Georgia,serif;font-size:40px;font-weight:400;letter-spacing:-0.02em;line-height:1.05">Projects</h1>' +
...
document.getElementById('view-home').addEventListener('click', function (e) {
  ...
  // Repair a missing memory block: a project-scoped sync rewrites it into every
  // context file. Stops propagation so the click does not also open the project.
  var fixBlock = e.target.closest('[data-px-fixblock]');
  if (fixBlock) {
    e.stopPropagation();
    if (fixBlock.getAttribute('data-busy')) return;
    fixBlock.setAttribute('data-busy', '1');
    fixBlock.textContent = 'Fixing…';
    fetch('/api/sync', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ project: fixBlock.getAttribute('data-px-fixblock') }),
    }).then(function (r) {
      if (!r.ok) throw new Error('sync failed');
      pxFp = '';
      loadProjectsIndex();
    }).catch(function () {
      fixBlock.removeAttribute('data-busy');
      fixBlock.textContent = 'Fix failed — retry';
    });
    return;
  }
  // First-run "Watch all N" — adopts everything discovery found, so a new
  // install never has to type a path it could have found itself.
  if (e.target.closest('[data-adopt-all]')) {
    adoptPaths((stDiscovered || []).map(function (p) { return p.path; }));
    return;
  }
...
// The Intent row draws only when an intent was actually captured. It used to
// always render for layout stability, which meant most cards carried a row
// whose only content was "(not captured)" — the same empty-block noise the
// Projects cards dropped. A row with nothing to say should not take space.
function intentRowHtml(goal, rowStyle) {
  if (!goal) return '';
  var style = rowStyle ? ' style="' + rowStyle + '"' : '';
  return '<div class="fmeta-row"' + style + '><span class="flabel">Intent</span>' + esc(goal) + '</div>';
}
...
function catchupCardHtml(e) {
  ...
  // Same glance chain as the Projects card: headline, then summary, never the
  // raw prompt. "wip" now means nothing has been distilled yet at all, so a
  // session that produced a headline but no long summary is no longer labelled
  // as still in progress.
  var glance = pxGlanceFor(e);
  var wip = glance.kind === 'none';
  ...
  var head = '<div data-card-toggle="' + id + '" style="display:flex;gap:14px;align-items:flex-start;padding:17px 12px 17px 4px;margin:0 -12px 0 -4px;cursor:pointer;border-radius:14px;transition:background .2s" style-hover="background:var(--surface2)">' +
    ...
    '<div style="flex:1;min-width:0">' +
      ...
      '<div style="font-size:15px;font-weight:600;letter-spacing:-0.01em;line-height:1.45;text-wrap:pretty">' +
        (wip
          ? '<span style="color:var(--amber)">Working now</span><span style="color:var(--text3);font-weight:400"> — the summary lands when this session ends</span>'
          : esc(glance.text)) + '</div>' +
...
function feedFilterBarHtml(entries) {
  ...
  // Distilled is a summary source, not a tool, so it gets its own control below
  // instead of masquerading as something the work was written in.
  var toolOpts = [all].concat(Object.keys(feedToolsSeen).sort()
    .filter(function (k) { return k !== DISTILLED_SOURCE; })
    .map(function (k) { return { label: feedToolsSeen[k], val: k }; }));
  var distilledOnly = feedFilters.source === DISTILLED_SOURCE;
  return feedSelectCtl('author', 'Person', personOpts, feedFilters.author) +
    ...
    feedSegment('source', 'Tool', toolOpts, distilledOnly ? null : feedFilters.source) +
    feedDistilledCtl(distilledOnly);
}
// Same feedFilters.source channel the Tool segment uses — checking the box just
// narrows to the distilled rows, so no filtering power is lost by taking
// Distilled out of the tool list.
function feedDistilledCtl(on) {
  return '<label data-feed-distilled style="display:inline-flex;align-items:center;gap:7px;height:30px;padding:0 13px 0 10px;' +
    'border:1px solid var(--border);border-radius:99px;background:var(--card);cursor:pointer;font-size:12px;' +
    (on ? 'color:var(--accent);font-weight:600' : 'color:var(--text2)') + '">' +
    '<input type="checkbox"' + (on ? ' checked' : '') + ' style="margin:0;accent-color:var(--accent);cursor:pointer">Distilled only</label>';
}
...
function buildDayCards(units) {
  ...
  cards.forEach(function (c) {
    ...
    // No distilled pick: the only card that still renders in that state is a
    // live one (the filter below drops the rest), so this says work is under
    // way rather than counting sessions nobody summarised.
    c.headline = pick
      ? runHeadline(displayRep(pick, c.self), pick.runs[0].entries[0], pick.live)
      : '<span style="color:var(--amber)">Working now</span>';
  });
  ...
  cards.forEach(function (c) {
    ...
    c.units.forEach(function (u) {
      ...
      // No summary, no checklist row — a live unit still shows, because it is
      // reporting that work is happening, not claiming what it produced. The
      // file tally below still counts this unit: the work happened whether or
      // not anyone wrote it down, and the day's totals must stay honest.
      if (rep || u.live) {
        rows.push({ glyph: u.live ? '◐' : '✓', text: runHeadline(rep, u.runs[0].entries[0], u.live), live: !!u.live });
      }
...
function runHeadline(rep, newest, live) {
  ...
  // Neither harvested prose NOR the prompt. The old fallback turned every
  // undistilled session into its own first sentence — "build that", "merge this
  // with the stable release, but keep it in the..." — which reads as a
  // transcript of what was typed, not a record of what changed.
  if (live) return '<span style="color:var(--amber)">Working now</span>';
  return '<span style="color:var(--text3)">session ended &middot; no summary captured</span>';
}
...
// bottom-right.
// "Distilled" is a summary SOURCE — it means MemBridge wrote the summary itself
// — so it never belongs in the list of tools the work was done in.
var DISTILLED_SOURCE = 'Distilled';
function daySources(c) {
  return (c.tools && c.tools.length) ? c.tools : (c.source ? [c.source] : []);
}
function dayTools(c) {
  return daySources(c).filter(function (s) { return s !== DISTILLED_SOURCE; });
}
function dayIsDistilled(c) {
  return daySources(c).indexOf(DISTILLED_SOURCE) !== -1;
}
// Headlines and checklist rows both arrive as small HTML fragments, so they can
// only be compared as the text a reader actually sees.
function dayRowText(html) {
...
function dayCardHtml(c, opts) {
  ...
  var workingTag = '<span style="flex:none;display:inline-flex;align-items:center;gap:5px;' + MONO + ';font-size:9px;letter-spacing:.1em;text-transform:uppercase;color:var(--amber);font-weight:600"><span style="width:5px;height:5px;border-radius:50%;background:var(--amber);animation:mbPulse 2s ease infinite"></span>working now</span>';
  // The headline is PICKED FROM the checklist, so the winning run would print
  // twice — once as the card title, once as its own bullet. Drop the duplicate
  // and carry its live state up to the headline so no "working now" is lost.
  var headlineText = dayRowText(c.headline);
  var headlineLive = false;
  var rows = (c.checklist || []).filter(function (r) {
    if (dayRowText(r.text) !== headlineText) return true;
    if (r.live) headlineLive = true;
    return false;
  });
  var rowHtml = function (r) {
    var glyphColor = r.live ? 'var(--amber)' : (r.glyph === '✓' ? 'var(--green)' : 'var(--text3)');
    var tag = r.live ? workingTag : '';
    return '<div style="display:flex;gap:9px;align-items:baseline;padding:3.5px 0">' +
      ...
      '<span style="flex:1;min-width:0;font-size:13px;color:var(--text2);line-height:1.5;text-wrap:pretty">' + r.text + '</span>' +
      tag +
    '</div>';
  };
  ...
  var head = '<div data-day-open="' + keyAttr + '" style="display:flex;gap:14px;align-items:flex-start;padding:2px 12px 2px 4px;margin:0 -12px 0 -4px;cursor:pointer;border-radius:14px;transition:background .2s">' +
    ...
    '<div style="flex:1;min-width:0">' +
      '<div style="display:flex;align-items:center;gap:9px;font-size:12px;color:var(--text3);flex-wrap:wrap">' +
        ...
        // Distilled is how the summary was made, not a tool anyone codes in, so
        // it reads as its own mark rather than sitting in the tool row.
        (dayIsDistilled(c) ? '<span style="padding:2px 9px;border-radius:99px;border:1px solid color-mix(in srgb, var(--green) 40%, transparent);background:color-mix(in srgb, var(--green) 10%, transparent);' + MONO + ';font-size:9.5px;letter-spacing:.1em;text-transform:uppercase;color:var(--green)">Distilled</span>' : '') +
      ...
      '</div>' +
      '<div style="display:flex;gap:9px;align-items:baseline;margin-top:6px">' +
        '<span style="flex:1;min-width:0;font-size:15px;font-weight:600;letter-spacing:-0.01em;line-height:1.45;text-wrap:pretty;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">' + c.headline + '</span>' +
        (headlineLive ? workingTag : '') +
      '</div>' +
    '</div>' +
  '</div>';
...
function dayDetailHtml(c, opts) {
  ...
  // Same rule as the day card and its checklist: a session that produced no
  // summary has nothing to report, so it gets no card here either. Drilling in
  // used to reveal exactly the "session ended · no summary captured" rows the
  // level above had already dropped.
  var units = (c.units || []).filter(function (u) {
    return !!(u.live || (u.rep && u.rep.rep) || (!c.self && u.repHarvested && u.repHarvested.repHarvested));
  });
...
function feedDayGroupHtml(entries, opts) {
  ...
  if (opts && opts.unitCards) {
    // The project page renders one card per unit. Same rule as everywhere else:
    // a session that produced no summary has nothing to report, so it is not
    // given a card that only says so.
    units = units.filter(function (u) {
      var self = !!(u.runs && u.runs[0] && u.runs[0].entries[0] && u.runs[0].entries[0].self);
      return !!(u.live || (u.rep && u.rep.rep) || (!self && u.repHarvested && u.repHarvested.repHarvested));
    });
  ...
  // A day whose every session went unsummarised has nothing to report, so the
  // card goes the same way its rows did — gating the checklist alone left the
  // card standing with "N sessions · no summaries shared" as its headline, the
  // same row of nothing one level up. Filtered at RENDER time so buildDayCards
  // stays a faithful data builder and its totals still see every session.
  var cards = buildDayCards(units).filter(function (c) {
    return !!(c.checklist && c.checklist.length);
  });
...
document.getElementById('view-feed').addEventListener('change', function (e) {
  ...
  // "Distilled only" rides the same source filter the Tool segment sets, so
  // ticking it replaces whatever tool was selected and clearing it shows all.
  var dist = e.target.closest('[data-feed-distilled]');
  if (dist) {
    feedFilters.source = e.target.checked ? DISTILLED_SOURCE : null;
    resetFeedPaging(); feedFp = ''; loadFeed();
  }
});
...
function sessionPageHtml(t, subs) {
  ...
  var liveBadge = live
    ...
    : '';
  // Never the prompt. A session with no distilled summary has nothing to say
  // about what CHANGED, and echoing the ask back dresses the question up as the
  // answer. The prompts are still one click away in the expanded detail.
  var headline = t.rep
    ? esc(t.rep.summaryFull || t.rep.summary)
    : t.repHarvested
      ? esc(t.repHarvested.summaryFull || t.repHarvested.summary)
      : live
        ? '<span style="color:var(--amber)">Working now</span><span style="color:var(--text3);font-weight:400"> &mdash; the summary lands when this session ends</span>'
        : '<span style="color:var(--text3)">session ended &middot; no summary captured</span>';
...
// "Add a project" leads with what MemBridge can already see: the folders your
// AI tools have worked in, adoptable in one click. Typing a path is the
// fallback underneath, not the primary way in.
function renderAddDiscovered() {
  var host = document.getElementById('addDiscovered');
  if (!host) return;
  if (stDiscoverBusy) {
    host.innerHTML = '<div style="font-size:12.5px;color:var(--text3);padding:10px 0">Looking through your AI tools&rsquo; session logs&hellip;</div>';
    return;
  }
  if (!stDiscovered || !stDiscovered.length) {
    host.innerHTML = '<div style="font-size:12.5px;color:var(--text3);padding:10px 0">Nothing new found — every project with AI activity is already watched.</div>';
    return;
  }
  var rows = stDiscovered.map(function (p) {
    return '<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-top:1px solid var(--border)">' +
      '<div style="flex:1;min-width:0">' +
        ...
        '<div style="' + MONO + ';font-size:10px;color:var(--text3);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + esc(p.path) + '</div>' +
      '</div>' +
      '<span data-adopt-one="' + esc(p.path) + '" style="font-size:12px;color:var(--accent);cursor:pointer;font-weight:600">Watch</span></div>';
  }).join('');
  host.innerHTML =
    '<div style="display:flex;align-items:center;gap:10px;padding-bottom:4px">' +
      '<div style="flex:1;font-size:12.5px;color:var(--text2)">' + stDiscovered.length + ' found</div>' +
      '<span data-adopt-all style="font-size:12px;color:var(--accent);cursor:pointer;font-weight:600">Watch all</span></div>' +
    '<div style="max-height:230px;overflow-y:auto">' + rows + '</div>';
}
...
function openAdd() {
  ...
  // Always re-scan on open: projects appear as you work, and a stale list is
  // worse than a one-second wait.
  stDiscoverBusy = true;
  renderAddDiscovered();
  fetch('/api/scan').then(function (r) { return r.json(); }).then(function (d) {
    stDiscovered = ((d && d.projects) || []).filter(function (p) { return !p.tracked; });
    stDiscoverBusy = false;
    renderAddDiscovered();
  }).catch(function () {
    stDiscoverBusy = false;
    stDiscovered = [];
...
// The modal sits outside view-home and settingsRoot, so its adopt clicks need
// their own delegation. Close on adopt: the project then shows up as a real
// row behind the modal, which is the confirmation.
addOverlay.addEventListener('click', function (e) {
  var one = e.target.closest && e.target.closest('[data-adopt-one]');
  if (one) { var p = one.getAttribute('data-adopt-one'); closeAdd(); adoptPaths([p]); return; }
  if (e.target.closest && e.target.closest('[data-adopt-all]')) {
    var all = (stDiscovered || []).map(function (x) { return x.path; });
    closeAdd(); adoptPaths(all); return;
  }
...
// Discovered-but-unadopted projects (GET /api/scan). null until the user asks:
// a scan re-reads every session file from byte 0 across every adapter, which
// takes seconds on a busy machine, so it must never run on a plain Settings
// render. null = never scanned, [] = scanned and nothing left to adopt.
var stDiscovered = null;
var stDiscoverBusy = false;
var stDiscoverNotice = null;
...
// Projects your AI tools have worked in that MemBridge is NOT yet watching.
// Discovery already sees them; the ingestion gate drops their sessions until
// they are adopted, so this is the bridge between "found" and "watched".
// Nothing is written to any folder until the user clicks — adopting a project
// is what lets a memory block land in it on the next sync.
function settingsDiscoveredSection() {
  var lbl = '<div style="' + STLABEL + '">Discovered on this Mac</div>';
  var link = 'font-size:13px;color:var(--accent);cursor:pointer;font-weight:600';
  var notice = '';
  if (stDiscoverNotice) { notice = '<div class="notice" style="margin-bottom:10px">' + esc(stDiscoverNotice) + '</div>'; stDiscoverNotice = null; }

  var inner;
  if (stDiscoverBusy) {
    inner = '<div style="padding:14px 18px;font-size:12.5px;color:var(--text3)">Looking through your AI tools&rsquo; session logs&hellip;</div>';
  } else if (stDiscovered === null) {
    inner = '<div style="padding:14px 18px;display:flex;align-items:center;gap:12px">' +
      '<div style="flex:1;font-size:12.5px;color:var(--text3)">Find folders your AI tools have already worked in, without typing any paths.</div>' +
      '<span data-discover-scan style="' + link + '">Scan this Mac</span></div>';
  } else if (!stDiscovered.length) {
    inner = '<div style="padding:14px 18px;display:flex;align-items:center;gap:12px">' +
      '<div style="flex:1;font-size:12.5px;color:var(--text3)">Nothing new &mdash; every project with AI activity is already watched.</div>' +
      '<span data-discover-scan style="' + link + '">Scan again</span></div>';
  } else {
    var rows = stDiscovered.map(function (p) {
      var tools = Object.keys(p.bySource || {}).join(', ');
      var meta = (p.sessionCount ? p.sessionCount + (p.sessionCount === 1 ? ' event' : ' events') : '') +
        (tools ? ' &middot; ' + esc(tools) : '');
      return '<div style="display:flex;align-items:center;gap:12px;padding:14px 18px;border-bottom:1px solid var(--border)">' +
        '<div style="flex:1;min-width:0">' +
          '<div style="font-size:13.5px;font-weight:600">' + esc(p.name) + '</div>' +
          '<div style="' + MONO + ';font-size:10.5px;color:var(--text3);margin-top:3px">' + esc(p.path) + '</div>' +
          (meta ? '<div style="font-size:11px;color:var(--text3);margin-top:2px">' + meta + '</div>' : '') +
        '</div>' +
        '<span data-adopt-one="' + esc(p.path) + '" style="font-size:12px;color:var(--accent);cursor:pointer;font-weight:600;transition:opacity .2s" style-hover="opacity:.7">Adopt</span></div>';
    }).join('');
    var bar = '<div style="display:flex;align-items:center;gap:12px;padding:11px 18px;border-bottom:1px solid var(--border)">' +
      '<div style="flex:1;font-size:12.5px;color:var(--text2)">' + stDiscovered.length + ' project' + (stDiscovered.length === 1 ? '' : 's') + ' found, not watched yet</div>' +
      '<span data-adopt-all style="font-size:12px;color:var(--accent);cursor:pointer;font-weight:600">Adopt all</span></div>';
    var foot = '<div style="display:flex;align-items:center;padding:13px 18px">' +
      '<span data-discover-scan style="' + link + '">Scan again</span></div>';
    inner = bar + rows + foot;
  }
  return lbl + notice + '<div style="' + STCARD + ';margin-bottom:34px">' + inner + '</div>';
}

// Adopting is reachable from Settings AND from the first-run Projects screen,
// so every refresh here re-renders whichever one the user is actually on.
function refreshDiscoverView() {
  if (currentTab() === 'settings') renderSettingsView(); else loadHome();
}

// Ask the server what it can see, keeping only what is not already watched.
function runDiscover() {
  if (stDiscoverBusy) return;
  stDiscoverBusy = true;
  refreshDiscoverView();
  fetch('/api/scan').then(function (r) { return r.json(); }).then(function (d) {
    var all = (d && d.projects) || [];
    stDiscovered = all.filter(function (p) { return !p.tracked; });
    stDiscoverBusy = false;
    refreshDiscoverView();
  }).catch(function () {
    stDiscoverBusy = false;
    stDiscovered = [];
    stDiscoverNotice = 'Could not scan just now — try again.';
    refreshDiscoverView();
    setPill(false);
  });
}

// One request for the whole set: a partial sweep would leave the list in a
// state the user has to re-reason about.
function adoptPaths(paths) {
  if (!paths.length || stDiscoverBusy) return;
  stDiscoverBusy = true;
  refreshDiscoverView();
  fetch('/api/projects/adopt', {
    ...
    stDiscoverBusy = false;
    var n = (d && d.adoptedCount) || 0;
    var failed = (d && d.skipped) || [];
    stDiscovered = (stDiscovered || []).filter(function (p) { return paths.indexOf(p.path) === -1; });
    stDiscoverNotice = n
      ? ('Now watching ' + n + ' project' + (n === 1 ? '' : 's') + ' — memory starts building on the next sync.')
      : null;
    if (failed.length && !n) stDiscoverNotice = 'Nothing was adopted (' + esc(failed[0].reason || 'failed') + ').';
    // Re-pull the watched list so adopted projects appear as real rows.
    if (currentTab() === 'settings') loadSettings(); else loadHome();
  }).catch(function () {
    stDiscoverBusy = false;
    stDiscoverNotice = 'Could not adopt just now — try again.';
    refreshDiscoverView();
    setPill(false);
  });
}
...
if (settingsRoot) {
  settingsRoot.addEventListener('click', function (e) {
    ...
    if (e.target.closest('[data-discover-scan]')) { runDiscover(); return; }
    var adoptOne = e.target.closest('[data-adopt-one]');
    if (adoptOne) { adoptPaths([adoptOne.getAttribute('data-adopt-one')]); return; }
    if (e.target.closest('[data-adopt-all]')) {
      adoptPaths((stDiscovered || []).map(function (p) { return p.path; }));
      return;
    }
...
function feedEntryHtml(e, opts) {
  ...
  var summaryLine =
    intentRowHtml(e.goal) +
    '<div class="fsummary">' + (wip
      ? '<span class="fworking-lbl">Working now</span><span style="color:var(--text3);font-weight:400"> &mdash; the summary lands when this session ends</span>'
      : esc(glance.text)) + '</div>' +
    ((e.decisions || e.gotchas) ? '<div class="fsub">' + esc([e.decisions, e.gotchas].filter(Boolean).join(' · ')) + '</div>' : '');
...
// Spec §6: the human surface for live teammate decisions. The in-terminal line
// was proved impossible (FileChanged suppresses systemMessage), so this card is
// how a decision reaches a PERSON -- it rides the existing 5s project poll, no
// new endpoint and no new mechanism.
//
// Each row is who, what they decided, and when. The when is load-bearing: the
// payload has no age filter by design, so without it a three-month-old decision
// would read as one that just landed. data-ago is the page's own live-relative
// hook, so refreshAgo keeps it honest between rebuilds.
function pjTeammateNotesHtml(tn) {
  var items = (tn && tn.fresh) || [];
  if (!items.length) return '';
  var rows = items.map(function (n) {
    var when = n.ts
      ? '<span style="font-size:11.5px;color:var(--text3);flex:none" data-ago="' + esc(n.ts) + '">' + esc(ago(n.ts)) + '</span>'
      : '';
    return '<div style="display:flex;gap:12px;align-items:baseline;padding:11px 20px;border-top:1px solid var(--border)">' +
      '<span style="font-size:13px;font-weight:600;flex:none">' + esc(n.author || 'a teammate') + '</span>' +
      '<span style="flex:1;font-size:13px;color:var(--text2);line-height:1.55">' + esc(n.text) + '</span>' +
      when + '</div>';
  }).join('');
  // Never drop what is over the cap in silence -- say how much is not here and
  // where the rest lives.
  var hidden = Math.max(0, (tn.total || items.length) - items.length);
  var rest = hidden
    ? '<div style="padding:11px 20px;border-top:1px solid var(--border);font-size:12.5px;color:var(--text3)">' +
        hidden + ' more in the feed</div>'
    : '';
  var head = '<div style="display:flex;align-items:center;gap:10px;padding:14px 20px">' +
    '<span style="font-family:\\'JetBrains Mono\\',monospace;font-size:9.5px;letter-spacing:.1em;text-transform:uppercase;color:var(--text3)">What your teammates decided</span>' +
    '</div>';
  return '<section style="margin-top:28px;border:1px solid var(--border);border-radius:16px;background:var(--card);box-shadow:var(--shadow-md);overflow:hidden">' +
    head + rows + rest + '</section>';
}
function renderProject(feed, detail, value, cx) {
  ...
  // Newly arrived teammate decisions sit above the Catch-Up: they are the
  // sharpest thing on the page and the reason to look at it at all. Team-only
  // projects have no local folder, so no index and nothing to draw.
  var teammateNotes = local ? pjTeammateNotesHtml(detail.teammateNotes) : '';
  ...
  var h = head + pausedBanner + teammateNotes + catchup + stats + notice +
...
// First-run gate. One status read decides whether the wizard takes the window;
// swShouldRun refuses on anything it cannot positively confirm, so a failed
// fetch shows the normal dashboard instead of hijacking a working install.
// An explicit #setup in the hash always wins — that is a deliberate revisit.
refreshStatus().then(function (s) {
  if (location.hash === '#setup') { swEnter(); return; }
  if (swShouldRun(s) && !location.hash) swEnter();
});`;
};

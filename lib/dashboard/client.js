...
/* ===================== The Projects index (#home) =====
   ...
   people, a 14-day sparkline and the ⋯ menu, so ~15 projects read at a
...
// Pure: sessions-per-day over the trailing 14 days (oldest bucket first),
// counted from the recent feed window. The clock is injected (now) so tests
// can pin it; buckets are local calendar days, matching the feed's grouping.
function pxSparkCounts(p, recent, entryInProject, now) {
  var DAY = 86400000;
  var n = new Date(now);
  var today = new Date(n.getFullYear(), n.getMonth(), n.getDate()).getTime();
  var counts = [];
  for (var i = 0; i < 14; i++) counts.push(0);
  (recent || []).forEach(function (e) {
    if (!entryInProject(e, p)) return;
    var d = new Date(e.ts);
    if (isNaN(d)) return;
    var back = Math.round((today - new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime()) / DAY);
    if (back >= 0 && back < 14) counts[13 - back]++;
  });
  return counts;
}
...
// who | 14d sparkline | ⋯. Same data-px-* hooks as the old editorial rows,
...
function pxRowHtml(p) {
  ...
  var spark = '<div class="px-spark" title="Sessions, last 14 days">' + pxSparkCounts(p, pxData.recent, pxEntryInProject, Date.now()).map(function (v) {
...
function renderProjectsIndex() {
  ...
  var footer = solo
    ...
    : '';
  var grid =
    '<div class="px-hdr"><span>Project</span><span>Latest session</span><span>Who</span><span>14d</span><span></span></div>' +
    (vis.length

...
function pxHomeSavingsLine(totals) {
  ...
  if (tokens <= 0) return pxSavingsZeroLine(totals && totals.reads);
...
function pxProjectSavingsLine(avoided, reads) {
  var a = avoided || {};
  var tokens = a.tokens || 0;
  if (tokens <= 0) return pxSavingsZeroLine(reads);
...
function pxRowHtml(p) {
  ...
  var savingsCell = '<div class="px-acc">' + esc(pxProjectSavingsLine(savingsEntry && savingsEntry.avoided, savingsEntry && savingsEntry.reads)) + '</div>';

...
// OTHER tools' sessions (Claude Code / Cowork) into the same root — those must
// be skipped, never stamped 'Codex'. The discriminator is the FILE's own
// originator: an imported session carries the source tool's branding
// (Claude / Cowork). NOTE: history_mode "legacy" is NOT a foreignness signal —
// current Codex (>= ~0.144) stamps "legacy" on genuine native sessions too, so
// keying off it black-holed every recent Codex session (see
// scan.reopenMislabeledForeignCodex, which reverses that damage). Anything
// that fails the gate stays out of the feed here; if another adapter (Claude
// Code) owns the original transcript in its own root, it attributes it there.
  ...
  extractEvents(entries, fileState) {
    ...
    for (const e of entries) {
      ...
      if (e.type === 'response_item' && p.type === 'message' && p.role === 'user') {
        ...
        // token_count payloads never carry turn_id in practice, so the id
        // falls back to the timestamp -- but two genuinely different requests
        // can land on the same millisecond, and ledger.buildRequests dedupes
        // on this id, silently dropping the second one. usageSeq is a
        // per-file counter persisted on fileState (same pattern as cwd /
        // lastText / validated below) so the fallback id stays unique even
        // when timestamps collide.
        fileState.usageSeq = (fileState.usageSeq || 0) + 1;
        events.push({
          ts: e.timestamp, project: fileState.cwd, source: this.displayName,
          kind: 'usage',
          messageId: p.turn_id || (e.timestamp + '#' + fileState.usageSeq),
      ...
      if (text !== null && isRealPrompt(text)) {
        // Codex records ONE user turn TWICE: as a `response_item`/`message`
        // with role 'user' (the item sent to the model) and as an
        // `event_msg`/`user_message` (the UI event). Both branches above
        // match, so without this every Codex prompt was captured twice --
        // measured on a real rollout, 11 entries in, 2 identical prompt
        // events out for one message. It inflated prompt volume in the feed
        // and in search for every Codex user.
        //
        // Keyed on TIMESTAMP + text, not text alone: the two copies carry the
        // byte-identical timestamp (verified: both `2026-08-07T14:32:43.273Z`),
        // while a user genuinely sending the same message twice -- "yes",
        // "continue" -- gets a different one and must still count twice.
        // Deduping on text alone would silently swallow those.
        //
        // Held on fileState, like cwd/lastText/usageSeq, because the pair can
        // straddle two incremental reads: a local this.<var> would compare
        // against nothing on the read that picks up the second copy.
        //
        // A plain space separates the two halves: an ISO-8601 timestamp
        // cannot contain one, so no (ts, text) pair can collide with a
        // different one, and the source stays diffable text.
        const key = e.timestamp + ' ' + text.trim();
        if (fileState.lastPromptKey !== key) {
          fileState.lastPromptKey = key;

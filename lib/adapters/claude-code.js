...
module.exports = {
  ...
  extractEvents(entries, fileState) {
    ...
    for (const e of entries) {
      ...
      // A subagent (sidechain) record's token spend still counts toward the
      // ledger, but nothing else about a sidechain turn (reads, edits, todos,
      // prompt/summary extraction) is real project activity worth surfacing.
      if (e.isSidechain) {
        if (e.type === 'assistant' && e.message && e.message.usage) {
          const session = typeof e.sessionId === 'string' && e.sessionId ? e.sessionId : undefined;
          pushUsage(e, session, true);
        }
        continue;
      }

...
module.exports = {
  ...
  extractEvents(entries, fileState) {
    ...
    for (const e of entries) {
      ...
      // Sidechain records need it too: a subagent transcript is a SEPARATE
      // file (<session-id>/subagents/agent-<id>.jsonl), so the filename
      // fallback would invent a per-subagent session and detach its edits from
      // the session that delegated them. Every sidechain record carries the
      // parent sessionId, which is the one that must win.
      const session = typeof e.sessionId === 'string' && e.sessionId ? e.sessionId : undefined;
      // A subagent (sidechain) turn contributes its FILE EDITS and its token
      // spend, and nothing else. The edits are the point: a session that
      // delegates its implementation has no other record that those files were
      // touched, and dropping them both starves the Stop hook's worthiness
      // gate (lib/hooks.js counts kind:'edit' per session) and misattributes
      // the work in the feed.
      // Deliberately still excluded, and each for its own reason:
      //   prompts  - a subagent's prompt is the orchestrator's briefing, not
      //              something a human typed, and admitting it would bury the
      //              user's own words under instruction text.
      //   reads    - the read stream feeds the ledger's waste accounting,
      //              which measures ONE context being re-derived. A subagent
      //              request is a separate stream carrying its own context
      //              (see lib/ledger-fold-recall-ride.js, which already drops
      //              sidechain requests), so its reads are not that waste.
      //   todos,   - renderers keep only the LATEST snapshot per session, so a
      //   tasks      subagent's private checklist would overwrite the session's.
      //   summary  - the closing text is a subagent's report to its caller,
      //              and it would likewise clobber the session's own summary.
      if (e.isSidechain) {

...
// a minified bundle or a base64 blob is exactly that. The cost is a handful of
// extra tokens at the chunk seams of pathological input, which is a far better
// failure than a hook that hangs.
//
// 256 is measured, not picked: on a 200 KB unbroken punctuation run the caps
// 1024 / 512 / 256 / 128 cost 1514 / 784 / 399 / 209 ms and every one of them
// produced the IDENTICAL token count, so the smaller bound is free here. It is
// also free on ordinary source, which never comes close to the bound -- the
// longest single piece in MemBridge's own lib/ is 47 bytes, and the fixture
// corpus (which includes a 2000-byte punctuation run and a 4 KB base64 blob)
// still matches the reference implementation exactly at this cap.
const MAX_PIECE_BYTES = 256;

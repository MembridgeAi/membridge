...
function normalize(prev) {
  ...
  if (isUnmigrated(p)) {
    ...
    p = {
      // inCost/outCost deliberately dropped: USD is no longer persisted at
      // all (see foldProjectLedger's return in lib/ledger-fold.js), so the
      // migration must not carry a legacy ledger's stale dollar total across.
      requests: p.requests, volume: p.volume,
  ...
  return {
    ...
    volume: num(p.volume),
    // No inCost/outCost accumulator: a priced total is neither folded nor
    // written down any more, so reading one back off disk would only revive
    // a figure that has no consumer and a stale rate table behind it.

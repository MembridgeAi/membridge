...
// --- the controlled comparison (measured-savings spec, Tier 3) --------------
// The ONLY block an effect figure may ever be derived from (lib/roi.js). It is
// deliberately not `avoided`/`holdout`: those are cumulative lifetime
// bookkeeping that predate the clean cohort, they were never a controlled
// comparison, and they must keep accumulating across an epoch change because
// other things (the dashboard's lifetime avoidance, the net-negative
// auto-pause) legitimately depend on their continuity.
//
// Four values per arm, per project, exactly as the spec specifies:
//   reads   -- eligible reads in this arm (the unit of comparison)
//   tokens  -- sum of what those reads COST, in tokens
//   tokensSq-- sum of the squares of the same per-read costs
// Pooled means compose correctly from sums and counts, which is why sums and
// counts are all that is ever stored. tokensSq is the one addition the spec
// does not enumerate, and it is needed for a reason: a difference of means
// without a confidence interval is a number pretending to be a result, and an
// interval needs a second moment. It is an aggregate exactly like the others
// -- no per-read row, no path, no session id, nothing new about a user.
//
// EPOCH GATE. Evidence gathered under an older assignment scheme is not
// migrated, not converted and not counted: on a mismatch the whole block is
// zeroed at the current epoch, so there is no stale total left on disk for a
// future reader to sum with the new one. See lib/holdout-epoch.js for what
// changed and why old evidence cannot be retro-fitted. `epoch` is written on
// every ledger from here on, so an absent epoch (every ledger written before
// this change) is epoch 0 and resets on first fold, which is the correct
// outcome: those totals were gathered per read, from split sessions.
const zeroArm = () => ({ reads: 0, tokens: 0, tokensSq: 0 });

function normalizeArm(a) {
  const v = a && typeof a === 'object' ? a : {};
  return { reads: num(v.reads), tokens: num(v.tokens), tokensSq: num(v.tokensSq) };
}

function normalizeComparison(c) {
  const v = c && typeof c === 'object' ? c : {};
  const fresh = { epoch: HOLDOUT_EPOCH, served: zeroArm(), withheld: zeroArm() };
  if (v.epoch !== HOLDOUT_EPOCH) return fresh; // wrong epoch, or none: drop it
  return { epoch: HOLDOUT_EPOCH, served: normalizeArm(v.served), withheld: normalizeArm(v.withheld) };
}
...
function normalize(prev) {
  ...
  return {
    ...
    comparison: normalizeComparison(p.comparison),
  };
}

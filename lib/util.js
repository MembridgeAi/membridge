...
const DEFAULT_CONFIG = {
  ...
  // start uploading the verbatim prompts of every user who installed before
  // it — a privacy default must never change under someone by update.
  team: { url: '', anonKey: '' },
...
// whether the verbatim ask/goal rides along with a pushed entry (summaries,
// decisions, gotchas and file lists sync either way). A new install shares
// them, because a teammate row without one is frequently just a name and a
// timestamp: measured on live data, 0 of 21 rows pulled from one teammate
// carried an ask, and 13 of the 21 had no readable content at all. Putting it
// in DEFAULT_CONFIG instead would be a privacy break — getConfig deep-merges
// that object into every config already on disk, so the flag would flip for
// people who installed under the old off-by-default and never agreed to it.
// Either way it stays reversible: `membridge team share-prompts off`, or the
// per-session toggle in the app.
function freshInstallConfig() {
  return { ...DEFAULT_CONFIG, team: { ...DEFAULT_CONFIG.team, sharePrompts: true } };
}
...
function saveState(state) {
  ...
  // ORDER IS LOAD-BEARING, and both statements below guard a different
  // monotone fact. This one restores deletion watermarks (teamDeletedThroughTs)
  // from disk, mutating `state` IN PLACE, so a syncTeams pass holding a stale
  // in-memory copy cannot erase a deletion the user confirmed mid-pass. It must
  // run FIRST, because reconcile below reads the state it returns: putting it
  // after would hand the ledger a copy whose watermarks had not been corrected.
  ...
  // NB: `toWrite`, not `state`, is what gets serialised — persisting `state`
  // here would silently drop every revocation reconcile just put back (SEC-3).

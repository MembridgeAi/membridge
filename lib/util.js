...
const DEFAULT_CONFIG = {
  ...
  // Token-measurement plumbing (usage/read events) is capped SEPARATELY from
...
function saveUserConfig(raw) {
  ...
  // SECURITY: unconditional. This used to chmod only when the LEGACY
  // advisor.apiKey field was set, but the multi-provider path stores keys at
  // advisor.providers[<id>].apiKey and deletes the legacy field — so the guard
  // never fired and a file full of API keys sat at 0644. writeFileSync's mode
  // only applies when it creates the file, so an existing config needs this.

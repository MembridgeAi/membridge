...
function safePendingCut(lines, naiveCut) {
  ...
  let cut = naiveCut;
  // Tracks the furthest confirmation line found for any pair this call ends
  // up protecting -- the floor guard below needs it if `cut` is pinned at 0.
  let maxConfirmIdx = -1;
  for (let i = naiveCut; i < lines.length && droppedPending.size; i++) {
    ...
    if (pendingIdx !== undefined) {
      ...
      maxConfirmIdx = Math.max(maxConfirmIdx, i);
      droppedPending.delete(key);
    }
  }
  // FLOOR GUARD: the retreat above can pin `cut` at exactly 0 -- forced only
  // when line 0 ITSELF is a still-open pending row whose confirmation shows
  // up later in the kept tail. Left alone, that makes rotation a PERMANENT
  // no-op: the identical straddling pair sits at the identical position on
  // every future call, so a file already well past MAX_EVENTS_BYTES never
  // shrinks -- worst of all under the exact conditions FINDINGS 1/2 above
  // harden settlement against, where the fold's own commit() can't drain the
  // queue either. There is no pairing-safe cut greater than 0 that still
  // KEEPS this pair (any cut > 0 splits it), so guaranteeing progress here
  // means resolving it the other safe way: drop the pair TOGETHER (advance
  // past its confirmation) instead of keeping it, which still never orphans
  // one end without the other. This only fires when retreat produced zero
  // progress; any `cut > 0` already shrank the file and is left untouched --
  // pairing safety still wins whenever a pair genuinely straddles the floor
  // too (maxConfirmIdx tracks exactly the pairs that forced the retreat, so
  // jumping past them resolves precisely the ones blocking progress, no
  // others).
  if (cut === 0 && maxConfirmIdx >= 0) return maxConfirmIdx + 1;
  return cut;
}

...
function rankForHook(projectPath, proj, corpus, query) {
  ...
  try {
    ...
    // Both halves, same rule as lib/activity.js: the flag in state, and the
    // ledger that survives a state.json rebuild. The FTS index is built from
    // <homeDir> and is not rebuilt by a state discard either, so the flag
    // alone stops excluding a project the moment state.json is reset.
    const excludeTeamFor = (proj && proj.teamAccessLost) || revocationLedger.isRevoked(projectPath)
      ? [projectPath] : [];
...
function doRunSearch() {
  ...
  // CONSENT first, and it is not a feature toggle: this hook deliberately
  // ships with no switch of its own ("memory delivery is what MemBridge is"),
  // which is exactly why a removed or declined install kept intercepting every
  // Grep and Glob. Nothing above has any observable effect -- a stdin parse, a
  // state read and a project lookup -- so this is a true no-op.

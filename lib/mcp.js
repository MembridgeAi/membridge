...
// The per-project entry assembly (local + cached team rows, normalized) lives
// in lib/activity.js so lib/hooks-search.js can build the SAME entries without
// requiring this module, which loads the MCP SDK at module scope. See that
// module's header.
const { mapTeamRow, projectEntries } = require('./activity');
...
// mapTeamRow (imported from lib/activity.js above) passes through everything
// normalizeTeam knows how to carry: the stored row shape is pullProject's
// `mapped` (lib/teamsync.js), and archive rows share that exact shape
// (team-archive.js), so an omission there silently blinds search and activity
// to that field. It is imported rather than redefined here so the archive
// reader below and the per-project assembly agree field for field.
...
// allActivity used to call it fresh on every single search_memory for every
...
// session. This process is a long-lived stdio server (one `membridge mcp`
// invocation serves many tool calls), so the parsed-AND-NORMALIZED entries
// can be memoized here for the process's lifetime instead of recomputed
// every call.
//
...
// no credentials), so the MCP no-network-call invariant holds. Only
// searchMemory opts in: get_recent_activity and get_project_memory must
// keep their exact current payload size, so they call allActivity with no
// opts and never touch the archive.
function allActivity(state, config, regexes, opts = {}) {
  const local = [];
  const team = [];
  for (const [key, proj] of trackedProjectEntries(state, config)) {
    const name = path.basename(key);
    // Local entries plus this project's cached team rows, both already
    // normalized and redacted (lib/activity.js). The teamAccessLost guard
    // lives in there; it is repeated below only to gate the ARCHIVE, which
    // this call deliberately does not include.
    ...
    if (opts.includeArchive) {
      ...
      if (link && link.projectId) {
        ...
        // same author/ts/source values as the raw row here (mapTeamRow never
        // passes a selfUserId at this call site, so `self` — the only thing
        // that could change `author` — is always false).
        const seen = new Set(util.teamRowsFor(proj).map(e => `${e.author}|${e.ts}|${e.source}`));
        for (const entry of loadArchiveEntries(link.projectId, name, regexes)) {
          if (seen.has(`${entry.author}|${entry.ts}|${entry.source}`)) continue;
          team.push(entry);
        }
      }
    }
...
function searchMemory(args) {
  ...
  // The strict-then-relaxed two-pass policy (natural-language fallback) now
  // lives in lib/search.js as rankWithFallback, because the PreToolUse
  // Grep/Glob hook needs the same one. See rankWithFallback's own comment.
  const ranked = searchLib.rankWithFallback(filtered, args.query);

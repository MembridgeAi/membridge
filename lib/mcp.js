...
function registerTools(server) {
  ...
  server.registerTool('get_recent_activity', {
    ...
    description: "Newest-first AI activity across every tracked, non-paused project, combining local sessions and cached teammate activity. THIS IS THE TOOL for \"what is <teammate> working on\" or \"what has anyone touched today\" — pass `author` to narrow to one person (name substring, e.g. \"andrew\"); search_memory ranks by relevance with no recency weighting and cannot answer it.\n\nAnswer from the NEWEST entry, and say how fresh it is rather than claiming the present tense: every entry carries `lastActivityAt`, so \"Andrew was on the invite redemption path, as of about 4 minutes ago\" is right and \"Andrew is working on X right now\" is not. If the newest entry is hours or days old, say that plainly — it means they have not synced since, not that they are idle.\n\nThis is a LOCAL read: teammate rows are what team sync has already pulled onto this machine, so there is always some lag and a teammate who started seconds ago may not appear at all. Each entry carries `live` plus `liveBasis` saying what that flag rests on: 'session-events' (your own session, judged on its newest real event) or 'synced-row' (a teammate, judged on when their row reached the backend). A `synced-row` live flag means a row arrived recently, which is evidence of recent work but never proof they are at the keyboard now — do not report it as presence.",
    inputSchema: {

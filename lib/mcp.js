...
// read (loadArchive is a plain fs read, teamsync.loadTeamLink just parses
// .membridge/team.json — no network call, no credentials), so the MCP
// no-network-call invariant holds. Only searchMemory opts in: get_recent_activity
// and get_project_memory must keep their exact current payload size, so they
// call allActivity with no opts and never touch the archive.
function allActivity(state, config, regexes, opts = {}) {
  ...
  for (const [key, proj] of trackedProjectEntries(state, config)) {
    ...
    const toTeamRow = e => feed.normalizeTeam({
      // Pass through everything normalizeTeam knows how to carry — the
      // stored row shape is pullProject's `mapped` (lib/teamsync.js), and
      // archive rows share that exact shape (team-archive.js). An omission
      // here silently blinds search and activity to that field.
    ...
    for (const e of proj.teamEntries || []) team.push(toTeamRow(e));
    if (opts.includeArchive) {
      ...
      if (link && link.projectId) {
        ...
        // as fresh as the archive for a row it still holds.

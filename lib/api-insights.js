...
async function insightsPayload(config, windowDays, savings) {
  ...
  return {
    window: windowDays,
    // Every comparison below is suppressed when the fetch was truncated, for
    // the reason in fetchSince: the cap eats the OLDEST rows, so the prior
    // window empties out before the current one does and the delta reads as
    // growth that did not happen. Observed live at a 30d window: 2000 rows
    // fetched, every one of them inside the current window, prior therefore
    // empty, and the stat published "+2000". Nulling here extends the rule
    // pctDelta already follows -- never fabricate a comparison from a
    // baseline that was never actually read.
    //
    // `truncated` also makes each `count` a FLOOR rather than a total: the
    // cap is reached before the window's oldest row, so there are current-
    // window rows that were never fetched either. perPerson, topProjects and
    // byTool are undercounts on the same basis.
    truncated,
    sessions: {
    ...
    entriesShared: { count: current.length, delta: truncated ? null : current.length - prior.length },
    skeleton: skeletonStatsFrom(savings),
    assists: assistsFrom(savings),
    perPerson,
    topProjects,

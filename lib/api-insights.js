...
// an input-cost figure, not an assist count); mcpQueries = how many memory
// MCP tools (lib/mcp-usage.js MCP_TOOLS) have ever recorded a use -- a
// best-effort last-used tally, not a call counter, so this is a lower
// bound, and degrades to 0 (never a crash) with no tally file yet. Rejected:
// holdout.skips (measures what a call WOULD have cost, not a delivered
// instance) and the always-on context block (no per-instance signal
// exists). `available: false` only when `notesInjections` is absent from
...
function assistsFrom(savings) {
  ...
  const mcpQueries = mcpUsage.toolsUsedWithin(Infinity).length;
  return {
    available: true,
    total: recallServed + teammateNotes + mcpQueries,
    byKind: { recallServed, teammateNotes, mcpQueries },
  };
}
...
async function insightsPayload(config, windowDays, savings) {
  ...
  const rows = await fetchSince(config, team.team_id, new Date(priorStart).toISOString());
  ...
  const lookbackDays = windowDays * 2; // rows spans priorStart..now, i.e. two windows
...
module.exports = { insightsPayload, severityOf, assistsFrom };

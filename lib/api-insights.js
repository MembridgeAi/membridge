...
async function insightsPayload(config, windowDays, savings, teamId) {
  ...
  //
  // fetchSince itself is still a SEQUENTIAL cursor walk, and that is still the
  // dominant cost (~10 round trips). Making it concurrent is #70 and is not this
  // change; see that ticket for why the obvious range-partition is not safe.
  const [rawMembers, exactCurrent, exactPrior, fetched] = await Promise.all([
  ...
  // Concentration is built here so `perPerson` / `topProjects` are contiguous,
  // but the sentence in `.detail` reads `lookbackDays` below rather than
  // windowDays — a truncated fetch has NOT exhaustively searched the requested
  // range, and "only X has worked on this project in the last 30d" on a fetch
  // that reached 12 days back is the same overstatement `silentTeammateProblems`
  // exists to avoid, about a NAMED person and a NAMED project. Bound to what was
  // really searched, exactly as that sentence is.
  ...
  // How far back `rows` REALLY reaches, in TWO forms, because they mean
  // different things and the UI needs both:
  //
  //   * lookbackDays is the FLOOR of the paged fetch — the "in the last Nd"
  //     the silent-teammate sentence quotes, so a truncated fetch is not
  //     phrased as an unbounded "never".
  //   * coveredDays is the same figure a caller can trust regardless of the
  //     truncation state: on a complete fetch it is windowDays * 2, on a
  //     truncated fetch it is the same lookbackDays. Serialising both lets
  //     the UI say "30 days requested, 12 days reached" without deriving it
  //     from `truncated + window * 2`, which is exactly the arithmetic the
  //     client should not have to do.
  const oldest = oldestTs(rows);
  const lookbackDays = truncated && oldest !== null
    ? Math.max(1, Math.floor((now - oldest) / DAY_MS))
    : windowDays * 2;
  const coveredDays = lookbackDays; // one name for one number; both are exposed
  const concentration = projectGroups
    .filter(g => g.people.size === 1)
    .map(g => {
      const onlyPerson = [...g.people.values()][0] || 'Unknown';
      return { projectName: g.name, onlyPerson, detail: `only ${onlyPerson} has worked on this project in the last ${lookbackDays}d` };
    });
  ...
  return {
    ...
    // `truncated` used to be masked by `!exact`. Wrong: the two flags describe
    // DIFFERENT things and one silenced the other. `exact` covers the
    // TWO HEADLINE COUNTS (sessions, entries), which 027 counts in the database;
    // `truncated` covers the FEED FETCH, whose 10-page cap still limits every
    // breakdown built from those pages (perPerson, topProjects, byTool,
    // concentration, membersSyncing, problems). With 027 applied on production,
    // `exact` is always true, so the old expression made `truncated` always false
    // on the wire — including when the fetch really did stop at the cap. The
    // client's UI honesty layer sat behind that mask and never fired. Sent raw
    // now, so the client can label the breakdowns as capped even when the
    // headline counts are exact.
    truncated: Boolean(truncated),
    // How far back the fetch actually reached, in DAYS. `lookbackDays` is what
    // the silent-teammate and concentration sentences quote — the caller may
    // read the same number without re-deriving it — and `coveredDays` is the
    // same figure named to say what a UI would say: "30 days requested, N days
    // reached." On a complete fetch both equal windowDays * 2.
    lookbackDays,
    coveredDays,

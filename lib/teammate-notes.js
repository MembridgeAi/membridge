...
function buildIndex(entries, prev, now) {
  ...
  for (const row of Array.isArray(entries) ? entries : []) {
    ...
    // TWO SHAPES REACH THIS FUNCTION. A decrypted wire row carries
    // `author_name`; the rows lib/teamsync.js's pull stores in
    // state.projects[].teamEntries -- the only thing the daemon actually has in
    // hand at rebuild time -- rename it to `author`. Reading just one of them
    // does not fail loudly: every note comes out attributed to "a teammate",
    // AND every id changes, because the author is part of noteId.
    const author = str(row.author_name) || str(row.author) || 'a teammate';
    const ts = str(row.ts);
    if (!ts) continue;
...
// Spec §6: the human surface. Task 1 proved the in-terminal line impossible,
// so the dashboard -- which already polls every 5 seconds -- is where a human
// learns about a teammate decision without waiting for their agent to act.
//
// DELIBERATELY INDEPENDENT OF `seen`. Being shown a decision in a browser is
// not the same event as an agent having consumed it, and conflating the two
// would let a browser visit silently suppress an agent-side delivery. This
// function therefore reads `prose` and NOTHING under `seen`, and -- being
// pure -- writes nothing back.
//
// `now` is accepted and unused, and that is the design, not an oversight:
// there is no age filter here. Spec §5's "absence is not a reason to lose
// information" means an old decision still shows; the entry carries `ts` so
// the card can say WHEN rather than implying it just landed. The parameter
// stays because every function in this module takes an injected clock, and a
// caller should not have to know which ones happen not to need it yet.
function dashboardPayload(index, now) { // eslint-disable-line no-unused-vars
  const ix = index || emptyIndex();
  const all = Array.isArray(ix.prose) ? ix.prose : [];
  // Project to the three fields the card draws. The internal id and kind are
  // index bookkeeping and have no business in a browser payload.
  return {
    fresh: all.slice(0, PROSE_CAP).map(p => ({ author: p.author, ts: p.ts, text: p.text })),
    total: all.length,
  };
}

// The kill switch. Mirrors the shape lib/util.js's isProjectOff and the
// distill/recall config flags already use: ONLY the literal `false` disables.
// An absent, empty or malformed config must never silently switch the feature
// off -- a user who never opted out has opted in, and a typo in settings.json
// should not quietly cost them their teammates' decisions.
...
module.exports = {
  ...
  formatProse, formatFileNotes, noteId, isNotesEnabled, dashboardPayload,
};

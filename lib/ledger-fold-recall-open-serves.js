...
function buildOpenServe(pending, net) {
  return {
...
// into 'negative' this pass (at most once per serve -- see the
// classification-transition check below).
//
...
// strictly later than the serve's own ts, and not already applied
// (correctionKeyFor not yet in that serve's correctedBy). Multiple matching
// reads in one pass all apply, in order, and accumulate (spec: two
// follow-ups of 300 and 400 must net followTokens=700, not just the last
// one seen).
function applyCorrections(openServes, readEvents, projectPath, now) {
  ...
  for (const serve of openServes) {
    ...
    for (const e of reads) {
      ...
      if (!(String(e.ts) > String(current.ts))) continue;
      ...
      if (current.classified !== 'negative' && newClass === 'negative') {
        rejections.push(current.relPath); // at most once per serve -- classification only moves INTO 'negative' once (net is non-increasing as followTokens only grows)
      }

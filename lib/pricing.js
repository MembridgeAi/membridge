...
const TABLE = {
  ...
  unknown: { cacheWrite5mMult: 0, cacheWrite1hMult: 0, models: {}, fallback: [1.25, 0.125, 10.0] },
};
...
function priceOf(model, provider) {
  ...
  return {
    inPerMTok, cachedInPerMTok, outPerMTok,
    cacheWrite5mMult: p.cacheWrite5mMult, cacheWrite1hMult: p.cacheWrite1hMult,
  };
}
...
function requestCostUsd(norm, model, provider) {
  ...
  const inCost = ((n.input || 0) * p.inPerMTok
    ...
    + (n.cacheWrite5m || 0) * p.inPerMTok * p.cacheWrite5mMult
    + (n.cacheWrite1h || 0) * p.inPerMTok * p.cacheWrite1hMult) / 1e6;

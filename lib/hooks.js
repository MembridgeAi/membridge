...
function isHookInstalled() {
  try {
    ...
    // BOTH halves must be there. Checking only the runtime was a hole big
    // enough to swallow the whole check: the runtime is usually plain `node`,
    // which always resolves, so a registration pointing at a deleted
    // membridge-hook.js reported "installed" while every stop silently did
    // nothing -- see installKind, and the test pinning this.
    return membridgeCommands(settings).some(c => {
      if (!executableResolves(commandExecutable(c))) return false;
      const script = hookScriptOf(c);

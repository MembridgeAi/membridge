...
// A session's checkpoint state must follow the SESSION, not whichever project
// is currently winning its edit vote. sessionDominantRoot counts kind:'edit',
// so a session that delegates heavily can flip roots mid-flight: subagent
// edits landing in one repo can outvote the operator's own edits in another.
// When the winner moves, a per-project lookup finds no prior line, reports
// "never checkpointed", and isCheckpointDue fires on the very next Stop with
// minEdits instead of checkpointEvery. The agent is then told it supersedes
// zero earlier lines when it supersedes several, and writes a fresh roll-up
// instead of a superseding one.
//
// Both readers below therefore take EVERY tracked root and answer for the
// session across all of them. A session that legitimately spans two repos is
// one session with one checkpoint history, which is what the agent is being
// asked about.
function everyTrackedRoot(tracked, key) {
  const roots = tracked instanceof Set ? [...tracked] : Array.isArray(tracked) ? tracked : [];
  return roots.includes(key) ? roots : [key, ...roots];
}

function countSummaryLinesAcross(tracked, key, sessionId) {
  let n = 0;
  for (const root of everyTrackedRoot(tracked, key)) n += countSummaryLines(root, sessionId);
  return n;
}

function lastSummaryTsAcross(tracked, key, sessionId) {
  let newest = null;
  for (const root of everyTrackedRoot(tracked, key)) {
    const t = lastSummaryTs(root, sessionId);
...
function runStop() {
  try {
    ...
    const n = countSummaryLinesAcross(tracked, key, sessionId);
    if (!isCheckpointDue({ editTs, lastCheckpointTs: lastSummaryTsAcross(tracked, key, sessionId), minEdits, every })) return;

...
// The Stop hook's flight recorder: why each silent allow was silent, and
// whether a block was ever honoured. Required eagerly because runStop's very
// first statement uses it (a start line must be written before anything can
// kill this process); it pulls in nothing but fs/path/util.
const hookStops = require('./hook-stops');
// Whether this user has said MemBridge may write hooks at all, and whether the
// hooks already on disk may do anything. See lib/hook-consent.js: it is the
// record that was missing, and every writer and every hook body below consults
// it. Required eagerly and cheap by design (util only), so the hot Stop path
// pays a config read it was making anyway.
...
// session page renders as ONE bulleted list (ui/.../distill.ts whatGroups).
//
...
// Longest a label may be, measured AFTER internal whitespace is collapsed.
// Collapsing first is what keeps this parser and the UI's parseAreaPoint from
// disagreeing: the UI flattens a point to one line before matching, so a label
// written as "[a<20 spaces>b]" reaches it as "[a b]". Bounding the raw string
// here would have classified that same input as no-prefix on this side only.
const AREA_LABEL_MAX = 20;

// Read a leading "[Area] " off a point. Returns the area (null when absent)
// and the point without it.
//
// MIRRORED BY ui/src/features/session/distill.ts parseAreaPoint. The two run on
// the same rows -- the hook validates what an agent writes, the UI renders what
// any tool ever wrote -- so any rule that differs between them shows up as a
// point that was accepted one way and displayed another. Three rules, all
// shared, all there to stop a teammate's text being silently eaten:
//
//   * ANCHORED AT THE START, because a point legitimately contains brackets
//     mid-sentence ("Fixed the [object Object] label").
//   * The "]" must be followed by whitespace or end the string. A prefix is
//     written "[Tests] note"; "[clipWords](src/mappers.ts) now trims" is a
//     sentence that happens to open with a bracket, and stripping its label
//     would leave the reader a line starting "(src/mappers.ts)". Plenty of rows
//     never passed through this hook at all -- older installs, the Codex
//     adapter, anything synced before areas existed -- so the renderer meets
//     that shape for real.
//   * A blank label ("[   ] x") is NOT a prefix. It names no area, and treating
//     it as one gives the UI an empty heading that still counts toward its
//     grouping threshold.
function splitAreaPrefix(line) {
  const raw = String(line == null ? '' : line).trim();
  const m = /^\[([^\][\n]*)\](?=\s|$)\s*(.*)$/.exec(raw);
  if (!m) return { area: null, text: raw };
  const area = m[1].replace(/\s+/g, ' ').trim();
  if (!area || area.length > AREA_LABEL_MAX) return { area: null, text: raw };
  return { area, text: m[2].trim() };
}
...
function blockReason(target, sessionId, n) {
  ...
  // A superseding line is the session's ROLL-UP, and it answers a different
  // question than an early checkpoint does. Asking "what did you do" of a long
  // session yields everything, ordered by when it happened; asking what will
  // still matter yields the three or four things a teammate actually needs.
  // Narrowing it loses no history: the incremental trail is already rendered
  // separately from the checkpoint list (ui/.../session/distill.ts
  // distilledBullets), so this field is free to be the durable summary instead
  // of a second copy of that trail.
  const bulletAsk = n > 0
    ? 'the 3 to 5 things a teammate will still need to know about this session next week, most important first, never everything that happened'
  ...
  return 'MemBridge is working normally — this is not an error. MemBridge pauses the stop so this summary gets written. ' +
    ...
    `decisions: ${bulletAsk}, as SHORT BULLETS, one per line, at most ${POINTS_MAX} lines of at most ${POINT_MAX} characters each — each line ONE completed piece of work or ONE choice made, written as the outcome a teammate can act on, never as a description of your process. Like "Removed the transfer-ownership menu item, no RPC can perform it", NOT like "Ran four agent waves in worktrees, verified each branch, then integrated and pushed". Never a paragraph, never a restatement of file names or tool output. ` +
    `Start each line with the part of the codebase it belongs to, in square brackets — one of: ${AREAS.join(', ')} — like "[UI/UX] Removed the transfer-ownership menu item, no RPC can perform it". The bracket does not count against the character budget; omit it only if no area fits, or ""; ` +
    `gotchas: surprises or pitfalls hit, same shape — SHORT BULLETS, one per line, at most ${POINTS_MAX} lines of at most ${POINT_MAX} characters each, written so a teammate does not hit them again. ` +
    `Start each line with the part of the codebase it belongs to, in square brackets — one of: ${AREAS.join(', ')} — like "[UI/UX] Removed the transfer-ownership menu item, no RPC can perform it". The bracket does not count against the character budget; omit it only if no area fits, or ""; ` +
...
function runAppend(argv) {
  ...
  // VERIFICATION GROUNDWORK (additive, non-blocking). Ground the summary
  // against observable state — the daemon's edit ledger and the files on
  // disk — and stamp the result. See lib/hook-verify.js for why, and for why
  // it is deliberately narrow.
  //
  // Three properties this must keep, because Andrew's live summaries and his
  // teammates' feeds run through this exact line:
  //   1. ADDITIVE ONLY. Two new keys; no existing field changes meaning, and
  //      every summaries.jsonl line already on disk still parses (scan.js
  //      reads named fields and ignores the rest).
  //   2. NEVER BLOCKS THE WRITE. A `contradicted` line is still written. This
  //      records a fact about the claim; it does not adjudicate it. Refusing
  //      the write would be a product decision the owner has not made.
  //   3. NEVER FAILS THE APPEND. Any error in the verifier costs a stamp, not
  //      a summary.
  const projectPath = path.dirname(path.dirname(resolved));
  let verification = null;
  try {
    const health = hookStops.daemonHealth(util.getConfig());
    verification = require('./hook-verify').verifySummary({
      projectPath, entry: e, state: util.loadState(), health,
    });
    e.verification = verification.status;
    e.verificationChecks = verification.checks;
  } catch { /* unstamped is honest; a broken verifier must not cost a summary */ }
  try {
  ...
  // The last-successful-capture signal. Written HERE, by the process that
  // actually wrote the line: the Stop hook asks and exits, so it can never
  // learn whether its ask was honoured. Best-effort, after the real write.
  try {
    hookStops.capture({
      session: e.session,
      project: projectPath,
      verification: verification ? verification.status : null,
      chars: (e.did || '').length,
    });
  } catch {}
}
...
//
// Every exit path is RECORDED (lib/hook-stops.js) as well as taken. The
// behaviour is unchanged — still fail-open, still silent on stdout, still at
// most one block per stop cycle — but the reason for each silent allow is now
// written down, because the five silent allows below are NOT the same event
// and treating them as one hid a total capture outage behind a healthy-looking
// rc=0. See lib/hook-stops.js for the measurement that motivated this.
function runStop() {
  const rec = hookStops.begin('stop');
  let done = false;
  const finish = (outcome, extra) => { done = true; hookStops.end(rec, outcome, extra); };
  try {
    let payload;
    try {
      payload = JSON.parse(fs.readFileSync(0, 'utf8'));
    } catch {
      return finish(hookStops.OUTCOME.NO_PAYLOAD); // no/garbled payload: not a real hook invocation, allow
    }
    if (!payload || typeof payload !== 'object') return finish(hookStops.OUTCOME.NO_PAYLOAD);
    // NEVER block twice: a prior block already asked for the summary.
    if (payload.stop_hook_active === true) return finish(hookStops.OUTCOME.LOOP_GUARD);

    const config = util.getConfig();
    const distill = config.distill || {};
    // Consent, BEFORE the feature switch. These are two different questions and
    // this hook only ever asked the second one: `distill.enabled` is "do I want
    // summaries", and applyConsent('declined') never touched it -- so a user who
    // answered "Not now" kept getting their session ends blocked by a hook they
    // had just been told was not installed. An explicit no stops the body here,
    // whatever the feature switch says.
    if (hookConsent.declined(config)) return finish(hookStops.OUTCOME.DECLINED);
    if (distill.enabled === false) return finish(hookStops.OUTCOME.DISABLED);

    const cwd = typeof payload.cwd === 'string' && payload.cwd ? payload.cwd : null;
    const sessionId = typeof payload.session_id === 'string' ? payload.session_id : null;
    if (!cwd || !sessionId) return finish(hookStops.OUTCOME.NO_PAYLOAD);

    // Read BEFORE loadState so an unreadable state.json (which loadState
    // throws on) still lands in the error record with its cause attached.
    const health = hookStops.daemonHealth(config);
    ...
    //
    // Filtered to THIS SESSION'S EDITS while collecting, rather than copying
    // every event of every project into one array. sessionDominantRoot ignores
    // everything else (it skips any event that is not a kind:'edit' for this
    // session), so the answer is identical — but the array it builds is a
    // handful of entries instead of the whole history. Measured on the
    // author's machine at the time: 4119 events across 2 projects in a 4.2MB
    // state.json, on a hook with a 10s budget. Parsing state.json is still
    // O(all history) and is the remaining latency trend; this removes the
    // avoidable half of it.
    const sessionEditsByProject = new Map();
    const sessionEdits = [];
    for (const pk of Object.keys(state.projects || {})) {
      for (const e of state.projects[pk].events || []) {
        ...
        sessionEdits.push(e);
        const list = sessionEditsByProject.get(pk);
        if (list) list.push(e);
        else sessionEditsByProject.set(pk, [e]);
      }
    }
    ...
    let key = projectResolve.sessionDominantRoot(sessionEdits, sessionId, tracked);
    key = (key && scan.findProjectKey(state, key)) || scan.findProjectKey(state, cwd);
    if (!key) return finish(hookStops.OUTCOME.UNTRACKED, { session: sessionId, cwd, health });
    if (util.isProjectOff(key, config)) return finish(hookStops.OUTCOME.PAUSED, { session: sessionId, project: key });
    ...
    const editTs = (sessionEditsByProject.get(key) || []).map(e => e.ts);
    ...
    if (!isCheckpointDue({ editTs, lastCheckpointTs: lastSummaryTs(key, sessionId), minEdits, every })) {
      // THE distinction this instrumentation exists for. All three branches
      // behave identically (silent allow, exit 0) and mean completely
      // different things:
      //   not-due            edits landed, the next checkpoint is not due yet
      //   no-edits           the daemon is healthy and this session changed nothing
      //   no-edits-daemon-down  MemBridge CANNOT KNOW: the ledger it reads is
      //                      stale or absent, so a silent allow here is a
      //                      capture outage wearing a healthy hook's clothes.
      const outcome = editTs.length
        ? hookStops.OUTCOME.NOT_DUE
        : (hookStops.cannotKnow(health) ? hookStops.OUTCOME.NO_EDITS_DAEMON_DOWN : hookStops.OUTCOME.NO_EDITS);
      return finish(outcome, { session: sessionId, project: key, edits: editTs.length, lines: n, health });
    }
    process.stdout.write(JSON.stringify({ decision: 'block', reason: blockReason(summariesPath(key), sessionId, n) }) + '\n');
    // Recorded with the line count AT BLOCK TIME so a later capture record for
    // the same session can be matched against it: a block with no capture
    // after it is the model ignoring the ask, which was previously invisible
    // (the append happens in another process, after this one has exited).
    finish(hookStops.OUTCOME.BLOCKED, { session: sessionId, project: key, edits: editTs.length, lines: n });
  } catch (err) {
    ...
    if (!done) {
      try { hookStops.end(rec, hookStops.OUTCOME.ERROR, { error: String((err && err.message) || err) }); } catch {}
    }
  }
}
...
// Split a registered command into the part that is OURS and any prefix in
// front of it.
//
// Every command we write has the shape
//   [ELECTRON_RUN_AS_NODE=1 ]"<runtime>" "<script>"[ <subcommand>]
// but a real registration may be WRAPPED — on the author's own machine every
// one of the four is:
//   ~/.membridge/hook-exec-wrapper.sh <label> env ELECTRON_RUN_AS_NODE=1 "<runtime>" "<script>" [sub]
// That wrapper is the entire observability layer for hook execution ("the hook
// never ran" vs "the hook ran and said nothing"), it is a hand-installed
// artifact no code in this repo generates, and reconcile used to replace the
// WHOLE command string on every upgrade — silently deleting it. Splitting the
// command lets an upgrade rewrite only the part it owns and put the user's
// prefix back verbatim.
//
// Anchored on the script token and walked BACKWARDS (runtime, then any env
// assignments, then a bare `env`), so it does not care what the prefix is or
// how many arguments it takes. A command with no recognisable script token
// splits to an empty prefix and is left to the caller.
function splitHookCommand(command) {
  const raw = String(command || '');
  const tokens = raw.match(/"[^"]*"|\S+/g) || [];
  const unquote = t => t.replace(/^"|"$/g, '');
  const scriptIdx = tokens.findIndex(t => /(^|[/\\])membridge-hook\.js$/.test(unquote(t)));
  if (scriptIdx < 1) return { prefix: '', ours: raw, script: scriptIdx === 0 ? unquote(tokens[0]) : null };
  let start = scriptIdx - 1; // the runtime binary
  while (start > 0 && /^[A-Za-z_][A-Za-z0-9_]*=/.test(tokens[start - 1])) start--; // VAR=value
  if (start > 0 && tokens[start - 1] === 'env') start--;                            // the `env` shim
  return {
    prefix: tokens.slice(0, start).join(' '),
    ours: tokens.slice(start).join(' '),
    script: unquote(tokens[scriptIdx]),
  };
}

// Re-apply an existing registration's prefix to a freshly-built command.
const withPrefix = (existingCommand, command) => {
  const { prefix } = splitHookCommand(existingCommand);
  return prefix ? `${prefix} ${command}` : command;
};

// Is this registration already pointing at the same MemBridge as `command`,
// ignoring any wrapper in front of it? Compared on the OURS half so a wrapped
// entry is not rewritten (and its prefix churned) on every single launch.
const sameOurs = (existingCommand, command) => splitHookCommand(existingCommand).ours === command;
...
// ---------------------------------------------------------------------------
// CONSENT GATES (#48, #49). Two of them, deliberately different strengths.
//
// optedOut() — an explicit NO, and it stops EVERY writer below including the
// user-initiated ones (a force-update from the dashboard must not quietly
// undo `remove-hooks`). It is the mirror of mcp-register's recorded
// 'unregister': `membridge setup-hooks` is the way back in, and it is the only
// way back in, because it records the grant before it writes.
//
// installConsent() — the launch path's gate, and it is strictly stronger: it
// demands an affirmative grant, so an install nobody has been asked about
// writes nothing at all. That is #48's first half; the dialog in app/main.js
// can now truthfully say "installs a Claude Code hook" because nothing is
// installed until it is answered.
//
// The reason the reconcilers get the WEAKER gate and ensureInstalled the
// stronger one: the reconcilers are also the machinery behind setup-hooks, and
// a gate that demanded a grant inside them would make the grant impossible to
// act on. Gating the launch path once, above all four calls, plus every
// reconciler individually on the opt-out, leaves no door open in either
// direction.
function optedOut() {
  try { return hookConsent.declined(); } catch { return false; } // unreadable config: reconcile as before, never a silent stand-down
}

// Is ANY MemBridge-owned hook entry registered in settings.json? Presence
// only, deliberately -- this is not isHookInstalled.
//
// isHookInstalled additionally requires the command to still RESOLVE, which is
// the right question for "is the hook working" and the wrong one here. The
// commonest legacy install is precisely one whose command no longer resolves:
// the app moved, npm relinked, the checkout was a worktree that is now gone.
// Asking the liveness question would read a broken-but-consented install as
// "never installed", stand down, and leave the one machine that most needs a
// repair reconcile without one -- the exact repair ensureInstalled exists for.
function anyOwnedHookRegistered() {
  const { settings, existed } = readSettings(claudeSettingsPath());
  if (!existed) return false;
  const events = settings.hooks || {};
  const owns = (list, isOurs) => (Array.isArray(list) ? list : []).some(entry => {
    const inner = entry && Array.isArray(entry.hooks) ? entry.hooks : [entry];
    return inner.some(h => h && isOurs(h));
  });
  return owns(events.Stop, isOwnStopHook)
    || owns(events.PreToolUse, isOwnRecallHook)
    || owns(events.PreToolUse, isOwnSearchHook)
    || NOTES_HOOKS.some(spec => owns(events[spec.event], h => isOwnNotesHook(h)));
}

// The shape a refused reconcile returns. `wrote: false` is what every existing
// caller reads; `optedOut` is what a caller that cares can branch on. NOT named
// `declined` — reconcileStopHook already returns a `declined` count meaning
// "entries I refused to downgrade", and merging those two meanings is how a
// force-update ends up reporting the wrong reason.
const consentRefused = extra => ({ optedOut: true, wrote: false, file: claudeSettingsPath(), ...extra });

// The launch path's answer: 'granted', 'declined' or 'unknown'.
//
// The one thing it does beyond reading the record is the migration that keeps
// existing installs alive. A machine that has been running MemBridge for
// months may have no record at all — `config.hooks` did not exist, and a CLI
// user who ran `membridge setup-hooks` never saw the dialog that writes
// `distill.consent`. For them the hooks in settings.json ARE the record of a
// decision: they were installed by an explicit command, they have been running
// ever since, and standing down would break a working install over
// bookkeeping. So: probe the settings file once, record the inferred grant
// (stamped `via: 'pre-existing-install'` so it is never mistaken for an
// answered question), and never probe again.
//
// It cannot mis-fire forward: under this file's own rules nothing can put a
// MemBridge hook into settings.json without a grant, so "installed but no
// record" can only mean an install that predates the record.
function installConsent() {
  let state;
  try { state = hookConsent.state(); } catch { return hookConsent.UNKNOWN; }
  if (state !== hookConsent.UNKNOWN) return state;
  let installed = false;
  try { installed = anyOwnedHookRegistered(); } catch { return hookConsent.UNKNOWN; }
  if (!installed) return hookConsent.UNKNOWN; // fresh install: ASK first, write nothing
  try { hookConsent.record(hookConsent.GRANTED, 'pre-existing-install'); } catch { /* unwritable config: infer it again next launch */ }
  return hookConsent.GRANTED;
}
...
function reconcileStopHook() {
  if (optedOut()) return consentRefused({ command: hookCommand(), current: false, upgraded: 0, deduped: 0, yielded: 0, declined: 0, addedAllow: false });
  ...
  for (const entry of stop) {
    ...
    for (const h of entry.hooks) {
      ...
      seenOwned = true;
      // "Current" ignores any wrapper in front of our command (see
      // splitHookCommand): a registration routed through the user's
      // hook-exec-wrapper.sh is pointing at exactly this build and must not be
      // rewritten -- and rewriting it every launch is what deleted the wrapper.
      if (sameOurs(h.command, command) && h.membridgeFingerprint === fingerprint) {
        current = true;
        inner.push(h);
      } else if (standDownReason(h) === 'newer') {
        // A STRICTLY NEWER MemBridge wrote this. Rewriting it would point the
        // user's only Stop hook at an OLDER build -- observed live: a 0.2.8 dev
        // checkout silently taking the registration off the installed 0.3.2 app
        // on every launch, because ensureInstalled() calls this unconditionally.
        // A downgrade is never a reconcile.
        declined++;
        inner.push(h);
      } else if (standDownReason(h) === 'yield') {
        ...
        upgraded++;
        // withPrefix, never a bare `command`: the prefix is the user's, not
        // ours to drop. On this machine it is hook-exec-wrapper.sh, the entire
        // "did the hook run at all" observability layer, and no code in this
        // repo would ever put it back.
        inner.push({ ...h, command: withPrefix(h.command, command), membridgeFingerprint: fingerprint });
      }
    }
  ...
  // Yielding and declining both count as "nothing to do" alongside
  // already-current: we chose to leave their command in place, so there is
  // nothing to write on its account.
  if ((current || yielded || declined) && !upgraded && !deduped && !newAllow) {
    return { file, command, current, upgraded, deduped, yielded, declined, addedAllow: false, wrote: false };
  }
  ...
  return { file, command, current, upgraded, deduped, yielded, declined, addedAllow: !!newAllow, wrote: true };
}
...
function reconcilePreToolUseHook({ command, isOurs, matcher, timeout }) {
  // Both PreToolUse hooks come through here, so one gate covers recall AND
  // search — the two that kept intercepting every Read/Grep/Glob after a
  // remove-hooks, because they have their own `config.recall.enabled` switch
  // (default ON) and never read consent at all.
  if (optedOut()) return consentRefused({ command, matcher, current: false, upgraded: 0, deduped: 0, yielded: 0, declined: 0, rematched: 0 });
  const file = claudeSettingsPath();
  const { settings } = readSettings(file);
  const fingerprint = hookFingerprint();
  ...
  const upgradedList = list.map(entry => {
    ...
    const inner = entry.hooks.map(h => {
      ...
      // Same two stand-down rules as the Stop hook, and for the same reason:
      // ensureInstalled() runs all four reconcilers on every launch, so a
      // downgrade guard on Stop alone would still let a dev checkout drag the
      // recall/search/notes registrations back to an older build.
      const reason = standDownReason(h);
      if (reason === 'newer') {
        declined++;
        return h;
      }
      ...
      upgraded++;
      return { ...h, command: withPrefix(h.command, command), membridgeFingerprint: fingerprint };
    });
  ...
  if ((current || yielded || declined) && !upgraded && !rematched) {
    return { file, command, current, upgraded, rematched, yielded, declined, wrote: false };
  }
  const finalList = (current || upgraded || rematched || declined)
    ? upgradedList
  ...
  writeSettings(file, next);
  return { file, command, current, upgraded, rematched, yielded, declined, wrote: true };
}
...
function reconcileOwnedList(list, command, isOurs, timeout) {
  ...
  for (const entry of list) {
    ...
    for (const h of entry.hooks) {
      ...
      if (sameOurs(h.command, command) && h.membridgeFingerprint === fingerprint) {
        current = true;
        inner.push(h);
      } else if (isNewerRegistration(h)) {
        // Never downgrade -- same rule as the Stop and PreToolUse reconcilers.
        declined++;
        inner.push(h);
      } else {
        upgraded++;
        // sibling fields (timeout, ...) and any wrapper prefix preserved
        inner.push({ ...h, command: withPrefix(h.command, command), membridgeFingerprint: fingerprint });
      }
    }
    if (!inner.length) continue; // entry held only duplicate owned hooks: drop it whole
  ...
  if (seenOwned) return { list: kept, upgraded, deduped, declined, current, added: false };
  return {
    list: [...kept, { hooks: [{ type: 'command', command, timeout, membridgeFingerprint: fingerprint }] }],
    upgraded, deduped, declined, current: false, added: true,
  };
}
...
function reconcileNotesHooks() {
  if (optedOut()) return consentRefused({ commands: {}, upgraded: 0, deduped: 0, added: 0, retired: 0 });
  const file = claudeSettingsPath();
  const { settings } = readSettings(file);
...
function setupHooks() {
  // The grant is recorded BEFORE anything is written, and that order is the
  // point: this function IS the opt-in (the `membridge setup-hooks` command,
  // the Settings toggle, and the "Enable" button on the first-run dialog all
  // land here), and every writer below refuses while an opt-out stands. It is
  // also the documented way back in after `remove-hooks`, exactly as
  // `membridge mcp register` is after `membridge mcp remove`.
  //
  // Recording first is also the safe crash order, the same one mcp-register
  // argues for: a crash between the two leaves a grant with nothing installed
  // (the next launch reconciles it in), never an install nobody consented to.
  try { hookConsent.record(hookConsent.GRANTED, 'setup-hooks'); } catch { /* unwritable config must not block the install the user just asked for */ }
...
// ---------------------------------------------------------------------------
// Fingerprint VERSIONS, not fingerprint equality.
//
// The bug this fixes was live on the author's machine: the repo checkout is
// 0.2.8, the installed app that actually runs the hooks is 0.3.2, and the
// registration is correctly stamped v=0.3.2. Every comparison here was a bare
// `=== hookFingerprint()`, so:
//   - hookVintage reported the NEWER registration as 'outdated', i.e. the
//     signal fired in the wrong direction and invited a downgrade;
//   - reconcile then took that invitation on every launch (ensureInstalled
//     calls it unconditionally), rewriting the 0.3.2 registration to point at
//     the 0.2.8 checkout AND dropping the exec wrapper along with it. Measured
//     divergence between the two builds' Stop prompt: 2998 vs 2925 chars.
// Inequality alone cannot tell an upgrade from a downgrade. It has to compare.
// ---------------------------------------------------------------------------

// The version out of a `v=<x.y.z>` stamp, or null when absent/unparseable.
function fingerprintVersion(fp) {
  const m = /^v=(.+)$/.exec(String(fp || '').trim());
  const v = m && m[1].trim();
  return v || null;
}

// Numeric dotted-segment compare: -1 | 0 | 1, or null when either side is not
// comparable. Prerelease suffixes ('1.2.3-beta.1') compare on their numeric
// prefix and then rank BELOW the plain release, matching semver's intent
// without pulling in a semver dependency for a five-line job.
...
// True when this hook object was written by a STRICTLY NEWER MemBridge than
// the one running now. Fail-soft in both directions: an unstamped hook (a
// pre-fingerprint build) and an unparseable version are not "newer", so the
// ordinary upgrade path still reaches them.
function isNewerRegistration(h) {
  const theirs = fingerprintVersion(h && h.membridgeFingerprint);
  if (!theirs) return false;
  return compareVersions(theirs, membridgeVersion()) === 1;
}

// One place decides whether an owned registration is ours to rewrite. Two
// independent reasons to stand down, both about not making a working install
// worse:
//   'newer'  their build post-dates ours — rewriting is a DOWNGRADE;
//   'yield'  ours is throwaway (npx cache, linked worktree) and theirs is real.
// Anything else is a legitimate upgrade-in-place.
function standDownReason(h) {
  if (isNewerRegistration(h)) return 'newer';
  if (shouldYieldTo(h.command)) return 'yield';
  return null;
}
...
// 'current' | 'outdated' | 'newer' | 'unknown'. 'unknown' covers both "nothing
// owned is registered here" (there is no vintage to report -- isHookInstalled /
...
//
// 'newer' is the state that was missing, and its absence was not cosmetic. A
// registration stamped with a HIGHER version than the process asking was
// reported as 'outdated' -- so the dashboard offered "update available" for a
// hook that was already ahead, and the fix it offered was a downgrade. Live
// example: repo 0.2.8 reading the installed 0.3.2 app's registration. What the
// user needs to be told there is the opposite: this checkout is behind what is
// registered, so nothing here should touch it.
function hookVintage(list, isOwn) {
  ...
  if (owned.every(h => h.membridgeFingerprint === fp)) return 'current';
  // Any owned hook written by a newer build makes this whole channel 'newer':
  // it is the one state where the correct action is to do nothing.
  if (owned.some(isNewerRegistration)) return 'newer';
  return 'outdated';
}
...
// available" / "newer" / "unknown" pair, read fresh from settings.json every
// call.
...
// ---------------------------------------------------------------------------
// WHICH MEMBRIDGE ACTUALLY RUNS ON A SESSION STOP.
//
// There is one ~/.claude/settings.json and there may be several copies of
// MemBridge on a machine (an installed app, a global npm install, one or more
// dev checkouts). The hook that fires is whichever ABSOLUTE PATH is written in
// that file — which is frequently NOT the copy a developer is editing, and
// nothing anywhere said so. Measured consequence on the author's machine: the
// repo emitted a 2925-char Stop prompt while the registered app emitted 2998,
// so every local change to blockReason, the caps or isCheckpointDue had zero
// effect on captured data, and a local test run proved nothing about what was
// running. The split was inferable from three separate files and stated in
// none.
//
// This makes it one line of `membridge status`: the effective script, its
// stamped version, any wrapper in front of it, and how that compares to the
// process you just ran.
function effectiveHookOf(list, isOwn) {
  const owned = ownedHookObjects(list, isOwn);
  if (!owned.length) return { registered: false };
  const h = owned[0];
  const split = splitHookCommand(h.command);
  return {
    registered: true,
    command: h.command,
    script: split.script,
    wrapper: split.prefix || null,
    // The runtime of OUR half, not of the whole command: with a wrapper in
    // front, commandExecutable(h.command) is the wrapper, which answers a
    // different question than "which node/Electron runs the hook".
    runtime: commandExecutable(split.ours),
    version: fingerprintVersion(h.membridgeFingerprint),
    live: commandIsLive(h.command),
    timeout: Number.isFinite(h.timeout) ? h.timeout : null,
  };
}

function effectiveHooks() {
  const self = { script: hookScriptPath(), version: membridgeVersion() };
  let settings;
  try {
    ({ settings } = readSettings(claudeSettingsPath()));
  } catch (err) {
    return { file: claudeSettingsPath(), self, error: String((err && err.message) || err) };
  }
  const hooksObj = settings.hooks || {};
  return {
    file: claudeSettingsPath(),
    self,
    stop: { ...effectiveHookOf(hooksObj.Stop, isOwnStopHook), vintage: hookVintage(hooksObj.Stop, isOwnStopHook) },
    recall: { ...effectiveHookOf(hooksObj.PreToolUse, isOwnRecallHook), vintage: hookVintage(hooksObj.PreToolUse, isOwnRecallHook) },
    search: { ...effectiveHookOf(hooksObj.PreToolUse, isOwnSearchHook), vintage: hookVintage(hooksObj.PreToolUse, isOwnSearchHook) },
  };
}
...
function describeForceUpdate(r) {
  // Checked first of all: a force-update on an install the user opted out of
  // must say so, not report "already up to date" over a hook that is not there
  // and is not coming back until `setup-hooks` runs.
  if (r.optedOut) return 'not installed: MemBridge hooks are turned off for this machine — `membridge setup-hooks` turns them back on';
  // Checked before `yielded`, and before `wrote`: "I refused to downgrade your
  // working hook" is the most important thing this function can say, and a
  // force-update that silently did nothing is the shape this whole feature
  // exists to end.
  if (r.declined) return `left in place: it is registered by a NEWER MemBridge than this one (v${membridgeVersion()}) — updating would downgrade it`;
...
// The git clean filter (lib/git-block-strip.js) turned on for this machine,
// on the launch path.
//
// WHY HERE AND NOT IN setupHooks(). setupHooks() is reached only by three
// EXPLICIT user actions — the `setup-hooks` command, granting first-run
// consent, and switching session summaries on in Settings. Putting the filter
// there would couple it to a decision it has nothing to do with: a user who
// answers "Not now" to the summaries dialog (app/main.js) would never get it,
// and toggling summaries off and on would silently re-enable it. That is the
// same mistake bin/membridge.js deliberately avoids by wiring the MCP
// unregister outside hooks.removeHooks(). ensureInstalled() is the function
// whose stated job is "lands however MemBridge was installed", which is
// precisely the requirement here.
//
// WHY THERE IS NO "ALREADY DONE" FLAG. Cost first: this adds ONE `git config
// --get` spawn per launch (~14ms measured), not the per-repo sweep that forced
// ensurePostCommitForVersion above behind a version marker — the rest is
// readFileSync per tracked repo. That is cheap enough to pay every time, and
// paying it is what buys the property a cache cannot have: git is asked what is
// really configured, so no stored "installed" can outlive an install that moved
// or a ~/.gitconfig that was wiped, and both heal on the next launch. It also
// keeps a new read-modify-write out of state.json, which has no locking.
//
// The ONE thing that is remembered is the user saying no (config.gitFilter.
// enabled === false, set by `membridge git-filter off`), because that is an
// intent git cannot be asked about. Everything else is observed, not recalled.
function ensureGitFilter() {
  try {
    return require('./git-block-strip').ensureEnabled().state;
  } catch (err) {
    // Fail-open and silent, like every other step on this path: a machine
    // without git, or an unwritable ~/.gitconfig, must not disturb a launch.
    try { util.log(`git filter setup skipped: ${err && err.message}`); } catch {}
    return 'failed';
  }
}
...
function ensureInstalled() {
  try {
    // #48/#49: the gate this function never had. It used to write the Stop
    // hook, both PreToolUse hooks, the SessionStart hooks and a Bash
    // auto-approve rule on every launch, before the first-run dialog had asked
    // anything and after `remove-hooks` had removed all of it. Nothing here is
    // user-initiated -- it runs on a daemon boot and an app launch -- so it is
    // the one path that demands an affirmative grant rather than merely the
    // absence of a refusal.
    //
    // The MCP reconcile below is deliberately OUTSIDE the gate: registering
    // the MemBridge MCP server is a different decision with its own recorded
    // opt-out (mcp-register.ensureRegistered), and a user who wants no hooks
    // may still want their agent able to ask MemBridge questions. Folding it
    // in here would silently revoke a consent nobody withdrew.
    scheduleMcpReconcile();             // gated AND deferred: see below
    // ABOVE the consent gate, for the same reason scheduleMcpReconcile is: the
    // git filter carries its OWN opt-out (`membridge git-filter off`, remembered
    // as config.gitFilter.enabled === false), so consent to CAPTURE is not the
    // switch that governs it. It is also protective rather than acquisitive --
    // it stops MemBridge's own block being committed into the user's repo, and
    // a user who declined capture has, if anything, a stronger claim to that
    // protection than one who accepted. Gating it here made a declining or
    // not-yet-asked machine silently commit the block.
    const gitFilter = ensureGitFilter();
    const consent = installConsent();
    if (consent !== hookConsent.GRANTED) return { registration: 'no-consent', gitFilter, consent };
    ...
    // Its own try/catch, INSIDE this one, so a git failure cannot turn a
    // healthy hook registration into 'failed'. It no longer has to protect the
    // MCP reconcile that used to follow it: #48/#49 moved that call ABOVE the
    // consent gate (see scheduleMcpReconcile at the top of this function),
    // because MCP registration carries its own separate opt-out and must run
    // whether or not hooks were granted. Do not re-add a second call here —
    // one launch must schedule exactly one reconcile. ensureGitFilter has moved
    // above the consent gate for the same reason; do not re-add it here either.
    return { registration: rc && rc.wrote ? 'wrote' : 'current', gitFilter, consent };
  } catch (err) {
    try { util.log(`hook auto-register skipped: ${err && err.message}`); } catch {}
    return { registration: 'failed', gitFilter: 'failed' };
  }
}
...
// `remove-hooks` as its callers have always used it: one sentence for a human.
// The counted form below is what the first-run decline needs, because the
// sentence it prints has to depend on whether anything was really there.
function removeHooks(opts = {}) {
  return removeHooksDetailed(opts).message;
}

function removeHooksDetailed({ via = 'remove-hooks' } = {}) {
  ...
  const pcLine = pcRemoved
    ...
    : '';
  const file = claudeSettingsPath();
  // Throws on an unreadable or malformed settings.json, and that throw must
  // reach the caller BEFORE the opt-out below is recorded: this command tells
  // the user "refusing to touch <file>" and exits 1, so recording a decision
  // it says it did not make would be the same silent-false-success shape the
  // rest of this file guards against.
  const { settings, existed } = readSettings(file);
  // Recorded as an OPT-OUT, and recorded before the WRITE -- the mirror of
  // mcp-register.unregisterNow, for the same reason and in the same order.
  //
  // Before this, removal was cosmetic: `remove-hooks` (and the Settings
  // toggle, which routes here) stripped every entry, reported success, and the
  // next daemon boot or app launch put all four back through ensureInstalled.
  // The Stop hook then no-opped on `distill.enabled`, but recall and search
  // have their own switch, default ON, so they resumed intercepting every
  // Read/Grep/Glob for a user who had explicitly removed them.
  //
  // Before, not after, because a crash between the record and the strip must
  // leave the SAFE state: an opt-out with hooks still on disk is inert (every
  // body checks the same record) and the next launch will not re-add them; a
  // strip with no record recreates the exact bug above.
  try { hookConsent.record(hookConsent.DECLINED, via); } catch { /* unwritable config: the strip below still happens */ }
  if (!existed) return { removed: pcRemoved, message: `No Claude Code settings file at ${file} — nothing to remove.${pcLine}` };
  ...
  if (!removed && !removedAllow) return { removed: pcRemoved, message: `No MemBridge hook found in ${file} — nothing changed.${pcLine}` };
  ...
  return { removed: total + pcRemoved, message: `Removed the MemBridge hooks from ${file} (${total} entr${total === 1 ? 'y' : 'ies'}: Stop + recall/search PreToolUse + teammate-notes SessionStart/PostCompact where present); your other hooks are untouched.${pcLine}
Re-enable anytime with: membridge setup-hooks` };
}

module.exports = {
  runStop, runRecall, runAppend, runPostCommit, countSummaryLines, hasSummaryLine, lastSummaryTs, isCheckpointDue, blockReason, summariesPath, claudeSettingsPath, SUMMARIES_FILE, HEADLINE_MAX, GOAL_MAX, POINT_MAX, POINTS_MAX, AREAS, splitAreaPrefix,
  setupHooks, removeHooks, removeHooksDetailed, isHookInstalled, isRecallHookInstalled, hookCommand, recallCommand, searchCommand, appendAllowRule, postCommitCommand,
  ...
  // Version-aware comparison and command splitting: the two pieces that keep a
  // reconcile from downgrading a registration or dropping its wrapper.
  fingerprintVersion, compareVersions, isNewerRegistration, splitHookCommand,
  // Which MemBridge actually runs on a stop (see effectiveHooks).
  effectiveHooks,
  // Consent (#48/#49). installConsent is the launch path's gate and the only
  // thing that performs the pre-existing-install migration; hookConsent is
  // re-exported so a caller that already has lib/hooks does not need a second
  // require to read or record a decision.
  installConsent, hookConsent,
};

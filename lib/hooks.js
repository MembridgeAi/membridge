...
const NOTES_HOOKS = [
  ...
  { event: 'PostCompact', sub: 'notes-post-compact', timeout: 5 },
];
...
const NOTES_OWN_PATTERNS = new Map(NOTES_HOOKS.map(({ sub }) => [sub, [
...
function reconcileNotesHooks() {
  ...
  if (!upgraded && !deduped && !added) {
    return { file, commands, upgraded, deduped, added, wrote: false };
  }
  writeSettings(file, { ...settings, hooks: nextHooks });
  return { file, commands, upgraded, deduped, added, wrote: true };
}

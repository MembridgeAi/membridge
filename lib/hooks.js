...
function isOwnRecallHook(h) {
  ...
  return cmd.includes(HOOK_SCRIPT) || /(^|[\s"'/])membridge\s+hook\s+recall$/.test(cmd);
}
...
// removeHooks() for both the Stop and PreToolUse arrays — with a DIFFERENT
// ownership test each: Stop uses the historical broad "mentions membridge"
// rule, PreToolUse the narrow isOwnRecallHook (a user's own Read/Write hooks
// live there, and one merely mentioning "membridge" is not ours to delete).

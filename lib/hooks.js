...
// ~70 chars forces a phrase written in the agent's own words; the full ask
// still exists on the session page's prompt chain, so nothing is lost.
...
function blockReason(target, sessionId, n) {
  ...
  return 'MemBridge is working normally — this is not an error. MemBridge pauses the stop so this summary gets written. ' +
    ...
    `— goal: the intent behind the session in your own words, at most ${GOAL_MAX} characters — one short phrase on why this work happened, so a teammate knows the why at a glance; never restate the prompt (it is already captured verbatim), or ""; ` +
...
function runAppend(argv) {
  ...
  if (goal.length > GOAL_MAX) {
    return fail(`invalid line: "goal" is ${goal.length} characters, max ${GOAL_MAX} — write the session's intent in your own words as one short phrase, never a restated prompt. The verbatim prompt is already captured; "goal" is the why behind it.`);
  }
  try {
...
function reconcileRecallHook() {
  ...
  const upgradedList = list.map(entry => {
    ...
    const inner = entry.hooks.map(h => {
      ...
      // global recall hook at a throwaway copy of us (see shouldYieldTo),
      // and never stamp our fingerprint onto a hook we did not write.
  ...
  const finalList = (current || upgraded || rematched)
    ? upgradedList
    : [...upgradedList, { matcher: RECALL_MATCHER, hooks: [{ type: 'command', command, timeout: 5, membridgeFingerprint: fingerprint }] }];
...
function forceUpdateHooks() {
  ...
  return { stop: run(reconcileStopHook), recall: run(reconcileRecallHook) };
}
...
module.exports = {
  runStop, runRecall, runAppend, runPostCommit, countSummaryLines, hasSummaryLine, blockReason, summariesPath, claudeSettingsPath, SUMMARIES_FILE, HEADLINE_MAX, GOAL_MAX,
  setupHooks, removeHooks, isHookInstalled, isRecallHookInstalled, hookCommand, recallCommand, appendAllowRule, postCommitCommand,

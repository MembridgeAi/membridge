...
const RECALL_MATCHER = 'Read|Grep|Glob';
...
// named Membridge).
const isOwnAppendRule = v => typeof v === 'string' && v.toLowerCase().includes(HOOK_SCRIPT) && v.includes(' append');
...
function reconcileRecallHook() {
  ...
  if (current && !upgraded) {
    return { file, command, current, upgraded, wrote: false };
  }
  // A generous settings-level timeout: lib/hooks-recall.js's own 150ms
  // unref'd watchdog force-exits long before this — this is only a backstop
  // against Claude Code's own hook runner, not the real budget.
  const finalList = (current || upgraded)
    ? upgradedList
  ...
  writeSettings(file, next);
  return { file, command, current, upgraded, wrote: true };
}
...
function setupHooks() {
  ...
  if (r.current && !r.upgraded && !r.addedAllow && !rc.wrote) {
...
// removeHooks() for both the Stop and PreToolUse arrays.
function stripMembridgeEntries(list) {
  let removed = 0;
  const kept = [];
...
function removeHooks() {
  ...
  const stopResult = stripMembridgeEntries((settings.hooks || {}).Stop || []);
  const preResult = stripMembridgeEntries((settings.hooks || {}).PreToolUse || []);
  const removed = stopResult.removed + preResult.removed;
  ...
  return `Removed the MemBridge hooks from ${file} (${total} entr${total === 1 ? 'y' : 'ies'}: Stop + recall PreToolUse where present); your other hooks are untouched.${pcLine}
...
module.exports = {
  ...
  ensureInstalled, reconcileStopHook, reconcileRecallHook, ensurePostCommitForVersion,
};

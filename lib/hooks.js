...
function ensureInstalled() {
  try {
    ...
    // Its own try/catch, INSIDE this one, so a git failure cannot skip the MCP
    // reconcile below it or turn a healthy hook registration into 'failed'.
    const gitFilter = ensureGitFilter();
    scheduleMcpReconcile();             // gated AND deferred: see below
    return { registration: rc && rc.wrote ? 'wrote' : 'current', gitFilter };
  } catch (err) {

...
function normalizeTeam(row, opts) {
  ...
  return {
    origin: 'team',
    // Canonicalised, not taken verbatim -- see "THE INSTANT, not the spelling
    // of it" below. Postgres hands ts back as `+00:00` with trailing fraction
    // zeros trimmed, so a team row and its local twin spell one instant two
    // ways. dedupeKey already parses around that, but buildFeed's sort and the
    // windowStart floor still compare ts as STRINGS, and a string compare of
    // '...49+00:00' against '...49.000Z' orders by punctuation rather than by
    // time. Fixing this here rather than in those comparisons is deliberate:
    // the header warns that the page cursor rides the same axis as the sort in
    // three places that must agree, so making every ts on this path one
    // canonical spelling keeps them agreeing instead of moving one of them.
...
// rows from the feed.
// One spelling for one instant: the Z-form a local entry already carries
// (lib/scan.js's toISOString). Anything unparseable is returned UNCHANGED
// rather than coerced -- a row whose ts is junk keeps its junk and sorts to
// the tail, which is what the falsy-ts handling downstream already expects,
// whereas substituting an epoch would silently promote it to 1970 and a
// throw would drop a teammate's row entirely.
function canonicalTs(ts) {
  if (!ts) return '';
  const t = Date.parse(ts);
  return Number.isFinite(t) ? new Date(t).toISOString() : String(ts);
}
...
function buildFeed(input) {
  ...
  if (input.windowStart) {
    // Canonicalised for the SAME reason every ts on this path is: the compare
    // below is a string compare, and the caller's windowStart is hand-written
    // ISO that routinely omits milliseconds ('...02:00:00Z'). Against a
    // canonical entry ts ('...02:00:00.000Z') that orders by punctuation --
    // '.' (0x2E) sorts before 'Z' (0x5A) -- so the entry sitting exactly ON
    // the window boundary compares as OLDER than the boundary and drops out
    // of page one. Canonicalising one side and not the other is precisely the
    // desync this file's header warns about; both sides move together.
    const cutoff = canonicalTs(input.windowStart);
    let inWindow = 0;

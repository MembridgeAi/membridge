...
function buildRequests(events) {
  ...
  for (const e of events) {
    ...
    byKey.set(key, {
      ...
      ctx: norm.context,
      out: norm.output,
      // The per-field breakdown, carried alongside the ctx/out roll-up the
      // volume arithmetic uses. Tier 1 measured spend (lib/server.js
      // measuredSpendPayload) reports uncached input, cache reads and cache
      // writes separately, and it must read them off THIS record rather than
      // re-normalising: dedupe and provider selection happen here, so a second
      // normalisation pass elsewhere is a second place for the cache subset
      // rules to drift. Additive -- every existing consumer picks by name.
      input: norm.input,

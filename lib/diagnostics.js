...
// Precedence is by PRESENCE of the config key, not by its truthiness -- the same
// fix lib/counters.js's countersUrl() carries, and it matters more here because
// this endpoint takes the rich payload, not four anonymous counters. A blank
// diagnosticsUrl is a deliberate opt-out and must outrank the URL derived from
// the baked backend; `||` collapses "absent" and "explicitly emptied" into one
// falsy case, so an explicit opt-out still sent (lib/backend.json ships a real
// Supabase project, so the default is never empty in a MemBridge build). An
// absent key keeps that default, which is what our own builds ship for.
// Whitespace is trimmed on both sides of the decision: it reads as the same
// opt-out, and untrimmed it was truthy enough to clear sendDiagnostic's guard.
// The baked root is trimmed BEFORE the path is appended, so a whitespace-only
// url in backend.json cannot derive a bogus-but-non-empty endpoint. A non-string
// (including a backend.json with no "url") resolves to never-send.
function diagnosticsUrl(config, bakedRoot = BAKED.url) {
  if (config && typeof config.diagnosticsUrl === 'string') return config.diagnosticsUrl.trim();
  const root = (typeof bakedRoot === 'string' ? bakedRoot : '').trim().replace(/\/+$/, '');
  return root ? `${root}/functions/v1/diagnostics` : '';
}
...
//
// SEND-ONLY: this switch does not reach onto disk. `recordToolUse` in
// lib/mcp-usage.js writes a local tally, and it uses `tallyEnabled` below --
// not this function -- so a per-session `MEMBRIDGE_NO_DIAGNOSTICS=1` never
// erases a persistent file that this session was not going to send anyway.
...
// The DISK companion to diagnosticsEnabled. Gates writes to the local telemetry
// tally (lib/mcp-usage.js's mcp-usage.json). Governed by CONFIG ONLY:
//
//   • `config.diagnostics.enabled === false` -- a durable "no telemetry"
//     choice reasonably includes "no telemetry-shaped file on my disk",
//     even though this file is tool names + timestamps and never leaves
//     the machine on its own. That is the user's explicit standing choice
//     and it applies here as well as to sends.
//   • `MEMBRIDGE_NO_DIAGNOSTICS=1` -- deliberately NOT read here. That env
//     is a single-session override, and one shell setting it should not
//     invalidate a persistent file that the next session (without the env
//     set) is entitled to send. Before the split this env silently stopped
//     recordToolUse too, contradicting this file's own docs above.
function tallyEnabled(config) {
  const d = config && config.diagnostics;
  return !(d && d.enabled === false);
}

...
// sessionState: { served: {relPath: contentHash}, reads: {relPath: contentHash},
// interceptions: n }. Missing or corrupt -> a fresh session, never a throw
// (fail-open all the way down).
//
// `reads` is the READ-TIME hash: what the file looked like at the moment this
// session was about to read it. `served` is a different fact (what we handed
// back when we intercepted) and neither substitutes for the other. See
// tierFor in lib/recall.js for why Tier A needs this and cannot borrow the
...
// Record what this session saw, THIS read. Returns the next `reads` map, or
// null when nothing changed (the common case on a re-read of a stable file),
// so the caller can skip the write entirely.
function nextReadsMap(sessionState, relPath, hash) {
  const reads = (sessionState && sessionState.reads) || {};
  if (reads[relPath] === hash) return null;
  const next = { ...reads, [relPath]: hash };
...
function recordRead(projectPath, sessionId, sessionState, relPath, hash) {
  const nextReads = nextReadsMap(sessionState, relPath, hash);
  if (!nextReads) return;
  try {
...
function doRunRecall() {
  ...
  // With no cache entry the ONLY tier that can fire is A, and it requires this
  // session's earlier read of this path to have seen exactly these bytes.
  // It did not -- either this is the first read of the path, or the file
  // changed since the last one -- so record what this read is seeing and stop,
  // before the ledger parse (0.48ms on this repo's 604KB ledger) and decide(),
  // neither of which could reach a serve.
  //
  ...
  if (!storeEntry && sessionState.reads[relPath] !== hash) {
    recordRead(projectPath, sessionId, sessionState, relPath, hash);
    emitNotes(notesOut);
    return;
  }
  ...
  if (decision.serve) {
    ...
    saveSessionState(projectPath, sessionId, {
      served: nextServed,
      reads: nextReadsMap(sessionState, relPath, hash) || sessionState.reads,

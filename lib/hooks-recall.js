...
function buildNotesOutput({ projectPath, absPath, relPath, sessionId, now, config, proj }) {
  try {
    ...
    // The pause/exclude kill switch, gated on isProjectOff ALONE -- not the
    // `tracked` const further down this file, which also folds in
    // diagnostics.isRecallPausedForProject. That is a SEPARATE, recall-only
    // pause (config.recall.pausedProjects); conflating it here would make the
    // recall-pause switch silently also disable teammate notes. This has to be
    // its own check, and it goes before the index read since it is the
    // cheapest thing that can end this call.
...
function doRunRecall() {
  ...
  // CONSENT, before anything is read, served or recorded. This hook had no
  // consent check of any kind: `config.recall.enabled` defaults to ON, so a
  // user who answered "Not now" to the first-run dialog -- or who ran
  // `membridge remove-hooks` and had the entry silently reinstated on the next
  // launch -- kept having every single Read intercepted by a hook they were
  // told was not installed. Nothing above this line has any observable effect
  // (stdin parse and a path resolve), so returning here is a true no-op.
  if (hookConsent.declined(config)) return;

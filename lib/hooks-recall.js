...
// of file reads -- the one file here that could otherwise grow without limit.
// Past this size it is rotated down to its most recent half on a line
// boundary: the recent rows are the ones any comparison cares about, and
// halving (rather than truncating to empty) keeps enough history to stay
// useful between rotations.
...
// Drop everything before the midpoint of events.jsonl, cutting at the first
// newline at or after it so no half row survives. Best-effort: a rotation
// failure must never stop the row that triggered it from being written, so
// this swallows its own errors and lets the append proceed.
function rotateEvents(file) {
  try {
    if (fs.statSync(file).size <= MAX_EVENTS_BYTES) return;
    const text = fs.readFileSync(file, 'utf8');
    const cut = text.indexOf('\n', Math.floor(text.length / 2));
    fs.writeFileSync(file, cut === -1 ? '' : text.slice(cut + 1));
  } catch {}
}
...
function doRunRecall() {
  ...
  if (decision.serve) {
    // Order matters: the session-state commit is what marks this path served
    // and burns an interception, and it is the LAST thing to happen before
    // stdout. If the event write throws after it (a full disk, an unwritable
    // .membridge), the outer catch swallows the error and nothing is printed
    // -- so the agent gets an ordinary read, while the path would have been
    // locked out of recall for the rest of the session. Writing the event
    // first means a failure there costs one measurement row, not a serve.
    // Neither write may be skipped: stdout still comes after both.
  ...
  if (decision.reason === 'holdout') {
    ...
    if (wouldServe !== null) {
      appendEvent(projectPath, {
        ts: new Date().toISOString(), sessionId, relPath, holdout: true, wouldServe,
      });
    }
  }

...
const zeroAvoided = () => ({ tokens: 0, serves: 0, tierA: 0, tierB: 0, tierUnknown: 0, partialWins: 0, netNegatives: 0 });
const zeroHoldout = () => ({ skips: 0, callTokens: 0 });
// The controlled comparison's two arms (measured-savings spec, Tier 3). Same
// rows, different question: `avoided` asks "how many tokens did serving save",
// which only the served side can answer, while this asks "what did one
// eligible read COST", which both sides answer in the same units and can
// therefore be differenced. See lib/ledger-fold-state.js's normalizeComparison
// for the block these deltas land in, and lib/roi.js for the arithmetic.
//
// The per-read cost of a SERVED read is callTokens - net: what the session
// actually spent instead of the read it would have done. Deriving it from
// `net` rather than reading skeletonTokens directly is what makes corrections
// work -- when a follow-up read revises net down, the served arm's cost rises
// by exactly the same amount, with no second pass needed to notice.
// The per-read cost of a WITHHELD read is its callTokens: the read happened.
const zeroComparison = () => ({
  served: { reads: 0, tokens: 0, tokensSq: 0 },
  withheld: { reads: 0, tokens: 0, tokensSq: 0 },
});
// Spec §9: teammate-note injections. Tallied HERE, in the pass that consumes
// the queue, and not by any later reader of events.jsonl -- lib/ledger-fold-
// recall.js's commit() rewrites the file down to remainder+retained, and a
// notes_injected row is neither, so it exists for exactly one fold pass. A
// separate reader would find the rows already gone.
const zeroNotes = () => ({ injections: 0, tokens: 0 });
...
// knownNotesKeys (spec §9) is the third instance of the same
// durable-dedupe-evidence pattern as knownServeIds/knownHoldoutKeys, for the
// identical reason: a commit() queue-rewrite failure leaves the same
// notes_injected rows queued for the next pass, and an input-cost figure that
// climbs on its own every fold pass would be worse than none.
function settleRows(rows, now, buildOpenServe, knownServeIds, knownHoldoutKeys, knownNotesKeys) {
  const known = knownServeIds || new Set();
  const knownHoldout = knownHoldoutKeys || new Set();
  const knownNotes = knownNotesKeys || new Set();
  const avoidedDelta = zeroAvoided();
  const holdoutDelta = zeroHoldout();
  const comparisonDelta = zeroComparison();
  const notesDelta = zeroNotes();
  const newOpenServes = [];
  ...
  for (const serve of settleGroups) {
    ...
    // WHY THESE ARE THREE EXPLICIT BUCKETS AND NOT AN if/else.
    //
    // This split is the only thing that lets someone later ask why `avoided`
    // moved: Tier A rising means the POINTER tier stopped being suppressed,
    // Tier B rising means more files became ELIGIBLE. Those have different
    // causes and different follow-ups, and the counts are what separate them.
    //
    // The previous shape was `if A ... else tierB`, so a row carrying NO tier
    // -- a pre-upgrade queue row, or a torn one -- silently counted as a Tier B
    // serve. That is an unknown being recorded as a known, which is the exact
    // defect this ledger keeps producing, and it lands here on the one field
    // the attribution depends on: a batch of untiered rows would read as
    // "eligibility widened" when nothing of the sort happened.
    //
    // Tier C still folds into tierB deliberately: it shares B's mechanics (the
    // same cached-skeleton body -- lib/recall.js's tierBBody serves both) and
    // the ledger shape distinguishes pointer-vs-skeleton, not B-vs-C. That is a
    // JUDGEMENT about two things being alike. Folding `undefined` in with them
    // was never a judgement, just an else.
    if (serve.tier === 'A') avoidedDelta.tierA += 1;
    else if (serve.tier === 'B' || serve.tier === 'C') avoidedDelta.tierB += 1;
    else avoidedDelta.tierUnknown += 1;
    // Served arm of the controlled comparison. cost = what this eligible read
    // cost the session, which at settle time is the body it got instead.
    const servedCost = serve.callTokens - net;
    comparisonDelta.served.reads += 1;
    comparisonDelta.served.tokens += servedCost;
    comparisonDelta.served.tokensSq += servedCost * servedCost;
  ...
  for (const row of rows) {
    ...
    const withheldCost = Number.isFinite(row.callTokens) ? row.callTokens : 0;
    holdoutDelta.skips += 1;
    holdoutDelta.callTokens += withheldCost;
    // Withheld arm of the same comparison. Accumulated from the same row as
    // holdoutDelta above, one line apart, so the two can never disagree about
    // how many withheld reads there were -- but kept in its own block because
    // holdout.skips/callTokens are lifetime totals that survive an epoch
    // change and this must not.
    comparisonDelta.withheld.reads += 1;
    comparisonDelta.withheld.tokens += withheldCost;
    comparisonDelta.withheld.tokensSq += withheldCost * withheldCost;
    newHoldoutKeys.push(key);
  }

  // Spec §9. A notes_injected row carries no pending/confirmation pair (an
  // injection either reached stdout or the note was never marked delivered --
  // see buildNotesOutput's own commit ordering), so there is nothing to group:
  // one row, one injection. relPath is null for a session-scoped injection and
  // is part of the key either way, so the two shapes can never collide.
  for (const row of rows) {
    if (row.kind !== 'notes_injected') continue;
    const key = `${row.ts}|${row.sessionId}|${row.relPath}`;
    if (knownNotes.has(key)) continue; // already tallied -- seenKeys IS the durable dedupe evidence
    notesDelta.injections += 1;
    notesDelta.tokens += Number.isFinite(row.tokens) ? row.tokens : 0;
    newNotesKeys.push(key);
  }

  return { avoidedDelta, holdoutDelta, comparisonDelta, notesDelta, newOpenServes, retained, newHoldoutKeys, newNotesKeys };
}

module.exports = {
  settleRows, groupServeRows, byteSizeOf, estimateReadTokens,
  zeroAvoided, zeroHoldout, zeroComparison, zeroNotes, PENDING_CONFIRM_GRACE_MS,
};

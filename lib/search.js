...
// - The file-graph backfill (below) is the one thing here that is not purely
//   lexical, and it is additive by construction: it only ever fills page slots
//   a short result set left empty, so it cannot demote a lexical hit or change
//   what "no results" means. Same discipline as the usage boost above.

const fileGraph = require('./file-graph'); // pure too — no fs, no state
...
// File-graph backfill
// ---------------------------------------------------------------------------
// Sessions that touched the same code, for a query whose words did not reach
// them. This is the one gap BM25 structurally cannot close: it joins entries
// that used the same WORDS, and two people reworking one function while
// describing it in different language share nothing for it to match on.
//
// BACKFILL, NOT FUSION, and the distinction is the whole safety argument. A
// lexical hit is never reordered, demoted, or displaced by a file neighbour;
// neighbours are appended only into slots a short page left empty. So the
// worst case for a query the lexical ranker already answers well is that
// nothing changes at all -- there is no room -- and the change can only add
// results where today there is blank space. That also makes it monotone
// against the recall harness: no existing expectation can break by adding
// rows below the ones it asserts.
//
// Which is also why it fires exactly when memory fails people. A narrow query
// that returns two results has eighteen empty slots; a broad one that fills
// the page gets no backfill and needs none.
const FOOTPRINT_HITS = 5;
const BACKFILL_MIN_AFFINITY = 0.05;

// Rank the graph's entries by how strongly they connect to a set of files,
// strongest first, skipping anything already served. Pure ordering policy --
// the graph itself, and every weighting decision in it, lives in
// lib/file-graph.js, exactly as the scorer lives here and not in the index.
function fileBackfill(graph, footprintFiles, opts = {}) {
  if (!graph || !Array.isArray(footprintFiles) || footprintFiles.length === 0) return [];
  const exclude = opts.exclude instanceof Set ? opts.exclude : new Set(opts.exclude || []);
  const min = Number.isFinite(opts.minAffinity) ? opts.minAffinity : BACKFILL_MIN_AFFINITY;
  const out = [];
  for (const key of graph.byKey.keys()) {
    if (exclude.has(key)) continue;
    const affinity = fileGraph.affinityToFiles(graph, key, footprintFiles);
    if (affinity > min) out.push({ key, affinity });
  }
  // Total order: affinity, then key. Two runs over one corpus must produce the
  // same page, or an eval comparing rankers is partly comparing Map order.
  out.sort((a, b) => (b.affinity - a.affinity) || String(a.key).localeCompare(String(b.key)));
  if (Number.isFinite(opts.limit)) return out.slice(0, opts.limit);
  return out;
}

// The files that define what a query turned out to be ABOUT: those of its
// best lexical hits. Capped at FOOTPRINT_HITS because the tail of a ranking is
// where relevance has already decayed, and letting it vote would aim the
// backfill at whatever the query half-matched.
function footprintOf(hits, limit = FOOTPRINT_HITS) {
  const files = new Set();
  for (const h of (hits || []).slice(0, limit)) {
    for (const f of (h && h.files) || []) {
      const n = fileGraph.normFile(f);
      if (n) files.add(n);
    }
  }
  return [...files];
...
module.exports = {
  ...
  fileBackfill, footprintOf, FOOTPRINT_HITS, BACKFILL_MIN_AFFINITY,
};

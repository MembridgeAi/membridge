...
// realpathSync.NATIVE, not realpathSync: on Windows a path can reach us in 8.3
// short form (os.tmpdir() yields C:\Users\RUNNER~1\...) while git records the
// long form (C:\Users\runneradmin\...). Plain realpathSync does not expand 8.3
// names, so canonicalizing the two sides with it leaves them textually
// unrelated, path.relative answers '..\..\..', and the worktree falls through
// to git's raw path instead of the project's path space -- which is exactly
// how this shipped broken on the Windows CI legs while every macOS and Linux
// leg passed. The .native variant resolves both forms to one canonical spelling.
const realpathFn = fs.realpathSync.native || fs.realpathSync;
function realpathOr(p) {
  try { return realpathFn(p); } catch { return p; }
}

function linkedWorktreesOf(mainRoot) {
  // Fail CLOSED on a missing or blank root. `path.resolve('')` is
  // process.cwd(), so this used to answer "which worktrees does <nothing>
  // have" with the worktrees of whatever directory the process happened to be
  // started in -- and this function decides where the injected block gets
  // WRITTEN, so a caller with an unresolved root could write into an
  // unrelated repo's worktrees. An absent root is not a repo, and the only
  // safe answer is none. (This is what the long-standing "worktrees: a
  // non-repo directory returns []" failure actually was: not a fixture
  // reading the real registry, but this fallback firing on any machine whose
  // cwd is a repo that has worktrees.)
  const raw = String(mainRoot || '').trim();
  if (!raw) return [];
...
// Is git PART-WAY THROUGH an operation that owns this working tree?
//
// WHY ANYTHING NEEDS TO ASK. MemBridge writes the managed block into the user's
// own tracked files (CLAUDE.md / AGENTS.md by default, unchanged since v0.1.0).
// While a rebase, merge, cherry-pick, revert or bisect is mid-flight, git is
// mid-way through rewriting that tree and requires it to be clean at each step.
// A write landing in that window does not merely race — it WEDGES the
// operation, and it does so behind a message that names the wrong cause.
// Measured, against a control that ran the identical sequence with no write:
//
//   $ git rebase origin/main            # pauses on a real conflict in app.js
//   $ <resolve app.js>; git add app.js  # index now has NO unmerged entries
//   $ git rebase --continue
//   You must edit all merge conflicts and then
//   mark them as resolved using git add
//
// There were no merge conflicts left: `git ls-files -u` was empty and so was
// `git diff --name-only --diff-filter=U`. `git diff-files` held exactly one
// path — the file MemBridge had just written. git refuses `--continue` on ANY
// unstaged change and reports it as an unresolved conflict, so the user goes
// hunting for conflict markers that do not exist. The same run with a plain
// unrelated file dirtied instead reproduces it identically, which is the proof
// that this is not about our file: it is about writing into a tree git is
// holding.
//
// SCOPE, deliberately narrow. This asks "is an operation in progress", NOT "is
// the tree dirty". Refusing to write into a merely-dirty tree would be a
// different and much worse rule, because after the first write the block ITSELF
// is what makes the tree dirty — so the daemon would write once and then never
// update the context again until the user committed, which is the product
// failing quietly instead of git failing loudly.
//
// Returns the marker name (for the log line) or null. Fails OPEN — an
// unreadable or non-git directory answers null and the caller writes as before,
// because a missing answer here must never stop context from being delivered.
function gitOperationInProgress(dir) {
  const raw = String(dir || '').trim();
  if (!raw) return null;
  let gitDir;
  try {
    const dotGit = path.join(path.resolve(raw), '.git');
    const st = fs.statSync(dotGit);
    if (st.isDirectory()) {
      gitDir = dotGit;
    } else {
      // A linked worktree's `.git` is a file pointing at
      // <main>/.git/worktrees/<name>, and THAT is where its own in-progress
      // state lives — a rebase in one worktree must not silence writes in a
      // sibling, and a rebase in the main checkout must not be missed here.
      const m = /^\s*gitdir:\s*(.+?)\s*$/m.exec(fs.readFileSync(dotGit, 'utf8'));
      if (!m) return null;
      gitDir = path.resolve(path.dirname(path.resolve(raw)), m[1]);
    }
  } catch { return null; }
  // Directories and files git creates for the duration of an operation and
  // removes when it ends or is aborted. `sequencer` covers a multi-step
  // cherry-pick or revert whose *_HEAD is absent between picks.
  for (const marker of ['rebase-merge', 'rebase-apply', 'MERGE_HEAD', 'CHERRY_PICK_HEAD',
    ...
    } catch { /* one unreadable marker is not evidence about the others */ }
  }
  return null;
}
...
module.exports = { wireKeyFor, ledgerKeyFor, checkoutRoot, linkedWorktreesOf, gitOperationInProgress, clearCache };

...
//   cand=<path>       a `claude` binary APPEARED (or vanished) at a location we
//                     can probe. See the note on `firstCandidate` below.
...
// THE STICKY-MISS BUG THIS CLOSES. A `no-binary` skip used to be PERMANENT.
// Nothing in the fingerprint described whether a `claude` binary could be found
// -- `bin=` and `rec=` cover only the config override and the install-time
// record, and both are empty on exactly the machines that miss. So the first
// launch that failed to resolve the binary stamped a fingerprint, and every
// launch after it recomputed the same string, answered 'current', and returned
// before re-trying. Installing Claude Code the next day changed nothing: the
// gate stayed shut until the MemBridge VERSION changed. Measured on a real
// install -- claude-code sat at skipped/no-binary from 03:03 onward with the
// binary present on disk the whole time.
//
// That is this repo's characteristic bug wearing a different hat: a gate that
// records "decided" when what actually happened was "could not decide yet".
//
// The cheap half of the answer. Resolving properly would mean the login-shell
// query (77ms on a bare zsh, seconds under oh-my-zsh), which is the very cost
// the gate exists to avoid paying per launch. A statSync over a handful of
// fixed paths is microseconds and catches the case that matters: a binary that
// APPEARED where we can see it. The expensive authorities (the shell query) are
// still consulted only when the gate actually opens.
//
// Deliberately NOT "re-run whenever the last run reported a failure": that
// reopens the gate on failures which recur every launch and cost the full 2.1s
// `claude mcp get` each time (a foreign entry, an unreadable config), trading a
// silent miss for a permanent tax. Keying on the observable that CHANGES when
// the miss becomes fixable keeps the retry free and self-limiting -- once the
// binary resolves and registration succeeds, the fingerprint settles again.
function firstCandidate(exists = p => { try { return fs.statSync(p).isFile(); } catch { return false; } }) {
  for (const c of claudeBinMod.CANDIDATES) if (exists(c)) return c;
  return '';
}
...
function registrationFingerprint(opts = {}) {
  ...
  const parts = [
    ...
    `cand=${opts.firstCandidate ? opts.firstCandidate() : firstCandidate()}`,
...
// ---------------------------------------------------------------------------
//
// THE GAP THIS CLOSES. The injected context block carries a line telling the
// agent its memory is queryable over MCP, and lib/digest.js prints that line
// only when THIS target's own agent is registered (digest.mcpLiveFor). But
// whether a block is re-rendered at all is decided per PROJECT, by the dirty
// flag lib/scan.js's sync pass filters on. Registration is GLOBAL state and
// nothing marked a project dirty when it changed, so the moment registration
// flipped from absent to live -- every upgrade, and some fresh installs --
// every block already on disk was stale, and a project with no new events
// stayed that way forever. Measured on a real install: 8 of 9 projects
// carried a nudge-less block dated days before the registration stamp.
//
// WHY NOT WIDEN THE DIRTY FILTER. Because a synced project still has events:
// re-rendering every tracked project on every tick is a regression this repo
// already found and fixed once. This is the other shape -- a ONE-SHOT write
// on the rare tick where the verdict actually moved, after which the normal
// pass renders and clears the flags exactly as it always did.
//
// VERDICTS, NOT ROWS. A row whose `reason` moved from 'cli' to 'updated'
// while its `status` stayed 'registered' says nothing the block can render,
// and comparing raw rows would turn every such reconcile into a full
// re-render storm. So the comparison runs through digest.mcpLiveFor itself,
// per target -- the very function whose answer the block is made of.
function mcpVerdictChanged(prevRows, nextRows) {
  // Lazy require, same precedent (and same reason) as the one inside
  // lib/digest.js's renderBlock: digest is reached from this area through
  // memorydb/hooks, and keeping the edge inside the function means no future
  // require here can close a load cycle at module load time.
  const digest = require('./digest');
  return Object.keys(digest.TARGET_MCP_AGENT)
    .some(target => digest.mcpLiveFor(target, prevRows) !== digest.mcpLiveFor(target, nextRows));
}

// Mark every tracked project for one re-render. Read-modify-write against the
// live state for the same reason saveRegistration is: the daemon writes
// state.json on every tick and this runs alongside it. Returns how many
// projects were marked (0 also means "nothing was written").
function markAllProjectsDirty() {
  const state = util.loadState();
  const projects = state.projects || {};
  const keys = Object.keys(projects);
  if (!keys.length) return 0;
  const marked = {};
  for (const key of keys) marked[key] = { ...projects[key], dirty: true };
  util.saveState({ ...state, projects: marked });
  return keys.length;
}

// The whole invalidation, in one fail-open call. Exported for the suite and
// for ensureRegistered below, nothing else. Every failure path answers 0
// rather than throwing: a stale nudge line is a cosmetic loss, a launch that
// died over one is not.
function invalidateOnRegistrationChange(prevRows, nextRows) {
  try {
    if (!mcpVerdictChanged(prevRows, nextRows)) return 0;
    return markAllProjectsDirty();
  } catch (err) {
    try { util.log(`mcp nudge invalidation skipped: ${err && err.message}`); } catch {}
    return 0;
  }
}
...
function registerNow(opts = {}) {
  const resolve = opts.resolveClaudeBin || claudeBinMod.resolveClaudeBin;
  // Read BEFORE registerAll writes anything: the verdict this registration
  // moved away from is only knowable from the record it is about to replace.
  ...
  const recorded = (opts.recordBin !== false && seen && seen.source === 'shell')
    ...
    : false;
  // THIS is the common setup path, not an edge case: `membridge mcp register`
  // is what install.sh runs and what the app's Settings "re-register MCP"
  // button calls. It stamps a fingerprint, so every ensureRegistered() after
  // it answers 'current' and returns before its own invalidation can run --
  // leaving the launch path unable to be the second chance at this. Same
  // ordering as there, and for the same reason: a crash between the two writes
  // must leave the gate OPEN, never a fingerprint recorded for a registration
  // whose blocks were never invalidated.
  const invalidated = invalidateOnRegistrationChange(prev && prev.rows, rows);
  saveRegistration(stamp('register', safeFingerprint(opts), rows));
  return { rows, recordedBin: recorded ? seen.path : null, invalidated };
}
...
function unregisterNow(opts = {}) {
  ...
  // The mirror of registerNow's, and the more urgent half: an agent told to
  // call a server that is gone is worse off than one never told about it, and
  // the launch path cannot correct this one either -- the opt-out this very
  // call is about to record makes ensureRegistered() answer 'opted-out' from
  // here on, forever.
  const invalidated = invalidateOnRegistrationChange(prev && prev.rows, rows);
...
function ensureRegistered(opts = {}) {
  try {
    ...
    // Invalidate BEFORE stamping the new fingerprint. The two are separate
    // state writes, and this order is the safe one: a crash between them
    // leaves the gate open, so the next launch reconciles and re-marks (one
    // redundant re-render). Stamping first would close the gate on a
    // registration whose blocks never got invalidated, which is the stale
    // state this exists to end.
    const invalidated = invalidateOnRegistrationChange(prev && prev.rows, rows);
    saveRegistration(stamp('register', fingerprint, rows));
    return { ran: true, reason: 'reconciled', rows, invalidated };
  } catch (err) {
...
module.exports = {
  ...
  registrationFingerprint, recordClaudeBin, invalidateOnRegistrationChange,
  firstCandidate,

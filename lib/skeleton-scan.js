...
function scanLineBraces(line, prevState) {
  ...
  while (i < line.length) {
    ...
    // NOTE: there is deliberately no bare `#` line-comment rule here. This
    // module is only ever invoked from lib/skeleton-strip.js's brace-mode
    // path (JS/TS/Go); .py/.yml go through the separate indentMode branch
    // and never reach scanLineBraces at all. So a `#` here is never a real
    // comment starter -- it's either absent, or (in JS) an ES2022 private
    // class field/method, e.g. `#count` or `#increment() {`. Treating it as
    // "rest of line ignored" swallowed real braces (a method's own `{`),
    // corrupting depthBefore for every line after it and silently producing
    // wrong structure with ok:true. See the skeleton-scan regression test.

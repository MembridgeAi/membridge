...
// its `finish()` (which truncates the queue and applies rejection bumps) is
// called ONLY after writeLedger below has actually succeeded, so a failed
// ledger write leaves the queue exactly as it was for the next pass to
// retry, instead of losing rows a write that never landed already claimed.
function updateLedger(projectPath, events, config) {
  const plumbingCap = config && config.maxPlumbingEvents;
  const settlement = settleRecallQueue(projectPath, events);

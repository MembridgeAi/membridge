...
// Parse ledger.json. Missing file -> null (safe default: nothing has been
// folded yet, updateLedger folds from scratch). Unreadable or unparseable
// file -> throw, so callers refuse to write over it. ledger.json is the only
// file in a project whose contents cannot be rebuilt (its dedupe evidence
// has no other source once lost -- see the module header), and its one real
// caller, updateLedger, is a read-modify-write that runs on every sync tick
// for every project: load -> fold this pass's window on top -> write. ONLY a
// genuinely absent file (ENOENT) is safe to treat as "nothing folded yet" --
// every other failure, including a present-but-corrupt file, means the
// accumulated evidence IS there and must not be silently discarded. Same
// rule as lib/util.js's loadUserConfig/loadState (see their comments).
...
// settleRecallQueue's OPEN_SERVE_TTL_MS gate (fix round 2: settlement itself
// is immediate and no longer clock-gated -- only how long a settled serve
// stays correctable is) -- production callers (scan.js) pass nothing and get
// the real clock. maxLines (fix round 3) is the same injection pattern
// applied to settleRecallQueue's MAX_FOLD_LINES bound -- omitted, every real
// caller gets the production default.
function updateLedger(projectPath, events, config, nowFn = Date.now, maxLines) {
  // The fold's dedupe horizon is sized off "the window that feeds it", and
  // that window is now the LARGER of the two plumbing budgets: `events` holds
  // usage and reads capped independently (lib/digest.js), and readKeys/seenKeys
  // share one horizon. Passing only the usage cap would leave readKeys
  // undersized for anyone who raises maxReadEvents above it -- exactly the
  // window-exceeds-horizon case keyHorizonFor exists to prevent, which
  // re-folds already-counted events and inflates totals (measured at 38%).
  const plumbingCap = Math.max(
    (config && config.maxPlumbingEvents) || 0,
    (config && config.maxReadEvents) || 0,
  ) || undefined;
  // Read once and share: settleRecallQueue needs the previous ledger's
  // openServes list (fix round 2, revisable settlement) to run its
  // correction pass, and foldProjectLedger needs the same previous ledger to
  // fold onto -- both must see the exact same snapshot within one pass.
  const prevLedger = readLedger(projectPath);
  const settlement = settleRecallQueue(projectPath, events, prevLedger, nowFn, maxLines);
  ...
  const next = foldProjectLedger(prevLedger, events, plumbingCap, projectPath, settlement);

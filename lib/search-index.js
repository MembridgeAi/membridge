...
// crypto: the known-projects signature in pruneUntracked below. node:sqlite is
// deliberately NOT required here -- see sqliteCtor().
...
const FP_PREFIX = 'fp:';
// Signature of the project set the reconciliation sweep last ran against.
// Deliberately NOT cleared by replaceAll: a rebuild refills from tracked
// projects only, so it cannot produce an orphan, and clearing it would buy a
// full scan on the next search for nothing.
const KNOWN_PROJECTS_KEY = 'known_projects';
...
// Deletion
// ---------------------------------------------------------------------------
// WHY THIS SECTION EXISTS. This index is a DERIVED store that outlived its
// source's rules. lib/server.js deleteProject removed <project>/.membridge,
// pruned the team archive, dropped the key from state.projects and tombstoned
// the path -- and nothing here. The only writer that removes a project's rows
// is replaceProject, reached only from lib/activity.js freshenProject, which
// is only ever called for a project that is still TRACKED. A deleted project
// therefore left its rows behind permanently: full prompt text, summaries,
// decisions and gotchas, still served by /api/search, by the MCP search_memory
// tool, and by the PreToolUse Grep hook -- for a project the product told the
// user it had deleted.
//
// Everything below is the deletion half that was missing. It is here rather
// than in the caller because this module owns every statement that touches
// SQLite, and a second hand-written copy of "how you remove a project" is
// exactly how one deletion path keeps purging while another quietly stops.

// How many rows this index holds for one project. Reported by the caller
// rather than assumed: a delete that says what it removed can be checked, and
// a delete that assumes cannot.
function projectRowCount(db, projectPath) {
  const row = db.prepare('select count(*) as c from entries where project = ?').get(projectPath);
  return Number(row && row.c) || 0;
}

// Every project path this index currently holds rows for. The '' guard skips
// the empty owner that pre-`owningProject` rows could carry (see insertRows).
function indexedProjects(db) {
  return db.prepare('select distinct project from entries where project != \'\'')
    .all().map(r => r.project);
}

// Remove one project from the index COMPLETELY: its rows AND its fingerprint.
// Returns the number of rows actually removed, counted inside the same
// transaction that removes them, so the caller reports what happened rather
// than what it hoped happened.
//
// The fingerprint matters as much as the rows. Left behind it is a claim that
// this project's slice of the index is up to date, which is what a later
// re-add of the same path compares against -- and it is what lib/hooks-search.js
// rankForHook reads to decide whether the index knows about a project at all.
function pruneProject(db, projectPath) {
  let removed = 0;
  inTransaction(db, () => {
    removed = projectRowCount(db, projectPath);
    db.prepare('delete from entries where project = ?').run(projectPath);
    db.prepare('delete from meta where k = ?').run(FP_PREFIX + projectPath);
  });
  return removed;
}

// Reconciliation: drop rows for projects the index still holds but that this
// machine no longer knows about at all.
//
// This is the backstop for the purge at delete time, not a replacement for it.
// The purge runs against a SQLite file three processes write; if it fails, or
// if a delete happened under a build that never purged, the rows are still
// there and nothing else would ever revisit them. Running here costs one
// query on a pass the daemon already makes.
//
// `known` MUST be every path in state.projects -- NOT the caller's tracked/
// non-paused subset. Pausing or archiving a project takes it out of
// lib/activity.js trackedProjectEntries (via isProjectOff) while leaving it
// very much a project the user still has; reaping against that narrower set
// would silently delete the searchable memory of every PAUSED project, which
// is a far worse bug than the one this function exists to fix. The suite pins
// that distinction directly.
//
// Returns the paths it actually pruned, so the caller can report a number
// instead of asserting one.
//
// TWO GUARDS, both of which cost less than the harm they prevent.
//
// 1. COST. This runs on the freshen that precedes every search, and
//    `select distinct project` is a full scan of the content table: measured
//    at 29ms against 50k rows, on a search that otherwise costs 219ms. So the
//    scan is skipped unless the project set has actually MOVED since the last
//    sweep. That is sound rather than merely cheap: rows only ever enter the
//    index through replaceProject (called for tracked projects) or replaceAll
//    (which refills from tracked projects), so an orphan cannot appear while
//    the project set holds still. The signature is stored rather than kept in
//    memory because three processes share this file and each has its own.
//
// 2. AN EMPTY `known` IS INCONCLUSIVE, NOT "the user has no projects". This
//    is the same judgement lib/server.js archiveSharedProject makes about an
//    empty team list, and for the same reason: util.loadState returns a FRESH
//    (empty) state on a STATE_VERSION mismatch and on the pre-v2 sessionless-
//    event discard, so an upgrade or a rebuilt state.json presents exactly
//    this shape while the index legitimately holds everything. Sweeping then
//    would destroy every project's searchable memory in one pass, silently,
//    on upgrade. The delete-time purge already covers the real "deleted my
//    last project" case, so refusing here costs nothing and the log line is
//    what makes the refusal visible.
function pruneUntracked(db, known) {
  const keep = [...new Set(known || [])].sort();
  if (!keep.length) {
    if (indexedProjects(db).length) {
      try {
        util.log('search index: refusing to reap — this machine reports no projects at all, ' +
          'which is a discarded or version-bumped state.json far more often than an empty one');
      } catch { /* logging must never break a search */ }
    }
    return [];
  }
  const signature = crypto.createHash('sha1').update(keep.join('\n')).digest('hex');
  if (readMeta(db, KNOWN_PROJECTS_KEY) === signature) return [];
  const dropped = [];
  for (const projectPath of indexedProjects(db)) {
    if (keep.includes(projectPath)) continue;
    pruneProject(db, projectPath);
    dropped.push(projectPath);
  }
  // Written LAST and only after the loop completed: a throw partway through
  // must leave the signature stale so the next pass retries, rather than
  // recording a sweep that only half happened.
  db.prepare('insert or replace into meta values (?, ?)').run(KNOWN_PROJECTS_KEY, signature);
  return dropped;
}
...
function runMatch(db, match, opts = {}) {
  ...
  // Scope to the projects the caller may currently see. An ALLOW-list, not a
  // deny-list, and that direction is the whole point: the index is a cache of
  // a corpus whose membership changes underneath it (pause, archive,
  // `.membridge-off`, delete), and a deny-list can only exclude what the
  // caller already knew to name. A row for a project that has left the
  // visible set is exactly the row nobody remembers to deny.
  //
  // This is the SAFETY half of the fix; pruneProject (from lib/server.js
  // purgeSearchIndex, at delete time) and pruneUntracked (the sweep on
  // freshenIndex) are the HYGIENE half. Neither replaces the other, and that
  // is the whole reason both landed together:
  //
  //   - The purge bounds index growth and eventually removes hidden rows, but
  //     it is a WRITE to a SQLite file three processes share. If it fails, or
  //     the delete happened under a build that never purged, the rows survive.
  //   - Only DELETE reaches the delete-time purge. Pause, archive and
  //     `.membridge-off` hide a project without deleting it, and pruneUntracked
  //     deliberately does NOT reap those (it keys on state.projects, so a
  //     paused project keeps its rows). Their rows stay on disk by design.
  //
  // In both cases the rows are still there and this predicate is the only
  // thing standing between them and a caller. It fails closed and it needs no
  // schedule, so the window between hiding a project and any purge running is
  // harmless rather than merely short.
  //
  // Absent (undefined) means "unscoped", for callers that already scoped by
  // `project` above. An EMPTY array means "nothing is visible" and returns
  // nothing -- fail closed, because the alternative reading ("no filter
  // requested") turns a machine that tracks no projects into one that serves
  // everything it has ever indexed.
  if (Array.isArray(opts.projects)) {
    const visible = opts.projects.filter(Boolean);
    if (!visible.length) return [];
    sql += ` and project in (${visible.map(() => '?').join(', ')})`;
    params.push(...visible);
  }
...
module.exports = {
  ...
  pruneProject, pruneUntracked, projectRowCount, indexedProjects,

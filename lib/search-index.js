...
// Every project this index currently holds rows for, read off the per-project
// fingerprints rather than `select distinct project from entries`: the
// fingerprint is written by the only per-project writer (replaceProject) and
// cleared by the only per-project remover (removeProject, below), so the two
// agree by construction — and `entries` is an FTS5 table, where a distinct
// scan means reading every row of the thing this index exists to avoid
// scanning.
//
// This is what lets a caller notice a project that has LEFT the tracked set.
// Iterating the tracked set can only ever find projects that are still in it,
// which is precisely why rows for a hidden project were never revisited.
function indexedProjects(db) {
  return db.prepare('select k from meta where k like ?').all(`${FP_PREFIX}%`)
    .map(r => String(r.k).slice(FP_PREFIX.length))
    .filter(Boolean);
}

// Drop one project from the index entirely: its rows AND its fingerprint.
//
// The fingerprint has to go with the rows. Left behind, it describes a corpus
// that is no longer stored, so a project that later becomes visible again
// would compare as already-fresh and the freshener would skip refilling the
// rows this just deleted -- the same trap replaceAll documents above, arrived
// at from the other direction.
function removeProject(db, projectPath) {
  inTransaction(db, () => {
    db.prepare('delete from entries where project = ?').run(projectPath);
...
function runMatch(db, match, opts = {}) {
  ...
  //
  // This is the SAFETY half of the fix; the purge in lib/activity.js's
  // freshenIndex is the hygiene half. Neither replaces the other: the purge
  // runs on a schedule, so between a user hiding a project and the next
  // freshen there is a window in which the rows are still on disk, and this
  // predicate is what makes that window harmless.
  //
...
module.exports = {
  open, close, indexPath, replaceAll, replaceProject, removeProject,
  indexedProjects, projectFingerprint,

...
// LAST RESORT ONLY, and knowingly incomplete -- it cannot cover nvm, asdf or an
// arbitrary custom prefix, which is exactly why the login-shell query above it
// exists. Exported so `membridge status` can show what was tried.
//
// WHY THE PER-USER PREFIXES ARE HERE AND NOT LEFT TO THE SHELL QUERY. The query
// runs `command -v claude`, which can only see directories already ON PATH. A
// per-user prefix that the user never added to PATH is therefore invisible to
// it -- the query returns exit 1 and we fall through to this list. That is not
// hypothetical: it is the shape that shipped this bug. `~/.npm-global` is the
// directory npm's own "resolving EACCES permissions" guide tells users to
// create (`npm config set prefix ~/.npm-global`), so it is the most common
// user-owned npm prefix there is, and a `claude` installed into it is reachable
// by the shell only if the user also edited their rc file -- which that guide
// asks for as a separate step, and which is skipped constantly.
//
// So these are not guesses at where someone might have put it. Each is a
// location an OFFICIAL installer writes to, which is what separates them from
// the arbitrary prefixes this list correctly refuses to chase.
const CANDIDATES = [
  path.join(os.homedir(), '.local', 'bin', 'claude'),
  // Claude Code's own local-install target (`claude migrate-installer`).
  path.join(os.homedir(), '.claude', 'local', 'claude'),
  // npm's documented user-owned global prefix -- frequently off PATH.

...
async function readAccess(config, projectPath) {
  ...
  // S-54: THE PROJECT'S DEFAULT IS REPORTED OR REFUSED, NEVER GUESSED.
  //
  // This one backend answer decides two things the panel states as fact: the
  // "New members join with access" toggle, and `canSee` for every member with
  // no explicit project_access row. It used to read
  //
  //     Array.isArray(projectRows) && projectRows.length ? !!projectRows[0].default_access : true
  //
  // so an empty or unreadable answer became `true` — the same defect shape as
  // the H-4 bug immediately above (a value asserting something the code never
  // established), reached the same way (an empty PostgREST result is not a
  // fact). 028 narrowed the guess from a hardcoded `true` to the project's real
  // default; it could not remove the guess, because the guess is in the caller.
  //
  // Reachable state, no adversary: `.membridge/team.json` is a COMMITTED file
  // that linkProject deliberately adopts byte-identical, while public.projects
  // is backend state. A manager whose project row is gone from the backend
  // while the project is still linked locally lands here. (The H-4 manager gate
  // above runs FIRST on purpose: a non-manager must still get 403, and must not
  // learn from a 409/502 whether the project row exists.)
  //
  // WHY REFUSE RATHER THAN FALL CLOSED. Falling closed would be a different
  // lie in the opposite direction, and not a harmless one: every member renders
  // toggled OFF, AccessPanel emits its "loses access to this project right away"
  // note once per member (ui/src/features/project/AccessPanel.tsx), so the
  // manager reads a revocation event that never happened — and the obvious
  // remedy, flipping a toggle back on, writes a real project_access row keyed to
  // a project no predicate can reach. A fabricated alarm plus a plausible,
  // inert fix is worse than no panel.
  //
  // WHY NOT A THIRD "UNKNOWN" STATE. It is the most truthful model and it
  // cannot land here alone: ui/src/data/LocalDaemonClient.ts getProjectAccess
  // coerces with `?? true`, AccessPanel's `rows.filter(row => !row.canSee)`
  // reads null as hidden, and `<Toggle on={null}>` renders off — so shipping
  // `null` from the daemon degrades into exactly the fall-closed lie above.
  // That needs a UI ticket; this stops at the daemon boundary.
  //
  // Refusing degrades honestly in today's UI with no change at all: ProjectPage
  // renders the panel only `if (showAccessPanel && accessRows.length > 0)`, so
  // an error omits the panel rather than filling it in. Omission is silent
  // (filed as a follow-up: the reason should be shown), but silence beats a
  // confident false statement about who can read a project.
  //
  // Two statuses because they need two different remedies, and reporting either
  // as the other is its own misstatement. Neither reuses 401/403/404 — each
  // already means something specific on this endpoint.
  if (!Array.isArray(projectRows)) {
    // A 200 whose body restCall could not parse (`res.json().catch(() => null)`).
    // Transient: retry.
    throw new AccessError(502, "couldn't read the backend's answer about who can see this project — try again");
  }
  if (!projectRows.length) {
    // Read fine, no row for this project id. Durable: the local link points at
    // a project the backend does not have.
    throw new AccessError(409,
      'the backend has no record of this project, so who can see it cannot be determined — ' +
      'it is still linked locally, so relink it (`membridge team link`) to restore its access list');
  }
  const rawDefault = projectRows[0].default_access;
  if (typeof rawDefault !== 'boolean') {
    // projects.default_access is `boolean not null` (026:15), and 028's header
    // calls that NOT NULL the load-bearing fact behind can_see_project failing
    // closed. If a null ever arrives here the constraint is gone, which is
    // precisely the state where inventing EITHER boolean is wrong.
    throw new AccessError(502, "the backend did not report this project's default access — try again");
  }
  const defaultAccess = rawDefault;
  // A member with no project_access row falls back to the PROJECT's default,
  // which is now always a value the backend actually stated. Enforcement is
  // can_see_project's job (028_enforce_project_access_default.sql); this must
  // agree with it, or the panel tells an admin the opposite of what the backend
  // will do.
  //
  // activeMembers: this is a CONTROL surface -- each row is a per-project
  // visibility toggle an admin can flip. A soft-deleted account (auth.users
  // .deleted_at set; the team_members row survives it, so listMembers still
  // returns them) has no access to grant or revoke, and offering the toggle
  // invites an admin to act on somebody who is gone. Present-tense state, so
  // it is filtered; readAudit below deliberately is NOT -- see its comment.
  return {
    members: teamsync.activeMembers(members).map(m => ({
      memberId: m.user_id,
      name: m.display_name,
...
async function writeAccess(config, rawBody) {
  ...
  await writeAudit(config, creds, {
    ...
    // NO `path` HERE. This used to carry projectPath -- the ACTOR'S ABSOLUTE
    // LOCAL PATH, e.g. /Users/marco/billing-poc -- which readAudit surfaced as
    // objectName and the panel rendered to every manager on the team. Two
    // separate faults in one field: it published a home directory and a
    // username into a shared trail, and it told a reader whose checkout is
    // somewhere else precisely nothing. The row already carries the identity
    // that travels (objectKey is link.projectId, the backend project id), so
    // readAudit resolves the project's SHARED name from that instead.
    detail: { memberId, canSee },
  });
  return { ok: true };
}
...
async function writeAccessDefault(config, rawBody) {
  ...
  await writeAudit(config, creds, {
    ...
    objectType: 'project',
    objectKey: link.projectId,
    detail: { defaultAccess }, // no `path` -- see writeAccess above
  });
...
async function accessMatrix(config, teamId) {
  ...
  // Same reasoning as readAccess: the matrix is every local project x every
  // member, and every cell is an actionable grant. A deleted account is not a
  // column anybody should be filling in.
  const memberList = teamsync.activeMembers(members).map(m => ({ id: m.user_id, name: m.display_name }));
...
async function readAudit(config, limit, teamId) {
  ...
  // NOT filtered through activeMembers, unlike readAccess and accessMatrix
  // above, and this is deliberate. Here the roster is only a NAME-RESOLUTION
  // MAP for historical rows: dropping a deleted account would render their
  // past actions as actorName 'Unknown', which is the precise bug 050's
  // closing note warns about -- a row saying we do not know, when the record
  // says exactly who it was. Whether a deleted person's name should stay on
  // work they did is the open question in docs/ACCOUNT-DELETION.md §8.4, and
  // it is not answered by quietly breaking the audit trail here.
  ...
  // The same treatment projects get that people already got. A project-typed
  // row's object_key IS the backend project id, so its name can be resolved
  // from the team's own project list -- one shared label every teammate sees
  // identically, instead of whatever the actor's checkout happened to be
  // called on their disk.
  //
  // Resolving at READ time rather than capturing at write time is what makes
  // this fix reach HISTORICAL rows: every access row ever written already
  // carries the project id, so rows written before this change get the right
  // name too, with nothing rewritten. team_audit is append-only by design
  // (024 §5) and rewriting it was never an option.
  //
  // Best-effort, like every other lookup in this file: a blip must degrade a
  // row's LABEL, never fail the panel that is trying to show what happened.
  const projectRows = await teamsync.projectStats(config, team.team_id).catch(() => []);
  const projectNameById = new Map((projectRows || [])
    .filter(r => r && r.project_id && r.name)
    .map(r => [String(r.project_id), r.name]));
  ...
  return {
    events: rows.map(r => {
      ...
      return {
        id: r.id,
        at: r.created_at,
        // Roster first, then — only when the row's ACTOR IS ITS SUBJECT — the
        // name captured on the row at write time, then 'Unknown'.
        //
        // The self-acting rows are the ones this matters for, and they are
        // exactly the ones about people who leave: `member-joined` (046, actor
        // and subject are both the joiner) and `own-data-deleted` (035, which
        // exists SO THAT an ex-member can erase what they pushed — its actor
        // has often already gone). Before 046 the gap was reachable only
        // through 035 and never exercised; now every departed member has a
        // join row naming them as actor, so "Unknown joined the team" would be
        // the most common line in the trail of anybody who has left. That is
        // the same failure the targetName fix below addresses, on the sibling
        // field, and 046's header names it as a known limit — this closes it.
        //
        // The actor === subject guard is what keeps this honest: a name is
        // only ever reused for the person the row already says it is about.
        // A row where a manager acted ON someone else falls through to
        // 'Unknown' rather than borrowing the target's name and reporting the
        // wrong person as the actor, which would be worse than saying nothing.
        actorName: (() => {
          // FIRST, and the order is load-bearing. A null actor_id is not "we
          // could not resolve this person" — it is a positive fact that 050
          // introduced: `team_audit.actor_id ... on delete set null`, so null
          // means their ACCOUNT WAS DELETED. Collapsing it into 'Unknown'
          // would resurrect the bug this file already fixed once, on a row the
          // database can describe exactly.
          //
          // It must also never reach the self-acting fallback below. That
          // fallback reuses the name captured on the row, which is right for a
          // member who LEFT (they still exist and still have a name) and
          // exactly wrong for an account that was erased — the row would
          // announce who it was, and the deletion would be cosmetic. Checking
          // null first is what stops that, so do not reorder these branches.
          if (r.actor_id == null) return 'A deleted account';
          const roster = nameById.get(r.actor_id);
          if (roster) return roster;
          const selfActing = detail && typeof detail.memberId === 'string' &&
            detail.memberId === r.actor_id;
          const captured = detail && typeof detail.targetName === 'string' && detail.targetName
            ? detail.targetName : null;
          return (selfActing && captured) || 'Unknown';
        })(),
        action: r.action,
        ...
        // Shared project name first; then, for a row written before the path
        // was dropped, the BASENAME of that historical path.
        //
        // The basename rather than the whole path, and rather than nothing:
        // showing it raw keeps leaking the home directory and username the
        // fix exists to remove, and showing nothing turns an old row into
        // "that project" and loses what it was about. The basename keeps the
        // row's meaning and drops the part that was never anyone else's
        // business. It is a LABEL of last resort only -- the row's identity is
        // still object_key, which is the project id and is never a basename,
        // so two projects that happen to share a folder name remain two rows
        // about two different projects.
        objectName: (() => {
          const shared = projectNameById.get(String(r.object_key || ''));
          if (shared) return shared;
          if (detail && typeof detail.path === 'string' && detail.path) {
            return path.basename(detail.path) || null;
          }
          return null;
        })(),
        // Roster name first, then the name CAPTURED WITH THE EVENT, then null.
        // Never the raw user id — that is the case the comment above names as
        // useless, and this line used to produce it: the ternary read
        // `nameById.get(detail.memberId) || detail.memberId`, so a detail
        // carrying BOTH memberId and targetName never reached the targetName
        // branch at all. member-removed carries exactly that shape, and
        // lib/server.js reads the display name off the roster before the
        // removal precisely so this row would not "render as a bare user id
        // forever" — that capture was dead code.
        //
        // Latent until 044: before it, a removed member could walk back in with
        // the standing invite code, so they were usually back ON the roster by
        // the time anyone read the trail and the lookup happened to succeed.
        // Making removal durable made the fallback reachable, and it rendered a
        // UUID where the correct name was sitting one field away.
        //
        // Null rather than the id when neither is available, per the rule
        // above: absent and unknown must stay distinguishable, and the UI
        // already says "someone who has since left" for null
        // (ui/src/features/members/auditText.ts:35).
        targetName: (() => {
          if (!detail) return null;
          const captured = typeof detail.targetName === 'string' && detail.targetName
            ? detail.targetName : null;
          if (typeof detail.memberId === 'string') return nameById.get(detail.memberId) || captured;
          return captured;
        })(),
        // `path` is stripped on the way out, not just ignored. Rows written
        // before the fix still hold the actor's absolute path in the database
        // -- team_audit is append-only by design (024 §5) so they cannot be
        // rewritten -- and shipping the raw detail would keep handing that
        // home directory and username to every manager who opens the panel.
        // objectName above has already taken the basename it needs, so
        // nothing downstream loses anything by this.
        detail: (() => {
          if (r.detail == null) return null;
          if (!detail || !('path' in detail)) return JSON.stringify(r.detail);
          const { path: _dropped, ...rest } = detail;

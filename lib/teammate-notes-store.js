...
function backfillProjects(state, now) {
  ...
  for (const [key, proj] of Object.entries(projects)) {
    try {
      ...
      // the existsSync guard below never rewrites an index once present, that
      // index was PERMANENT: teammate decisions, gotchas and per-file notes kept
      // being served into every agent's context for a project the user had
      // explicitly unlinked.
      //
      // The removal is the self-heal, and it is what makes this fix reach
      // installs that unlinked BEFORE it landed: those already have both the
      // stale rows and the stale index on disk, and nothing else would ever go
      // back for them. Deleting is safe because this file is a DERIVED cache —
      // re-linking rebuilds it from teamEntries on the next pull, so the worst
      // case of being wrong is one rebuild, while the worst case of leaving it
      // is indefinite disclosure.
      //
      ...
      if (!isTeamLinked(key)) {
        try {
          if (fs.existsSync(notesPath(key))) {
...
module.exports = {
  notesPath, read, write, update, rebuildTeammateNotes, backfillProjects, afterTeamPull,

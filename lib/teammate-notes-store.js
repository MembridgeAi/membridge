...
function backfillProjects(state, now) {
  ...
  for (const [key, proj] of Object.entries(projects)) {
    try {
      ...
      //
      // REMOVED, not emptied, and that asymmetry with the revoked branch below
      // is deliberate. Re-linking has to rebuild the index, and the only thing
      // that would rebuild it is this backfill -- which skips any project that
      // already HAS an index. clearTeammateNotes leaves an empty file behind, so
      // using it here would make the erase permanent until a teammate happened
      // to push again. The revoked branch has no such obligation: access is not
      // coming back on its own.
      //
      // Deleting is safe because this file is a DERIVED cache -- re-linking
      // rebuilds it from teamEntries on the next pass, so the worst case of
      // being wrong is one rebuild, while the worst case of leaving it is
      // indefinite disclosure. That removal is also what reaches installs which
      // unlinked BEFORE this landed: they already have both the stale rows and
      // the stale index on disk, and nothing else would ever go back for them.
      //
      ...
      if (!isTeamLinked(key)) {
        try {
          if (hasIndex) {
            fs.unlinkSync(notesPath(key));
            util.log(`teammate notes: dropped the index for ${key} — no longer linked to a team`);
          }
        } catch { /* best-effort, same rule as the archive prune on unlink */ }
        continue;
      }

...
function backfillProjects(state, now) {
  ...
  for (const [key, proj] of Object.entries(projects)) {
    try {
      if (!proj || !Array.isArray(proj.teamEntries) || !proj.teamEntries.length) continue;
      if (fs.existsSync(notesPath(key))) continue;
      if (rebuildTeammateNotes(key, proj.teamEntries, now)) rebuilt.push(key);
    } catch { /* next project */ }
  }
  return rebuilt;
}

module.exports = { notesPath, read, write, update, rebuildTeammateNotes, backfillProjects };

...
//
// It is ALSO the reconciler for the opposite direction. A revoked project's
// index has to be erased even on a pass that never reaches syncTeams'
// access-lost branch — and there is such a pass: when visibleProjectIds cannot
// answer (`visible === null`, a network blip) that branch is skipped entirely,
// while teamAccessLost stamped by an earlier, confirmed revocation still stands.
// Erasing here as well means the leak closes on ANY pass, not only the pass
// that first noticed.
//
...
//
// The unlink case is handled at its source (teamsync.unlinkProject) rather than
// here, because on disk an unlinked project is indistinguishable from a project
// that simply carries rows without a link file — a shape the legacy suite pins.
...
module.exports = { notesPath, read, write, update, rebuildTeammateNotes, clearTeammateNotes, backfillProjects, afterTeamPull };

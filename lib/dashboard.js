  ...
  units.forEach(function (u) {
    ...
    if (!c) {
      c = byKey[k] = {
        ...
        sources: [], // tool union across the day's units, accumulated below
    ...
    // Day-level tool union, deduped, newest-first (same rationale as
    // finalizeUnit's u.sources: c.source is the newest unit's tool only, and a
    // mixed Claude+Codex day rendered from it reads as all-newest-tool).
    // Old-shape units without a sources field degrade to their single source.
    (u.sources && u.sources.length ? u.sources : (u.source ? [u.source] : []))
      .forEach(function (s) { if (c.sources.indexOf(s) === -1) c.sources.push(s); });
...
function finalizeUnit(u) {
  ...
  // The unit's tool UNION, deduped, newest-first. u.source alone is the newest
  // entry's tool — a mixed burst (Claude + Codex within one 30-min window)
  // rendered from it alone reads as the newest tool only, hiding the other.
  var srcs = [];
  u.runs.forEach(function (r) {
    (r.entries || []).forEach(function (e) {
      if (e.source && srcs.indexOf(e.source) === -1) srcs.push(e.source);
    });
  });
  u.sources = srcs;
...
// Tool chips for a card meta row — one pill PER tool in the union. Rolled-up
// cards (unit bursts, day cards) can span tools; rendering only the newest
// one made every mixed day read as all-Claude, with Codex work invisible
// until the Tool filter narrowed the feed to it. The fallback arg covers
// old-shape data that only carries the single newest source.
function toolPillsHtml(sources, fallback) {
  var list = sources && sources.length ? sources : (fallback ? [fallback] : []);
  return list.map(function (s) {
    return '<span style="padding:2px 9px;border-radius:99px;border:1px solid var(--border);' + MONO + ';font-size:10px;letter-spacing:.04em;color:var(--text2)">' + esc(s) + '</span>';
  }).join('');
}
...
function unitHtml(u, opts) {
  ...
  var head = '<div data-card-toggle="' + keyAttr + '" style="display:flex;gap:14px;align-items:flex-start;padding:2px 12px 2px 4px;margin:0 -12px 0 -4px;cursor:pointer;border-radius:14px;transition:background .2s" style-hover="background:var(--surface2)">' +
    ...
    '<div style="flex:1;min-width:0">' + liveLabel +
      ...
      '<div style="display:flex;align-items:center;gap:9px;margin-top:7px;font-size:12px;color:var(--text3);flex-wrap:wrap">' +
        '<span style="color:var(--text2);font-weight:500">' + esc(who) + '</span>' +
        toolPillsHtml(u.sources, u.source) +
        // opts.hideProject: the project page is all one project, the pill is noise.
  ...
  var head = '<div data-day-open="' + keyAttr + '" style="display:flex;gap:14px;align-items:flex-start;padding:2px 12px 2px 4px;margin:0 -12px 0 -4px;cursor:pointer;border-radius:14px;transition:background .2s">' +
    ...
    '<div style="flex:1;min-width:0">' +
      '<div style="display:flex;align-items:center;gap:9px;font-size:12px;color:var(--text3);flex-wrap:wrap">' +
        '<span style="color:var(--text2);font-weight:500">' + esc(who) + '</span>' +
        toolPillsHtml(c.sources, c.source) +
  ...
  var cards = ordered.map(function (u) {
    ...
    var foot = '<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:9px">' +
      ...
      '<div style="display:flex;align-items:center;gap:10px;flex:none">' +
        // Per-session tool pill(s): the day card above rolls tools up into one
        // union row, so the drill-down is where a mixed day says which
        // session was which tool.
        toolPillsHtml(u.sources, u.source) + share +

...
/* ---- threaded feed: one thread per session, day-grouped by its newest entry.
...
function threadKey(e) {
  ...
  return [e.origin, e.authorId || e.author || '', e.projectId || e.projectPath || e.project || '', e.session].join('|');
}
...
// One session thread: header = the session's Result (or the amber in-progress
// fallback), meta = author · tool · project · time · "N prompts ▾"; children =
// the session's individual asks, indented behind a left line, collapsed by
// default. Reuses the data-card-toggle contract (header + next sibling), so
// the existing view-feed toggle handler and catchupExpanded persistence work
// unchanged, and the chevron must stay the LAST span in the header.
function threadHtml(t) {
  ...
  var head = '<div data-card-toggle="' + esc(t.key) + '" style="display:flex;gap:14px;align-items:flex-start;padding:17px 12px 17px 4px;margin:0 -12px 0 -4px;cursor:pointer;border-radius:14px;transition:background .2s" style-hover="background:var(--surface2)">' +
    '<div style="width:28px;height:28px;border-radius:10px;background:' + color + ';color:#fff;font-size:12px;font-weight:600;display:flex;align-items:center;justify-content:center;flex:none;margin-top:1px;box-shadow:var(--shadow-md)">' + initial + '</div>' +
    '<div style="flex:1;min-width:0">' +
      '<div style="font-size:15px;font-weight:600;letter-spacing:-0.01em;line-height:1.45;text-wrap:pretty">' + headline + '</div>' +
      '<div style="display:flex;align-items:center;gap:9px;margin-top:7px;font-size:12px;color:var(--text3);flex-wrap:wrap">' +
        '<span style="color:var(--text2);font-weight:500">' + esc(who) + '</span>' +
      ...
      '</div>' +
    '</div></div>';
  // Children read oldest-first (the session in the order it happened). A null
  // ask stays "(prompt not shared)" — redaction already happened server-side.
  var kids = t.entries.slice().reverse().map(function (e) {
    return '<div style="display:flex;gap:10px;align-items:baseline;padding:6px 0">' +
      '<div style="flex:1;min-width:0;font-size:13px;line-height:1.55;' + (e.ask ? 'color:var(--text2);font-style:italic' : 'color:var(--text3)') + '">' +
  ...
  var kidsBlock = '<div style="display:' + (open ? 'block' : 'none') + '">' +
    '<div style="margin:2px 0 18px 13px;padding:2px 0 2px 25px;border-left:2px solid var(--border)">' + kids + '</div></div>';
  return '<article style="border-bottom:1px solid var(--border)">' + head + kidsBlock + '</article>';
}

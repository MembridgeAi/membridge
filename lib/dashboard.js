  ...
  fetch('/api/projects').then(function (r) { return r.json(); }).then(function (projects) {
    ...
    fetch('/api/catchup').then(function (r) { return r.ok ? r.json() : {}; }).then(function (cu) {
      ...
      feedP.then(function (f) {
        ...
        var fp = JSON.stringify({ cu: cu, e: entries, p: projects, off: offline });
        if (fp === catchupFp && host.firstChild) { refreshAgo('view-home'); return; }
        catchupFp = fp;
        renderCatchup(host, cu, entries, projects, offline);
      });
    }).catch(function () { setPill(false); });
  }).catch(function () { setPill(false); });
}
...
function renderWelcome(host) {
  Promise.all([
    ...
    host.innerHTML = cuMainOpen() + cuSectionLabel() + cuTitle('Welcome to', 'MemBridge', false) +
...
function renderCatchup(host, cu, sinceEntries, projects, offline) {
  ...
  // ---- ALL CAUGHT UP (template lines 150–161) ----
  if (onTeam && !others.length) {
    host.innerHTML = cuMainOpen() + cuSectionLabel() + cuTitle('You’re all', 'caught up', false) + cuSince(cu) +
  ...
  // ---- NO TEAM (template lines 164–178) ----
  if (!onTeam) {
    host.innerHTML = cuMainOpen() + cuSectionLabel() + cuTitle('You’re not on', 'a team yet', false) + cuSince(cu) +
  ...
  var headlineCards = others.map(function (e) { return catchupCardHtml(e); }).join('');
  ...
  host.innerHTML = cuMainOpen() + cuSectionLabel() + cuTitle('While you were', 'out', true) + cuSince(cu) +
    // A · AI BRIEFING (template lines 206–218) + no-key hint slot (219–221)
    ...
    '<section style="margin-bottom:56px">' +
      '<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">' +
        '<div style="display:inline-flex;align-items:center;gap:8px;border-radius:99px;border:1px solid var(--accent-brd);background:var(--accent-soft);padding:4px 13px">' +
          ...
          '<span style="font-family:\\'JetBrains Mono\\',monospace;font-size:10px;letter-spacing:.15em;text-transform:uppercase;color:var(--accent)">Headlines</span></div>' +
        '<div style="flex:1"></div>' +
...
document.getElementById('view-home').addEventListener('click', function (e) {
  ...
  if (cu) {
    ...
    if (act === 'see-everything') { window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }); return; }

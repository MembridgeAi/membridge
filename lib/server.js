...
async function teamPayload() {
  ...
  return {
    ...
    // credential, and until 039 my_teams handed it to every member whatever
    ...
    // removed. 039 §2 makes the backend return null for a non-manager; this
    // is the same rule stated on the daemon side, so a backend that has not
    // had 039 applied yet does not leak through the client either. Belt and

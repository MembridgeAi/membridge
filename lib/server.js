...
async function handle(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === APP_PREFIX) {
      ...
      // No single team to attribute this to (the rename applies across every
      // team the member is on), so it follows the same fallback the other
      // body-less-teamId route (delete-my-data) uses: firstTeamId(), plain
      // teams[0]. recordAudit swallows its own failures, so a teamless
      // account (firstTeamId() -> null) just skips the audit row rather than
      // erroring the request.
      await apiAccess.recordAudit(getConfig(), {
        teamId: await firstTeamId(), action: 'member-renamed', objectType: 'member', objectKey: result.displayName,
        detail: { name: result.displayName, avatar: result.avatar, avatarColor: result.avatarColor },
      });
      json(res, 200, result);

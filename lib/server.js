...
const { syncOnce, getAdapters, findProjectKey, scanAll, trackedRoots, isTrackedProject } = require('./scan');
...
function scanPayload() {
  ...
  const projects = Object.entries(state.projects).map(([key, proj]) => {
    ...
    return {
      ...
      sessionCount: proj.events.length,
      bySource,
    };
  });
...
// Forget a project: strip injected blocks, drop its .membridge-beta dir and state.
...
module.exports = { startServer, statusPayload, projectsPayload, projectDetail, planPayload, toggleProject, addProject, adoptProjects, deleteProject, removeBlockFromProject, copyPayload, settingsPayload, saveSettings, teamPayload, teamProjectsPayload, runTeamSync, scanPayload, feedPayload, catchupPayload, markCaughtUp, undoCaughtUp };

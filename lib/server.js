...
const { normalizeAvoided, normalizeHoldout, normalizeNotes } = require('./ledger-fold-state');
...
// sparkline. Reads the same events the sessionIds/firstActivity loop below
// already walks, so this stays a single pass over proj.events rather than a
// second full scan.
const DAY_MS = 24 * 60 * 60 * 1000;
function dailySessionBuckets(proj, now) {
  const buckets = Array.from({ length: 7 }, () => new Set());
  for (const ev of proj.events) {
    if (!ev.session || digest.isPlumbing(ev)) continue;
    const t = Date.parse(ev.ts);
    if (!Number.isFinite(t)) continue;
    const age = now - t;
    if (age < 0 || age >= 7 * DAY_MS) continue;
    buckets[6 - Math.floor(age / DAY_MS)].add(ev.session);
  }
...
module.exports = { startServer, statusPayload, projectBlockPayload, projectsPayload, projectDetail, planPayload, toggleProject, addProject, adoptProjects, deleteProject, removeBlockFromProject, copyPayload, settingsPayload, saveSettings, teamPayload, teamProjectsPayload, runTeamSync, scanPayload, savingsPayload, feedPayload, catchupPayload, markCaughtUp, undoCaughtUp };

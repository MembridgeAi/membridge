...
async function handle(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === APP_PREFIX) {
      ...
      // The one audit event whose actor IS the subject. Written after the
      // join so the row lands on a team the caller is now a member of —
      // team_audit's RLS insert policy requires exactly that.
      if (joined && joined.team_id) {
        await apiAccess.recordAudit(getConfig(), {
          teamId: joined.team_id, action: 'member-joined', objectType: 'member', objectKey: null, detail: null,
        });
      }
      json(res, 200, joined);
      ...
      // invite on its own), so the event is filed against the caller's team —
      // the same teams[0] every other team read in this daemon uses.
      const revokeTeam = await firstTeamId();
      if (revokeTeam) {
        await apiAccess.recordAudit(getConfig(), {
    ...
    // (for the deletion) one who has already left the team. The backend
    // policy is scoped on authorship alone for that reason (028 §1). Anyone
    ...
    //
    // The only gate here is the typed confirmation on the POST, and that is
    // an are-you-sure, not an authorization check.
    // -----------------------------------------------------------------------
    } else if (req.method === 'GET' && url.pathname === '/api/team/my-data') {
      // The preview the confirmation screen reads its number from. Counts
      // come from the my_entry_counts RPC (security definer), NOT from a
      // local read: what matters is what is on the BACKEND, which is not the
      // same set as what is on this disk (other machines push too, and a
      // revoked project would under-report through the normal select path).
      ...
      // Lets the CLI and the UI name a folder instead of a uuid, and is the
      // value `--project` takes. Absent for a project only ever synced from
      // another machine, which is a real and different state from "unknown".
      ...
      if (!teamId) return json(res, 400, { error: 'team is required' });
      // Second deliberate step, after the collapsed disclosure the control
      // lives behind. 400 rather than 403: the caller is allowed to do this,
      // they just have not confirmed it.
      ...
      // audit row is written inside the RPC's own transaction (028 §3) rather
      ...
      // only visible sign of the scope of a write nothing else surfaces -- the
      // CLI prints it, and it is what a support conversation about "this
      // project stopped syncing" starts from.
...
module.exports = { startServer, boundPort, oauthCallbackPage, statusPayload, projectBlockPayload, projectsPayload, projectDetail, planPayload, toggleProject, addProject, adoptProjects, deleteProject, removeBlockFromProject, archiveProject, unarchiveProject, copyPayload, settingsPayload, saveSettings, teamPayload, teamProjectsPayload, runTeamSync, scanPayload, savingsPayload, measuredSpendPayload, feedPayload, sessionPayload, catchupPayload, markCaughtUp, undoCaughtUp, dailySessionBuckets, lastManagerBlock };

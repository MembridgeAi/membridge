...
function addProject(projectPath, opts = {}) {
  ...
  const backfilled = opts.backfill === false ? [] : resetOffsetsForAdoption(state, resolved);
  saveState(state);
  return { path: resolved, added: true, backfilled: backfilled.length };
}
...
// Forget a project: strip injected blocks, drop its .membridge dir and state.
// Offsets are left consumed here; the re-add is what recovers the history,
// because addProject backfills (alpha readiness F3). Before that, this line
// read "only future activity revives it", which F3 cites as the defect.

...
function projectBlockPayload(projectPath) {
  ...
  for (const target of effectiveTargets(config)) {
    ...
    // lastIndexOf, to match digest.inject() exactly — see the comment there.
    // This is the READER for the same span inject() REWRITES, so the two must
    // resolve it identically or the dashboard describes a block the writer
    // does not agree exists. With indexOf, an end-marker above the real block
    // (a CLAUDE.md that documents these markers, or a hand-edit) made the
    // payload either an honest-looking empty or, worse, a fabricated block
    // spanning the prose between two inline markers.
    const end = text.lastIndexOf(digest.END);

...
async function handle(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === APP_PREFIX) {
      ...
      // Manager-only, same shape as /api/team/audit above: readInvites carries
      // its own 401/403 status on the error, so it must not fall through to
      // the generic 500 handler below, which would turn a legitimate "you are
      // a member, not an admin" refusal into a server fault.
      try {

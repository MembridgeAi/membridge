...
async function feedPayload(opts) {
  ...
  // Home reads these lightweight flags to pick its empty / no-team state
  // without a second round-trip to /api/team.
  const out = feed.buildFeed({ local: f, team, teamUnavailable, limit });
  return { ...out, signedIn: !!creds, hasTeam: teamList.length > 0 };
}

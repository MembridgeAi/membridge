...
//
// That claim is now true all the way down, which it previously was not. A USD
// total WAS computed and written to every user's disk (ledger.json's
// inCost/outCost) and merely withheld from the wire, so the honest sentence
// was "computed and stored, never served". Nothing prices the ledger any
// more: lib/ledger-fold.js persists no dollar field and drops any a legacy
// ledger still carries, so there is no stale figure left to leak.
...
function deleteProject(projectPath) {
  ...
  // section, capture off) with nothing on screen connecting the two. The
  // config.exclude half is deliberately left alone -- a manual pause predates
  // archiving and is the user's own standing choice about that path.
  try {
    const raw = loadUserConfig();
    ...
    // lands here too and leaves the stale entry behind, which silently
    // restores the re-add-comes-back-archived trap, so it is logged rather
    // than swallowed.
    log(`delete: could not prune ${key} from config.archived: ${err.message}`);
  }
...
module.exports = { startServer, boundPort, statusPayload, projectBlockPayload, projectsPayload, projectDetail, planPayload, toggleProject, addProject, adoptProjects, deleteProject, removeBlockFromProject, archiveProject, unarchiveProject, copyPayload, settingsPayload, saveSettings, teamPayload, teamProjectsPayload, runTeamSync, scanPayload, savingsPayload, measuredSpendPayload, feedPayload, sessionPayload, catchupPayload, markCaughtUp, undoCaughtUp, dailySessionBuckets };

...
// Newest recorded commit timestamp for a project, or null when nothing is
// recorded. The dashboard's capture-health check compares this against the
// project's last captured session event: commits are written by the git
// post-commit hook and events by the transcript scan, so commits landing while
// events have gone silent means one path is alive and the other is not — the
// failure that otherwise just looks like a quiet project. Full parse, but the
// map is one small file per project.
function lastCommitTs(projectPath) {
  let newest = null;
  for (const r of loadCommitMap(projectPath)) {
    if (r && typeof r.ts === 'string' && (!newest || r.ts > newest)) newest = r.ts;
  }
  return newest;
}
...
module.exports = {
  ...
  recordCommit, loadCommitMap, lastRecordedSha, lastCommitTs, commitMapPath,
  isProvisionalCommit,
};

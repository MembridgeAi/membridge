...
// with what arguments. Respects the diagnostics DISK switch at record time
// (tallyEnabled, not diagnosticsEnabled): a durable
// `config.diagnostics.enabled === false` opts the tally out too, but a
// single-session MEMBRIDGE_NO_DIAGNOSTICS=1 does NOT — that env is a
// send-only override, and this write has no network in its path. Every path
// swallows errors — a failed tally must never surface into a tool response,
// and the MCP process must never gain a network call for this.
const fs = require('fs');
...
function recordToolUse(tool, opts = {}) {
  try {
    ...
    if (!diagnostics.tallyEnabled(opts.config)) return;
    ...
    // tick (same rationale as lib/team-archive.js's writeAtomic), so a fixed
...
// MCP_TOOLS is re-exported (not re-declared) so anything reporting the tally
// as a coverage figure -- "N of 6 memory tools in use" -- reads its
// denominator from the very allowlist toolsUsedWithin filters against. One
// array, so the two halves of that fraction cannot drift apart.
module.exports = { recordToolUse, toolsUsedWithin, tallyPath, MCP_TOOLS };

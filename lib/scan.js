...
function syncOnce(opts = {}) {
  ...
  for (const key of projectKeys) {
    ...
    // pass, not a dry run), so it stays cheap -- rebuilt once per project per
    // dirty pass, not on every poll. Best-effort like the memory update: a
    // write failure here must not block the rest of the sync.
    if (!opts.dryRun) {
      try {
        ledgerStore.writeLedger(key, ledgerStore.buildProjectLedger(proj.events || []));
      } catch (err) {
...
module.exports = { readNewLines, getAdapters, scanAll, scanSummaries, syncOnce, findProjectKey, trackedRoots, isTrackedProject, filterTrackedSessions, filterScratchpadResidue, foldWorktreeProjects, cleanupCodexMislabels, reopenMislabeledForeignCodex, SETTLE_GRACE_MS };

...
//
//   • Settle ATTRIBUTED (attributeCommit credits session S): only when S's
//     OWN newest scanned event is newer than the commit — the per-session
//     sequential-read guarantee then proves S's evidence at or before the
//     commit is complete. If a credited candidate's own events do NOT yet
//     pass the commit, stay provisional — and deliberately do NOT fall
//     through to the unattributed grace path: S's still-unscanned edits are
//     exactly the pending evidence.
//   • Settle UNATTRIBUTED (attributeCommit credits nobody): there is no
//     candidate whose file order can prove anything, so wait out a grace
//     window — settle only once the newest scanned event (any session) is
//     more than SETTLE_GRACE_MS past the commit ts. The window bounds the
//     cross-file scan race (how far one session file's draining can lag
//     another's); it is deterministic — it compares two DATA timestamps
//     (event ts vs commit ts), never the wall clock, so a pass replayed
//     over the same data always decides the same way.
//
...
function settleProvisionalCommits(key, proj) {
  ...
  // Newest scanned event ts, global and per session — the per-session map
  // is what the attributed gate leans on (see the header comment).
  ...
  for (const row of pending) {
    try {
      ...
      if (att.sessions.length) {
        // Attributed gate: EVERY credited session's own events must pass the
        // commit (a settled row is final, so a partially-proven attribution
        // waits whole). If any candidate is still catching up, stay
        // provisional — no grace fallback while pending evidence exists.
      ...
      } else if (!(newestEventTs > commitTs + SETTLE_GRACE_MS)) {

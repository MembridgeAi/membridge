...
function syncOnce(opts = {}) {
  ...
  for (const key of projectKeys) {
    ...
    // A directory git is mid-way through rewriting is not ours to write into:
    // see lib/repo-root.js's gitOperationInProgress for the measurement. Asked
    // ONCE per directory per pass, before any target, so a two-target config
    // cannot answer differently for CLAUDE.md and AGENTS.md and leave one of
    // them written. Per-directory rather than per-project because a rebase in
    // one worktree says nothing about its siblings.
    const busyDirs = new Map();
    if (!opts.dryRun) {
      for (const dir of injectDirs) {
        const marker = repoRoot.gitOperationInProgress(dir);
        if (marker) busyDirs.set(dir, marker);
      }
    }
    // Targets this project could not be written to this pass. Collected per
    // project (like busyDirs above) because the retry decision below is made
    // once for the project, not once per file.
    const injectErrors = [];
    for (const target of effectiveTargets(config)) {
      ...
      for (const dir of injectDirs) {
        ...
        } else if (busyDirs.has(dir)) {
          // Reported as a real outcome, not silently dropped: `skipped` is what
          // the CLI and the dashboard render, so a user who wonders why their
          // block is a few minutes stale during a long rebase can see why.
          changes.push({ file, action: 'deferred', reason: `git ${busyDirs.get(dir)} in progress` });
        } else {
          // Best-effort, exactly like the memorydb and ledger writes below --
          // and this was the ONE unguarded write in this loop. A target that
          // cannot be written (read-only mount, root-owned file, a directory
          // sitting where the file belongs, a full disk) threw straight out of
          // syncOnce, and saveState(state) is the LAST statement in this
          // function: the throw discarded the entire pass's offset advances and
          // lastSync stamps, and every project ordered after this one was never
          // injected at all. bin/membridge.js's tick() catches that throw into a
          // single log line, so it presented as a daemon reporting "running"
          // while capture stayed frozen and each later pass re-did and re-lost
          // the same work.
          try {
            if (digest.inject(file, block, digest.preambleFor(target))) {
              changes.push({ file, action: 'updated' });
            }
          } catch (err) {
            injectErrors.push(`${target}: ${err.message}`);
            changes.push({ file, action: 'failed', reason: err.message });
            log(`context block write failed for ${file}: ${err.message}`);
          }
        }
      }
    }
    ...
    if (!opts.dryRun) {
      // A deferred write is NOT a completed sync, and must not be recorded as
      // one. This codebase's characteristic defect is a flag marking a success
      // the code never achieved, and both of these lines are that flag: clearing
      // `dirty` would mean the next pass skips this project entirely (a full sync
      // only visits dirty projects), stranding a stale block until the user's
      // next agent turn; stamping `lastSync` would tell the badge, `membridge
      // status` and the MCP that work is captured when the file on disk still
      // predates it. Leaving both alone is what makes the deferral a RETRY:
      // dirty persists, so the very next tick after the rebase ends writes.
      //
      // A failed BLOCK WRITE is the same situation arriving by a different
      // route, so it takes the same branch: the file on disk still predates
      // this pass, and staying dirty is what makes the next tick retry once the
      // permission/space problem is resolved. Reported in `skipped` rather than
      // swallowed -- the CLI and dashboard render it, so "my block stopped
      // updating" has a visible cause instead of being silent.
      if (busyDirs.size || injectErrors.length) {
        skipped.push({
          project: key,
          reason: busyDirs.size
            ? `git ${[...busyDirs.values()][0]} in progress`
            : `could not write the context block (${injectErrors[0]})`,
        });
      } else {

...
// via the escape, premature attributed — settling). Events whose ts is
// absurdly past the pending commits' own ts (> SETTLE_TS_SANITY_MS beyond
// the newest pending commit) are ignored by the settle gates entirely —
// they still exist for attribution content, just not as catch-up proof.
//
...
// Sanity ceiling for event timestamps used as catch-up proof: an event dated
// more than this far past the newest PENDING commit's own ts is treated as
// clock corruption and ignored by the settle gates (it cannot legitimately
// certify anything about scan progress). 24h is far beyond any real
// commit-to-next-event gap within one working session, and far below the
// decades a genuinely garbage timestamp tends to be off by.
...
function settleProvisionalCommits(key, proj) {
  ...
  // Clamp reference: the newest pending commit's recorded ts. Events beyond
  // it + SETTLE_TS_SANITY_MS are ignored below (see the header comment).
  let maxPendingTs = -Infinity;
  for (const row of pending) {
    const t = Date.parse(row.ts);
    if (Number.isFinite(t) && t > maxPendingTs) maxPendingTs = t;
  }
  const tsCeiling = Number.isFinite(maxPendingTs) ? maxPendingTs + SETTLE_TS_SANITY_MS : Infinity;

...
const { settleRows, zeroAvoided, zeroHoldout, zeroComparison } = require('./ledger-fold-recall-settle');
...
function settleRecallQueue(projectPath, readEvents, prevLedger, nowFn = Date.now, maxLines = MAX_FOLD_LINES) {
  ...
  // The controlled comparison's delta, summed the same way from the same two
  // passes: settleRows adds this pass's newly settled serves and held-out
  // skips, applyCorrections re-prices served reads that a follow-up read
  // corrected. Both arms, both fields, one loop -- so a field added to one arm
  // and forgotten in the other is not possible.
  const comparisonDelta = zeroComparison();
  for (const arm of Object.keys(comparisonDelta)) {
    for (const k of Object.keys(comparisonDelta[arm])) {
      comparisonDelta[arm][k] = ((settled.comparisonDelta && settled.comparisonDelta[arm][k]) || 0)
        + ((corrected.comparisonDelta && corrected.comparisonDelta[arm][k]) || 0);
    }
  }

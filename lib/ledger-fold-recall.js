...
const { normalizeOpenServes, normalizeHoldout, pushBounded, LIMITS } = require('./ledger-fold-state');
...
function settleRecallQueue(projectPath, readEvents, prevLedger, nowFn = Date.now, maxLines = MAX_FOLD_LINES) {
  ...
  const settled = settleRows(rows, now, buildOpenServe, knownServeIds, knownHoldoutKeys);
  ...
  return { avoidedDelta, holdoutDelta: settled.holdoutDelta, holdoutSeenKeys, openServes, finish };
}

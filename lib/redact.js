...
const DEFAULT_PATTERNS = [
  ...
  // http/https/ssh/ftp/git/smb sit alongside the database schemes: the list
  // started as database-only, which left `https://user:pass@host` — the single
  // commonest credential-in-URL shape there is, and the one a `git clone` of a
  // private repo puts straight into a prompt — matching nothing at all. The
  // user:pass@ shape is what identifies this, not the scheme, so a wider
  // scheme list costs no precision: an ordinary URL has no `:` `@` pair
  // before its host.
  { name: 'connection-uri', repl: '$1[redacted:credentials]@',
    rx: /\b((?:postgres|postgresql|mysql|mongodb(?:\+srv)?|redis|amqp|https?|ssh|sftp|ftps?|git|smb):\/\/)[^\s:@/]+:[^\s:@/]+@/gi },

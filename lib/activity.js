...
function searchMemory(args) {
  ...
  // corpus invariant, sharing projectEntries above is the other half.
  ...
  // Usage tracking (lib/retrievals.js), for the entries actually SERVED —
  // the slice, never the full ranked list. Annotate each result with how
  // many times it had been retrieved BEFORE this call (so the number an
  // agent reads is history, not self-fulfilling), then append this serve.

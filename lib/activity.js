...
function freshenIndex(db, state, config, regexes, self = selfUserId()) {
  ...
  const tracked = trackedProjectEntries(state, config);
  // Purge before refilling: rows for projects that have LEFT the tracked set.
  //
  // This loop used to iterate `tracked` alone, and that is a set a hidden
  // project is by definition not in — so the moment a user paused, archived,
  // dropped a `.membridge-off` marker into, or deleted a project, its rows
  // were never revisited again. They stayed in the index forever, and
  // search_memory kept serving that project's asks, summaries, decisions,
  // gotchas and file paths after every other surface had stopped
  // acknowledging it existed.
  //
  // Doing it HERE covers all four controls with one implementation. The
  // alternative — purging inside deleteProject (lib/server.js) — would fix
  // only deletion and leave pause, archive and the marker file untouched,
  // and each of those is a user asking for the same thing.
  //
  // Hygiene, not the safety property: it bounds the index to what is
  // reachable rather than letting it grow without limit. What makes a stale
  // row unservable in the meantime is the visible-projects predicate pushed
  // into SQL by searchMemoryVia below.
  const visible = new Set(tracked.map(([key]) => key));
  let purged = 0;
  for (const key of searchIndex.indexedProjects(db)) {
    if (visible.has(key)) continue;
    searchIndex.removeProject(db, key);
    purged += 1;
  }
  let refreshed = 0;
  for (const [key, proj] of tracked) {
